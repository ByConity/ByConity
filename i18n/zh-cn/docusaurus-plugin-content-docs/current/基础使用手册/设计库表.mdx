...
### 设计 Bucket
Bucket 表是一个可选的表级设置，如果设置合适的Bucket, 系统会根据用户提交的 Cluster Key(即建表语句中提供的一个或者多个列、表达式）去整理表数据，将相同值的数据聚簇在同一个 bucket number 下。
...
由于Bucket表的设置比较复杂，请参考后文中 Bucket 表最佳实践的细则来设置。一般情况下，两种场景适合设置：
