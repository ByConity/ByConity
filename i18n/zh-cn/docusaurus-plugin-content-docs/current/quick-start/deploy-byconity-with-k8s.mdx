...
高可用的ByConity集群功能开启需要打开所有组件的zookeeper配置，参考server.yaml
...
```
官方提供了高可用的helm配置yaml文件，供参考：https://github.com/ByConity/byconity-deploy/blob/master/examples/k8s/value_HA_example.yaml。
使用helm更新集群：
```bash
helm upgrade --install --create-namespace --namespace byconity -f ./your/custom/values.yaml byconity ./chart/byconity
```
