...
namespace DB
{
    namespace
    {
                    ...
                    typeid_cast<const DataTypeFixedString*>(user_pro_type.get()))
                {
                    if constexpr (need_order)
                        res.reset(createWithSingleTypeLastKnown<AggregateFunctionFinderGroupFunnel>(
                            *attr_type, window, watch_start, watch_step,
                            watch_numbers, user_pro_idx, window_type, time_zone, num_virts, attr_related, time_interval, true, argument_types, params));
                    else
                        res.reset(createWithSingleTypeLastKnown<AggregateFunctionFinderGroupFunnel>(
                            *attr_type, window, watch_start, watch_step,
                            watch_numbers, user_pro_idx, window_type, time_zone, num_virts, attr_related, time_interval, false, argument_types, params));
                }
                else
                {
                    if constexpr (need_order)
        ...
        {
            if constexpr (need_order)
                return createAggregateFunctionFinderGroupFunnel<true>(name, argument_types, params);
            else
                return createAggregateFunctionFinderGroupFunnel<false>(name, argument_types, params);
        }
        ...
        {
            return createAggregateFunctionFinderGroupFunnel<false>(name, argument_types, params, true);
        }
    ...
    {
        factory.registerFunction("finderGroupFunnel", createAggregateFunctionFinderGroupFunnelHelper<false>, AggregateFunctionFactory::CaseInsensitive);
        ...
        factory.registerFunction("finderGroupFunnelStable", createAggregateFunctionFinderGroupFunnelHelper<true>, AggregateFunctionFactory::CaseInsensitive);
    }
}
