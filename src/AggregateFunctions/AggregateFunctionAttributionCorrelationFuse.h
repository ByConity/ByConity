...
struct AggregateFunctionAttributionCorrelationFuseData
{
    ...
    void merge(const AggregateFunctionAttributionCorrelationFuseData &other, Arena *arena)
    {
...
class AggregateFunctionAttributionCorrelationFuse final
        : public IAggregateFunctionDataHelper<AggregateFunctionAttributionCorrelationFuseData, AggregateFunctionAttributionCorrelationFuse> {
private:
    UInt64 N; // Return the largest first N events by value
    bool need_others; // Weather need other conversion

public:
    AggregateFunctionAttributionCorrelationFuse(
            UInt64 N_, bool need_others_,
            const DataTypes &arguments, const Array &params) :
            IAggregateFunctionDataHelper<AggregateFunctionAttributionCorrelationFuseData, AggregateFunctionAttributionCorrelationFuse>(
                    arguments, params),
            N(N_), need_others(need_others_) {}

    String getName() const override
    {
        return "attributionCorrelationFuse";
    }
       ...
       AnalysisEnum * outer_result = const_cast<AnalysisEnum *>(&(this->data(place).outer_result));
        ...
        else
        {
            ...
            for (size_t i = 0; i < outer_result->features.size(); i++)
                outer_result->correlation[i] = AggregateFunctionAttributionCorrelationFuseData::getRankCorrelation(outer_result->features[i]);
        }
    ...
    void getTopByValue(AnalysisEnum &outer_result) const
    {
        ...
        for (size_t i = 0; i < index_vec.size(); i++)
            outer_result.correlation[i] = AggregateFunctionAttributionCorrelationFuseData::getRankCorrelation(outer_result.features[index_vec[i].second]);
    }
