...
public:
    AggregateFunctionNullVariadic(AggregateFunctionPtr nested_function_, const DataTypes & arguments, const Array & params)
        : AggregateFunctionNullBase<result_is_nullable, serialize_flag,
            AggregateFunctionNullVariadic<result_is_nullable, serialize_flag>>(std::move(nested_function_), arguments, params),
        ...
        ssize_t if_argument_pos) const override
    {
        ...
        {
            final_flags = std::make_unique<UInt8[]>(batch_size);
            ...
            if (included_elements == 0)
                return;
            if (included_elements != batch_size)
            {
        ...
        this->setFlag(place);
        this->nested_function->addBatchSinglePlaceNotNull(
            batch_size, this->nestedPlace(place), nested_columns, final_flags_ptr, arena, -1);
    }
