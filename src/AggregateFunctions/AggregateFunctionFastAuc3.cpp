...
#include <AggregateFunctions/AggregateFunctionFastAuc3.h>
...
createAggregateFunctionFastAuc3(const std::string & name, const DataTypes & argument_types, const Array & parameters, const Settings *)
{
    ...
    AggregateFunctionPtr res(createWithTwoNumericTypes<FunctionFastAuc3>(*argument_types[0], *argument_types[1], argument_types, parameters, precision, min, max));
    if (!res)
...
void registerAggregateFunctionFastAuc3(AggregateFunctionFactory & factory)
{
    factory.registerFunction("fastAuc3", createAggregateFunctionFastAuc3, AggregateFunctionFactory::CaseInsensitive);
}
