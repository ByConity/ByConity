...
template<typename P, typename L>
class FunctionRegAucV2: public IAggregateFunctionDataHelper<FunctionRegAucV2Data<P, L>, FunctionRegAucV2<P, L>>
{
private:
    UInt64 num_reg_sample; // Number of max sample count to calculate regression AUC
    UInt64 flag;

    Float64 calcRegAucV2(ConstAggregateDataPtr __restrict place) const
    {
        auto & pair_array = const_cast<typename FunctionRegAucV2Data<P, L>::Array&>(this->data(place).pair_array);
    ...
    size_t calc_correct_pairs(PODArray<std::pair<P, L>, 4096, ArenaAllocator>& pair_array, const UInt64 p_flag) const {
        ...
        */
        ...
        for (; j < n; ++j) {
            if (j + 1 == n || pair_array[j + 1].first != pair_array[i].first) {
                cnt0 += (j + 1 - i) * (j - i) / 2;
                i = j + 1;
            }

            if (j + 1 == n || pair_array[j + 1] != pair_array[k]) {
                cnt1 += (j + 1 - k) * (j - k) / 2;
                k = j + 1;
            }
        }
    ...
    size_t calc_label_equal_pairs(PODArray<std::pair<P, L>, 4096, ArenaAllocator>& pair_array) const {
        ...
        size_t cnt = 0;
        for (; j < n; ++j) {
            if (j + 1 == n || pair_array[j + 1].second != pair_array[i].second) {
                cnt += (j + 1 - i) * (j - i) / 2;
                i = j + 1;
            }
        }
...
public:
    FunctionRegAucV2(const DataTypes & argument_types_,
                const UInt64 num_reg_sample_ = 1000000, const UInt64 flag_ = 1)
        : IAggregateFunctionDataHelper<FunctionRegAucV2Data<P, L>, FunctionRegAucV2<P, L>>(argument_types_, {}),
          num_reg_sample(num_reg_sample_), flag(flag_)
    {}

    String getName() const override
    { return "regression_auc2"; }
    ...
    {
        Float64 result = calcRegAucV2(place);
