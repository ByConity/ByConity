...
Void QueryAnalyzerVisitor::visitASTSelectWithUnionQuery(ASTPtr & node, const Void &)
{
    ...
    bool normalized = (select_with_union.is_normalized
                       && (select_with_union.union_mode == ASTSelectWithUnionQuery::Mode::ALL
                           || select_with_union.union_mode == ASTSelectWithUnionQuery::Mode::DISTINCT))
        || select_with_union.list_of_selects->children.size() == 1;
    if (!normalized)
        throw Exception("Unsupported ASTSelectWithUnionQuery", ErrorCodes::NOT_IMPLEMENTED);
                                           ...
                                           const String & right_table_qualifier, ASTSelectQuery & select_query)
{
    ...
    // validate join type & strictness, see also: https://bytedance.feishu.cn/sheets/shtcni8iFkRGalg0C9KDnR3Ieze
    {
