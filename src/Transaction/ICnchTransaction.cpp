...
{
    if (!global_context.getConfigRef().getBool("enable_lock_manager", true))
    {
    ...
    bool locked = cnch_lock->tryLock(global_context);
    if (locked)
    {
        {
...
void ICnchTransaction::reportLockHeartBeat()
{
    ...
    auto local_host_port = std::to_string(global_context.getRPCPort());
    for (const auto & host_port : host_ports)
    {
        ...
        {
            auto client = global_context.getCnchServerClientPool().get(host_port);
