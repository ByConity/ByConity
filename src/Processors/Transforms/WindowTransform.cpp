    ...
    , dt(dialect_type_)
{
    ...
    for (const auto & f : functions)
    {
        ...
        if (workspace.window_function_impl && workspace.window_function_impl->hasSecondStage())
        {
...
struct WindowFunctionNthValue final : public WindowFunction<IWindowFunction>
{
        ...
        : WindowFunction(name_, argument_types_, parameters_)
    {
        ...
        {
            throw Exception(ErrorCodes::BAD_ARGUMENTS,
                "Function {} takes exactly two arguments", name);
        }
...
struct StatefulWindowFunction : public WindowFunction<IWindowFunction>
{
        ...
        if (argument_types.size() == 3)
        {
            ...
            if (!interval_datatype->getKind().isFixedLength())
            {
                throw Exception(
                    ...
                    "The INTERVAL unit must be able to convert to fixed seconds, e.g., a second, minute, hour, week, '{}' given",
                    argument_types[ARGUMENT_INTERVAL]->getName());
            }
