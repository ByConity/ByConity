...
void registerOutputFormatCapnProto(FormatFactory & factory)
{
    ...
    {
        return std::make_shared<CapnProtoRowOutputFormat>(buf, sample, params,
                                                        FormatSchemaInfo(format_settings.schema.format_schema, "CapnProto", true,
                                                                format_settings.schema.is_server, format_settings.schema.format_schema_path), format_settings);
    });
}
