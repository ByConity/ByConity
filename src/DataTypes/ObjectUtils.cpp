...
namespace ErrorCodes
{
    ...
    extern const int TOO_MANY_SUBCOLUMNS_IN_JSON;
}
...
static std::pair<ColumnPtr, DataTypePtr> recursivelyConvertDynamicColumnToTuple(
    const ColumnPtr & column, const DataTypePtr & type)
{
    ...
    if (const auto * type_array = typeid_cast<const DataTypeArray *>(type.get()))
    {
        ...
        auto [new_column, new_type] = recursivelyConvertDynamicColumnToTuple(
    ...
    if (const auto * type_tuple = typeid_cast<const DataTypeTuple *>(type.get()))
    {
        ...
        for (size_t i = 0; i < tuple_size; ++i)
        {
            std::tie(new_tuple_columns[i], new_tuple_types[i])
                = recursivelyConvertDynamicColumnToTuple(tuple_columns[i], tuple_types[i]);
        }
...
static bool needCheckTypeCompatibilityLocal(const IStorage & storage)
{
    const auto * cloud_table = dynamic_cast<const StorageCloudMergeTree *>(&storage);
    return cloud_table == nullptr;
}
...
void convertDynamicColumnsToTuples(Block & block, const StorageSnapshotPtr & storage_snapshot)
{
    for (auto & column : block)
    {
        ...
        std::tie(column.column, column.type) = recursivelyConvertDynamicColumnToTuple(column.column, column.type);
        ...
        if (needCheckTypeCompatibilityLocal(storage_snapshot->storage))
        {
            ...
            getLeastCommonTypeForDynamicColumns(storage_column->type, {column.type, storage_column_concrete.type}, true);
        }
    }
}
...
void checkObjectHasNoAmbiguousPaths(const PathsInData & paths)
{
...
static DataTypePtr getLeastCommonTypeForObject(const DataTypes & types, bool check_ambiguous_paths)
{
    ...
    if (check_ambiguous_paths)
        checkObjectHasNoAmbiguousPaths(tuple_paths);
...
void limitObjectSubcolumns(const ColumnsDescription & object_columns, const UInt64 & subcolumns_threshold)
{
    auto columns = object_columns.getAll();
    for (const auto & column : columns)
    {
        auto subcolumns = object_columns.getSubcolumns(column.getNameInStorage());

        if (subcolumns.size() > subcolumns_threshold)
            throw Exception(
                ErrorCodes::TOO_MANY_SUBCOLUMNS_IN_JSON,
                fmt::format(
                    "Column [{}] has [{}] subcolumns exceed threshold [{}].",
                    column.getNameInStorage(),
                    subcolumns.size(),
                    subcolumns_threshold));
    }
}
...
UInt64 getColumnsCommitTimeForJSONTable(const IStorage & table, const NamesAndTypesList & search_part_columns)
{
    ...
    auto exclude_json_column = [&](const NamesAndTypesList & columns) -> NamesAndTypesList {
        NamesAndTypesList res;
        for (const auto & column : columns)
        {
            if (std::find(exclude_column_names.begin(), exclude_column_names.end(), column.name) == exclude_column_names.end())
                res.push_back(column);
        }
        return res;
    };

    auto search_part_columns_excluded_json = exclude_json_column(search_part_columns);

    if (search_part_columns_excluded_json == exclude_json_column(*table.part_columns))
        return table.commit_time.toUInt64();
    for (const auto & version : table.previous_versions_part_columns)
    {
        if (search_part_columns_excluded_json == exclude_json_column(*version.second))
