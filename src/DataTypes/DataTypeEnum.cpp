...
template <typename Type>
Field DataTypeEnum<Type>::stringToVisitorField(const String & ins) const
{
    // This routine assume ins was generated by FieldVisitorToString
    ReadBufferFromString rb(ins);

    NearestFieldType<Type> res;
    readText(res, rb);
    return res;
}

template <typename Type>
String DataTypeEnum<Type>::stringToVisitorString(const String & ins) const
{
    Field field = stringToVisitorField(ins);
    Type val = DB::get<Type>(field);
