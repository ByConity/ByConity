...
bool ExternalDictionaryLibraryBridgeHelper::bridgeHandShake()
{
    ...
    try
    {
        ReadWriteBufferFromHTTP buf(getPingURI(), Poco::Net::HTTPRequest::HTTP_GET, {}, http_timeouts);
...
ReadWriteBufferFromHTTP::OutStreamCallback ExternalDictionaryLibraryBridgeHelper::getInitLibraryCallback() const
{
    ...
    auto output_stream = getContext()->getOutputStream(ExternalDictionaryLibraryBridgeHelper::DEFAULT_FORMAT, out, sample_block);
...
BlockInputStreamPtr ExternalDictionaryLibraryBridgeHelper::loadAll()
{
    startBridgeSync();
...
BlockInputStreamPtr ExternalDictionaryLibraryBridgeHelper::loadIds(const std::vector<uint64_t> & ids)
{
    startBridgeSync();
...
BlockInputStreamPtr ExternalDictionaryLibraryBridgeHelper::loadKeys(const Block & requested_block)
{
    startBridgeSync();
    ...
    ReadWriteBufferFromHTTP::OutStreamCallback out_stream_callback = [requested_block, this](std::ostream & os)
    {
        ...
        auto output_stream = getContext()->getOutputStream(ExternalDictionaryLibraryBridgeHelper::DEFAULT_FORMAT, out_buffer, sample_block);
...
BlockInputStreamPtr ExternalDictionaryLibraryBridgeHelper::loadBase(const Poco::URI & uri, ReadWriteBufferFromHTTP::OutStreamCallback out_stream_callback)
{
    ...
    auto input_stream = getContext()->getInputFormat(ExternalDictionaryLibraryBridgeHelper::DEFAULT_FORMAT, *read_buf_ptr, sample_block, DEFAULT_BLOCK_SIZE);
