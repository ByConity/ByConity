...
public:
    ...
    DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName & arguments) const override
    {
        ...
        if (!argument_type.isDate() && !argument_type.isDateTime() && !argument_type.isDateTime64() && !argument_type.isStringOrFixedString())
            throw Exception(
        ...
        size_t input_rows_count) const override
    {
        ...
        {
            const auto output = std::make_shared<DataTypeDateTime64>(0);
            ColumnsWithTypeAndName temporary_columns{
                ColumnWithTypeAndName{datetime_resolver->build(arguments)->execute(arguments, output, input_rows_count), output, ""}
            };
            return executeImpl(temporary_columns, result_type, input_rows_count);
        }
...
private:
    ...
    FunctionOverloadResolverPtr datetime_resolver;
};
