...
namespace
{
    ...
    struct DateTime
    {
        ...
        [[maybe_unused]] void setEra(const String & text) // NOLINT
        {
            if (text == "bc")
    ...
    private:
        ...
        public:
            ...
            {
                if unlikely(cur > end || cur + len > end)
                    throw Exception(
            ...
            static Pos mysqlHHMMSSAMPM(Pos cur, Pos end, const String & fragment, DateTime & date)
            {
                checkSpace(cur, end, 8, "mysqlHHMMSSAMPM requires size >= 11", fragment);
                ...
                Int32 & result)
            {
                ...
                if unlikely(max_digits_to_read >= std::numeric_limits<decltype(number)>::digits10)
                    throw Exception(
                        ErrorCodes::CANNOT_PARSE_DATETIME,
                ...
                if unlikely(number < std::numeric_limits<Int32>::min() || number > std::numeric_limits<Int32>::max())
                    throw Exception(
                        ErrorCodes::CANNOT_PARSE_DATETIME,
