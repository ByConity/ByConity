...
static inline bool likePatternIsStrstr(const String & pattern, String & res, [[maybe_unused]] char escape_char)
{
    res = "";
...
template <>
inline bool likePatternIsStrstr<true>(const String & pattern, String & res, const char escape_char)
{
    res = "";
...
struct EscapeMatchImpl
{
        ...
        PaddedPODArray<UInt8> & res)
    {
        ...
        if ((like && escape_char == '\\' && likePatternIsStrstr<false>(pattern, strstr_pattern, escape_char)) ||
            (like && escape_char != '\\' && likePatternIsStrstr<true>(pattern, strstr_pattern, escape_char)))
        {
    ...
    static void vectorFixedConstant(
        const ColumnString::Chars & data, size_t n, const std::string & pattern, const char escape_char,
        PaddedPODArray<UInt8> & res)
    {
        if (data.empty())
            {
        ...
        if ((like && escape_char != '\\' && likePatternIsStrstr<true>(pattern, strstr_pattern, escape_char))
            || (like && escape_char == '\\' && likePatternIsStrstr<false>(pattern, strstr_pattern, escape_char)))
        {
            const UInt8 * begin = data.data();
            const UInt8 * pos = begin;
            const UInt8 * end = pos + data.size();
