...
public:
    ...
    {
        bool reversed_order = mysql_mode && name == "locate";
        const ColumnPtr & column_haystack = arguments[reversed_order].column;
        const ColumnPtr & column_needle = arguments[!reversed_order].column;
        ...
        if constexpr (isEscapeMatchImpl<Impl>())
        {
            ...
            char escape_char = '\\';
            {
                auto escape_seq = typeid_cast<const ColumnConst *>(&*arguments[2].column)->getValue<String>();
                if (!escape_seq.empty())
                {
