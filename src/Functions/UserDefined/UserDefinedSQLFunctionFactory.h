...
class 
UserDefinedSQLFunctionFactory : public IHints<1, UserDefinedSQLFunctionFactory>
{
public:
    ...
    bool registerFunction(const ContextMutablePtr & context, const String & resolved_function_name, const String & function_name, ASTPtr create_function_query, bool throw_if_exists, bool replace_if_exists);

    /// Unregister function for function_name.
    bool unregisterFunction(const ContextMutablePtr & context, const String &db_name, const String &function_name, bool throw_if_not_exists, bool cnch_local);
    ...
    bool tryGetVersion(const String &function_name, uint64_t** version_ptr) const;

    std::vector<uint64_t> tryGetAllVersions() const;
    ...
    static String getResolvedFunctionName(const String & database_name, const String &function_name);

    bool dropFunctionLocally(const ContextMutablePtr & context, const String &function_name, bool throw_if_not_exists);
...
private:
    ...
    void setFunction(const String & function_name, const IAST & create_function_query, const ContextPtr &context);
