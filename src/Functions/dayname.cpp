...
public:
    ...
    explicit FunctionMonthName(ContextPtr context) : context_ptr(context) { }
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t input_rows_count) const override
    {
        DataTypePtr month_type = std::make_shared<DataTypeString>();
        auto month_col = month_type->createColumnConst(input_rows_count, Field("month"));
        ColumnsWithTypeAndName input_col = {{month_col, month_type, "month"}};
