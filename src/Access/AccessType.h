...
\
    ...
    M(ALTER_CLEAR_MAP_KEY, "CLEAR MAP KEY", COLUMN, ALTER_COLUMN) \
...
\
    ...
    M(ALTER_ADD_FOREIGN_KEY, "ADD FOREIGN KEY", TABLE, ALTER_CONSTRAINT) \
    M(ALTER_DROP_FOREIGN_KEY, "DROP FOREIGN KEY", TABLE, ALTER_CONSTRAINT) \
    M(ALTER_ADD_UNIQUE_NOT_ENFORCED, "ADD UNIQUE", TABLE, ALTER_CONSTRAINT) \
    M(ALTER_DROP_UNIQUE_NOT_ENFORCED, "DROP UNIQUE", TABLE, ALTER_CONSTRAINT) \
...
\
    ...
    M(ALTER_SETTINGS, \
      "ALTER SETTING, ALTER MODIFY SETTING, MODIFY SETTING, RESET SETTING", \
      TABLE, \
      ALTER_TABLE) /* allows to execute ALTER MODIFY SETTING */ \
    ...
    M(ALTER_REPAIR_PARTITION, "ALTER REPAIR PART, REPAIR PARTITION", TABLE, ALTER_TABLE) \
    M(ALTER_FREEZE_PARTITION, "FREEZE PARTITION, UNFREEZE", TABLE, ALTER_TABLE) \
    M(ALTER_CLUSTER_BY, "ALTER MODIFY CLUSTER BY, MODIFY CLUSTER BY", TABLE, ALTER_TABLE) \
    M(ALTER_RECLUSTER_PARTITION, "ALTER RECLUSTER PARTITION, ALTER RECLUSTER PARTITION ID, ALTER RECLUSTER PARTITION WHERE", TABLE, ALTER_TABLE) \
    \
...
\
    ...
    M(ROLE_ADMIN, \
      "", \
      GLOBAL, \
      ACCESS_MANAGEMENT) /* allows to grant and revoke the roles which are not granted to the current user with admin option */ \
    ...
    M(SET_SENSITIVE, "", COLUMN, ACCESS_MANAGEMENT) \
...
\
    ...
    M(SYSTEM_DROP_COMPILED_EXPRESSION_CACHE, \
      "SYSTEM DROP COMPILED EXPRESSION, DROP COMPILED EXPRESSION CACHE, DROP COMPILED EXPRESSIONS", \
      GLOBAL, \
      SYSTEM_DROP_CACHE) \
    ...
    M(SYSTEM_RELOAD_EMBEDDED_DICTIONARIES, \
      "RELOAD EMBEDDED DICTIONARIES", \
      GLOBAL, \
      SYSTEM_RELOAD) /* implicitly enabled by the grant SYSTEM_RELOAD_DICTIONARY ON *.* */ \
    ...
    M(SYSTEM_DISTRIBUTED_SENDS, \
      "SYSTEM STOP DISTRIBUTED SENDS, SYSTEM START DISTRIBUTED SENDS, STOP DISTRIBUTED SENDS, START DISTRIBUTED SENDS", \
      TABLE, \
      SYSTEM_SENDS) \
    M(SYSTEM_REPLICATED_SENDS, \
      "SYSTEM STOP REPLICATED SENDS, SYSTEM START REPLICATED SENDS, STOP_REPLICATED_SENDS, START REPLICATED SENDS", \
      TABLE, \
      SYSTEM_SENDS) \
    M(SYSTEM_SENDS, "SYSTEM STOP SENDS, SYSTEM START SENDS, STOP SENDS, START SENDS", GROUP, SYSTEM) \
    M(SYSTEM_REPLICATION_QUEUES, \
      "SYSTEM STOP REPLICATION QUEUES, SYSTEM START REPLICATION QUEUES, STOP_REPLICATION_QUEUES, START REPLICATION QUEUES", \
      TABLE, \
      SYSTEM) \
    ...
    M(SYSTEM_RECALCULATE_METRICS, "RECALCULATE METRICS", TABLE, SYSTEM) \
...
\
    ...
    M(INTROSPECTION, \
      ...
      GROUP, \
      ALL) /* allows to execute functions addressToLine(), addressToSymbol(), demangle()*/ \
\
