...
/** Column for String values.
  */
class ColumnSketchBinary final : public COWHelper<IColumn, ColumnSketchBinary>
{
public:
    using Char = UInt8;
...
private:
    friend class COWHelper<IColumn, ColumnSketchBinary>;
    ...
    ColumnSketchBinary() = default;
    ColumnSketchBinary(const ColumnSketchBinary & src);

public:
    const char * getFamilyName() const override { return "Sketch"; }
    TypeIndex getDataType() const override { return TypeIndex::SketchBinary; }
    ...
    {
        assert(n < size());
        return Field(&chars[offsetAt(n)], sizeAt(n) - 1, true);
    }
...
#pragma GCC diagnostic pop
    ...
    {
        const ColumnSketchBinary & src = assert_cast<const ColumnSketchBinary &>(src_);
    ...
    {
        return scatterImpl<ColumnSketchBinary>(num_columns, selector);
    }
    ...
    {
        return typeid(rhs) == typeid(ColumnSketchBinary);
    }
