...
public:
    ...
    {
        assert(n < size());
        return JsonbField(reinterpret_cast<const char *>(&chars[offsetAt(n)]), sizeAt(n) - 1);
    }

    void get(size_t n, Field & res) const override
    {
        assert(n < size());
        JsonbField jsonb_field(reinterpret_cast<const char *>(&chars[offsetAt(n)]), sizeAt(n) - 1);
        res = jsonb_field;
    }
...
#pragma GCC diagnostic ignored "-Wmaybe-uninitialized"
    ...
    {
        const JsonbField & s = DB::get<const JsonbField &>(x);
        const size_t old_size = chars.size();
        const size_t size_to_append = s.getSize()+ 1;
        const size_t new_size = old_size + size_to_append;

        chars.resize(new_size);
        memcpy(chars.data() + old_size, s.getValue(), size_to_append);
...
#pragma GCC diagnostic pop
    ...
    {
        return scatterImpl<ColumnJsonb>(num_columns, selector);
    }
    ...
    {
        return getRatioOfDefaultRowsImpl<ColumnJsonb>(sample_ratio);
    }

    UInt64 getNumberOfDefaultRows() const override
    {
        return getNumberOfDefaultRowsImpl<ColumnJsonb>();
    }

    void getIndicesOfNonDefaultRows(Offsets & indices, size_t from, size_t limit) const override
    {
        return getIndicesOfNonDefaultRowsImpl<ColumnJsonb>(indices, from, limit);
    }
