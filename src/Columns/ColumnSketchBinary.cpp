...
ColumnSketchBinary::ColumnSketchBinary(const ColumnSketchBinary & src)
    : COWHelper<IColumn, ColumnSketchBinary>(src),
...
MutableColumnPtr ColumnSketchBinary::cloneResized(size_t to_size) const
{
    auto res = ColumnSketchBinary::create();
...
void ColumnSketchBinary::updateWeakHash32(WeakHash32 & hash) const
{
    auto s = offsets.size();
...
void ColumnSketchBinary::insertRangeFrom(const IColumn & src, size_t start, size_t length)
{
    if (length == 0)
        return;

    const ColumnSketchBinary & src_concrete = assert_cast<const ColumnSketchBinary &>(src);
...
void ColumnSketchBinary::insertRangeSelective(const IColumn & src, const Selector & selector, size_t selector_start, size_t length)
{
    const ColumnSketchBinary & src_concrete = static_cast<const ColumnSketchBinary &>(src);
...
ColumnPtr ColumnSketchBinary::filter(const Filter & filt, ssize_t result_size_hint) const
{
    if (offsets.empty())
...
StringRef ColumnSketchBinary::serializeValueIntoArena(size_t n, Arena & arena, char const *& begin) const
{
...
const char * ColumnSketchBinary::deserializeAndInsertFromArena(const char * pos)
{
...
const char * ColumnSketchBinary::skipSerializedInArena(const char * pos) const
{
    const size_t string_size = unalignedLoad<size_t>(pos);
    pos += sizeof(string_size);
    return pos + string_size;
...
ColumnPtr ColumnSketchBinary::index(const IColumn & indexes, size_t limit) const
{
    return selectIndexImpl(*this, indexes, limit);
}

template <typename Type>
ColumnPtr ColumnSketchBinary::indexImpl(const PaddedPODArray<Type> & indexes, size_t limit) const
{
    if (limit == 0)
        return ColumnSketchBinary::create();
...
void ColumnSketchBinary::compareColumn(
    const IColumn & rhs, size_t rhs_row_num,
    PaddedPODArray<UInt64> * row_indexes, PaddedPODArray<Int8> & compare_results,
    int direction, int nan_direction_hint) const
{
    return doCompareColumn<ColumnSketchBinary>(assert_cast<const ColumnSketchBinary &>(rhs), rhs_row_num, row_indexes,
                                         compare_results, direction, nan_direction_hint);
}

bool ColumnSketchBinary::hasEqualValues() const
{
    return hasEqualValuesImpl<ColumnSketchBinary>();
}

struct ColumnSketchBinary::ComparatorBase
{
    const ColumnSketchBinary & parent;

    explicit ComparatorBase(const ColumnSketchBinary & parent_)
        : parent(parent_)
    {
    }
...
struct ColumnSketchBinary::ComparatorCollationBase
{
    const ColumnSketchBinary & parent;
    const Collator * collator;

    explicit ComparatorCollationBase(const ColumnSketchBinary & parent_, const Collator * collator_)
...
void ColumnSketchBinary::getPermutation(PermutationSortDirection direction, PermutationSortStability stability,
...
void ColumnSketchBinary::updatePermutation(PermutationSortDirection direction, PermutationSortStability stability,
...
void ColumnSketchBinary::getPermutationWithCollation(const Collator & collator, PermutationSortDirection direction, PermutationSortStability stability,
                                               size_t limit, int /*nan_direction_hint*/, Permutation & res) const
{
    if (direction == IColumn::PermutationSortDirection::Ascending && stability == IColumn::PermutationSortStability::Unstable)
...
void ColumnSketchBinary::updatePermutationWithCollation(const Collator & collator, PermutationSortDirection direction, PermutationSortStability stability,
                                                  size_t limit, int /*nan_direction_hint*/, Permutation & res, EqualRanges & equal_ranges) const
{
...
ColumnPtr ColumnSketchBinary::replicate(const Offsets & replicate_offsets) const
{
...
void ColumnSketchBinary::gather(ColumnGathererStream & gatherer)
{
    gatherer.gather(*this);
}

void ColumnSketchBinary::reserve(size_t n)
{
    offsets.reserve(n);
}


void ColumnSketchBinary::getExtremes(Field & min, Field & max) const
{
    min = String();
    max = String();
...
ColumnPtr ColumnSketchBinary::compress() const
{
...
int ColumnSketchBinary::compareAtWithCollation(size_t n, size_t m, const IColumn & rhs_, int, const Collator & collator) const
{
    const ColumnSketchBinary & rhs = assert_cast<const ColumnSketchBinary &>(rhs_);
...
void ColumnSketchBinary::protect()
{
    getChars().protect();
    getOffsets().protect();
}

void ColumnSketchBinary::validate() const
{
