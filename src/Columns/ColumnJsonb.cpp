...
ColumnJsonb::ColumnJsonb(const ColumnJsonb & src)
    : COWHelper<IColumn, ColumnJsonb>(src),
...
MutableColumnPtr ColumnJsonb::cloneResized(size_t to_size) const
{
...
void ColumnJsonb::updateWeakHash32(WeakHash32 & hash) const
{
    auto s = offsets.size();
...
void ColumnJsonb::insertRangeFrom(const IColumn & src, size_t start, size_t length)
{
    if (length == 0)
        return;

    const ColumnJsonb & src_concrete = assert_cast<const ColumnJsonb &>(src);

    if (start + length > src_concrete.offsets.size())
        throw Exception("Parameter out of bound in IColumnJsonb::insertRangeFrom method.",
...
void ColumnJsonb::insertRangeSelective(const IColumn & src, const Selector & selector, size_t selector_start, size_t length)
{
    const ColumnJsonb & src_concrete = static_cast<const ColumnJsonb &>(src);
...
void ColumnJsonb::expand(const IColumn::Filter & mask, bool inverted)
{
...
const char * ColumnJsonb::skipSerializedInArena(const char * pos) const
{
    const size_t string_size = unalignedLoad<size_t>(pos);
    pos += sizeof(string_size);
    return pos + string_size;
...
ColumnPtr ColumnJsonb::indexImpl(const PaddedPODArray<Type> & indexes, size_t limit) const
{
    if (limit == 0)
        return ColumnJsonb::create();
...
void ColumnJsonb::compareColumn(
    const IColumn & rhs, size_t rhs_row_num,
    PaddedPODArray<UInt64> * row_indexes, PaddedPODArray<Int8> & compare_results,
    int direction, int nan_direction_hint) const
{
    return doCompareColumn<ColumnJsonb>(assert_cast<const ColumnJsonb &>(rhs), rhs_row_num, row_indexes,
                                         compare_results, direction, nan_direction_hint);
}

bool ColumnJsonb::hasEqualValues() const
{
    return hasEqualValuesImpl<ColumnJsonb>();
}

struct ColumnJsonb::ComparatorBase
{
    const ColumnJsonb & parent;

    explicit ComparatorBase(const ColumnJsonb & parent_)
        : parent(parent_)
    {
    }
...
struct ColumnJsonb::ComparatorCollationBase
{
    const ColumnJsonb & parent;
    const Collator * collator;

    explicit ComparatorCollationBase(const ColumnJsonb & parent_, const Collator * collator_)
...
void ColumnJsonb::getPermutation(PermutationSortDirection direction, PermutationSortStability stability,
...
void ColumnJsonb::updatePermutation(PermutationSortDirection direction, PermutationSortStability stability,
...
void ColumnJsonb::getPermutationWithCollation(const Collator & collator, PermutationSortDirection direction, PermutationSortStability stability,
                                size_t limit, int /*nan_direction_hint*/, Permutation & res) const
{
    if (direction == IColumn::PermutationSortDirection::Ascending && stability == IColumn::PermutationSortStability::Unstable)
...
void ColumnJsonb::updatePermutationWithCollation(const Collator & collator, PermutationSortDirection direction, PermutationSortStability stability,
                                size_t limit, int /*nan_direction_hint*/, Permutation & res, EqualRanges & equal_ranges) const
{
...
void ColumnJsonb::gather(ColumnGathererStream & gatherer)
{
...
ColumnPtr ColumnJsonb::compress() const
{
...
int ColumnJsonb::compareAtWithCollation(size_t n, size_t m, const IColumn & rhs_, int, const Collator & collator) const
{
...
void ColumnJsonb::validate() const
{
    if (!offsets.empty() && offsets.back() != chars.size())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "ColumnJsonb validation failed: size mismatch (internal logical error) {} != {}", offsets.back(), chars.size());
}
