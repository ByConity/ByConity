...
\
    ...
    M(UInt64, distributed_push_down_limit, 0, "If 1, LIMIT will be applied on each shard separately. Usually you don't need to use it, since this will be done automatically if it is possible, i.e. for simple query SELECT FROM LIMIT.", 0) \
...
\
    ...
    M(DedupKeyMode, dedup_key_mode, DedupKeyMode::REPLACE, "Handle different deduplication modes, current valid values: REPLACE, THROW. THROW mode can only be used in non-staging area scenarios", 0) \
    \
    /** Settings for Map */ \
