...
bool callOnBasicType(TypeIndex number, F && f)
{
    ...
    if constexpr (_datetime)
    {
        switch (number)
        {
            ...
            case TypeIndex::Time:         return f(TypePair<T, Decimal64>());
...
inline bool callOnBasicTypes(TypeIndex type_num1, TypeIndex type_num2, F && f)
{
    ...
    if constexpr (_datetime)
    {
        switch (type_num1)
        {
            ...
            case TypeIndex::Time: return callOnBasicType<Decimal64, _int, _float, _decimal, _datetime>(type_num2, std::forward<F>(f));
