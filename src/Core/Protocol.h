...
#define CASE_RETURN(x) case x: return #x;
    /**
     * It may be a bad idea to use enum in network packet transmission.
     * Cast enum to/from int is unsafy under some circumstances.
     * So I prefer UInt64 instead.
     */

    namespace HaServer
    {
        /// Begin from 1000, avoid misusing
        constexpr UInt64 Hello       = 1000;
        constexpr UInt64 Pong        = 1001;
        constexpr UInt64 Data        = 1002;
        constexpr UInt64 Exception   = 1003;
        constexpr UInt64 EndOfStream = 1004;
        constexpr UInt64 OK          = 1005;

        constexpr UInt64 Fetch       = 1010;
        constexpr UInt64 Put         = 1011;
        constexpr UInt64 LogEntry    = 1012;
        constexpr UInt64 LSN         = 1013;
        ...
        inline const char * toString(UInt64 packet)
        {
            switch (packet)
            {
                ...
                CASE_RETURN(Pong)
                CASE_RETURN(Data)
                CASE_RETURN(Exception)
                CASE_RETURN(EndOfStream)
                CASE_RETURN(OK)
                CASE_RETURN(Fetch)
                CASE_RETURN(Put)
                CASE_RETURN(LogEntry)
                CASE_RETURN(LSN)
    ...
    namespace HaClient
    {
        /// Begin from 2000, avoid misusing
        constexpr UInt64 Hello       = 2000;
        constexpr UInt64 Ping        = 2001;
        constexpr UInt64 Data        = 2002;
        constexpr UInt64 Exception   = 2003;
        constexpr UInt64 OK          = 2004;

        // for HaMergeTree
        constexpr UInt64 Fetch       = 2010;
        constexpr UInt64 Put         = 2011;
        constexpr UInt64 LogEntry    = 2012;
        constexpr UInt64 AllocLSN    = 2013;
        constexpr UInt64 CommitLSN   = 2014;
        constexpr UInt64 GetDelay    = 2015;
        ...
        // for mutation
        constexpr UInt64 GetMutationStatus = 2030;

        // for HaUniqueMergeTree
        constexpr UInt64 FetchManifestLogs      = 2100;
        constexpr UInt64 GetManifestStatus      = 2101;
        constexpr UInt64 GetManifestSnapshot    = 2102;
        ...
        inline const char * toString(UInt64 packet)
        {
            switch (packet)
            {
                ...
                CASE_RETURN(Put)
                CASE_RETURN(LogEntry)
                CASE_RETURN(AllocLSN)
                CASE_RETURN(CommitLSN)
                CASE_RETURN(GetDelay)
