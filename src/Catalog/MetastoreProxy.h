...
#define MERGEMUTATE_THREAD_START_TIME "MTST_"
    ...
    public:
        ...
        MetastoreProxy(CatalogConfig & config)
        {
            ...
            {
                throw Exception(
                    "Catalog must be correctly configured. Only support foundationdb and bytekv now.", ErrorCodes::METASTORE_EXCEPTION);
            }
        }
     ...
     */
        ...
        static std::string allDbPrefix(const std::string & name_space) { return escapeString(name_space) + '_' + DB_STORE_PREFIX; }
        ...
        static std::string
        deleteBitmapKey(const std::string & name_space, const std::string & uuid, const Protos::DataModelDeleteBitmap & bitmap)
        {
            std::stringstream ss;
            ss << deleteBitmapPrefix(name_space, uuid) << bitmap.partition_id() << "_" << bitmap.part_min_block() << "_"
               << bitmap.part_max_block() << "_" << bitmap.reserved() << "_" << bitmap.type() << "_" << bitmap.txn_id();
            return ss.str();
        }

        static std::string tableMetaPrefix(const std::string & name_space) { return escapeString(name_space) + '_' + TABLE_STORE_PREFIX; }

        static std::string udfMetaPrefix(const std::string & name_space) { return escapeString(name_space) + '_' + UDF_STORE_PREFIX; }

        static std::string dbTrashPrefix(const std::string & name_space) { return escapeString(name_space) + '_' + DATABASE_TRASH_PREFIX; }
        ...
        static std::string
        tableTrashKey(const std::string & name_space, const std::string & db, const std::string & table, const UInt64 & ts)
        {
        ...
        static std::string
        maskingPolicyTableMappingKey(const std::string & name_space, const std::string & masking_policy_name, const std::string & uuid)
        {
        ...
        static std::string
        kafkaOffsetsKey(const std::string & name_space, const std::string & group_name, const std::string & topic, const int partition_num)
        {
            return escapeString(name_space) + "_" + KAFKA_OFFSETS_PREFIX + escapeString(group_name) + "_" + escapeString(topic) + "_"
        ...
        static std::string transactionKey(UInt64 txn) { return TRANSACTION_STORE_PREFIX + toString(txn); }
        ...
        updateTransactionRecord(const String & name_space, const UInt64 & txn_id, const String & txn_data_old, const String & txn_data_new);
        ...
        bool updateTransactionRecordWithOffsets(
            const String & name_space,
            const UInt64 & txn_id,
            const String & txn_data_old,
        ...
        std::pair<bool, String>
        updateTransactionRecordWithRequests(SinglePutRequest & txn_request, BatchCommitRequest & requests, BatchCommitResponse & response);
        ...
        getTrashTableID(const String & name_space, const String & database, const String & table, const UInt64 & ts);
        ...
        getDictionariesInDB(const String & name_space, const String & database);
        IMetaStore::IteratorPtr getAllDictionaryMeta(const String & name_space);
        std::vector<std::shared_ptr<DB::Protos::DataModelDictionary>>
        getDictionariesFromTrash(const String & name_space, const String & database);
        ...
        IMetaStore::IteratorPtr getPartsInRange(
            const String & name_space,
            const String & table_uuid,
        ...
        getMetaInRange(const String & prefix, const String & range_start, const String & range_end, bool include_start, bool include_end);
        ...
        void setCnchLogMetadataInBatch(
