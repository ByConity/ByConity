...
{
    extern const Event IOUringAsynchronousReadWaitMicroseconds;
    extern const Event IOUringSynchronousReadWaitMicroseconds;
}
...
std::future<IAsynchronousReader::Result> AsynchronousReadBufferFromFileDescriptor::asyncReadInto(char * data, size_t size, Priority priority)
{
    ...
    {
        .descriptor = std::make_shared<IAsynchronousReader::LocalFileDescriptor>(fd),
        .offset = file_offset_of_buffer_end,
        .size = size,
        .buf = data,
        .priority = priority,
        .ignore = bytes_to_ignore
    };
    bytes_to_ignore = 0;
...
bool AsynchronousReadBufferFromFileDescriptor::nextImpl()
{
    ...
    if (prefetch_future.valid())
    {
        ...
        ProfileEventTimeIncrement<Microseconds> watch(ProfileEvents::IOUringAsynchronousReadWaitMicroseconds);
    ...
    else
    {
        ...
        // when construct this class with an existing_memory, memory.data() indeed can be nullptr
        if (unlikely(!memory.data()))
            throw Exception(ErrorCodes::LOGICAL_ERROR, 
                "AsynchronousReadBufferFromFileDescriptor::nextImpl found memory.data() is null");

        ProfileEventTimeIncrement<Microseconds> watch(ProfileEvents::IOUringSynchronousReadWaitMicroseconds);
        result = asyncReadInto(memory.data(), memory.size(), Priority{}).get();
    }
