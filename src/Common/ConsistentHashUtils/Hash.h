...
constexpr unsigned long long fio_crc64(const unsigned char *buffer, unsigned long length)
{
...
inline unsigned long long crc64(const std::string & db, const std::string & table)
{
    return fio_crc64(reinterpret_cast<const unsigned char*>(db.c_str()), db.length())
           ^ (fio_crc64(reinterpret_cast<const unsigned char*>(table.c_str()), table.length()) << 1);
}

/// xxhash3 implementation, used by rockdb
inline size_t xxhash3(const unsigned char *data, size_t length)
{
...
inline int32_t chooseNode4DbTable(const std::string & db, const std::string & table, int32_t num_buckets)
{
    return JumpConsistentHash(crc64(db, table), num_buckets);
}

inline int32_t consistentHashForString(const std::string & s, int32_t num_nodes)
{
    return JumpConsistentHash(fio_crc64(reinterpret_cast<const unsigned char*>(s.c_str()), s.length()), num_nodes);
}
