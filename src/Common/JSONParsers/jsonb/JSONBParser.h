...
#include <cmath>
#include <limits>
#include <Common/JSONParsers/jsonb/JSONBError.h>
...
class JsonbParserT {
 public:
  ...
  explicit JsonbParserT(OS_TYPE& os)
      : writer_(os), stream_pos_(0), err_(JsonbErrType::E_NONE) {}

  // parse a UTF-8 JSON string
  bool parse(const std::string& str, hDictInsert handler = nullptr) {
    return parse(str.c_str(), static_cast<unsigned int>(str.size()), handler);
  }

  // parse a UTF-8 JSON c-style string (NULL terminated)
  bool parse(const char* c_str, hDictInsert handler = nullptr) {
    return parse(c_str, static_cast<unsigned int>(strlen(c_str)), handler);
  }
 ...
 private:
  ...
  JsonbErrType handleParseValueFailure(bool parse_res, std::istream& in) {
    if (parse_res) {
      trim(in);
      if (!in.good()) {
  ...
  JsonbErrType handleParseFailure(std::istream& in) {
    ...
    switch (in.peek()) {
      case 'n':
        skipChar(in);
        error = handleParseValueFailure(parseNull(in), in);
        break;
      case 't':
        skipChar(in);
        error = handleParseValueFailure(parseTrue(in), in);
        break;
      case 'f':
        skipChar(in);
        error = handleParseValueFailure(parseFalse(in), in);
        break;
      case '"':
        skipChar(in);
        error = handleParseValueFailure(parseString(in), in);
        break;
      default:
        if (parseNumber(in)) {
          trim(in);
  ...
  bool parseKVPair(std::istream& in, hDictInsert handler) {
    return static_cast<bool>(parseKey(in, handler) && parseValue(in, handler));
  }
  ...
  bool parseKey(std::istream& in, hDictInsert handler) {
    ...
    while (in.good() && in.peek() != '"' &&
           key_len < JsonbKeyValue::sMaxKeyLen) {
      char ch = nextChar(in);
      if(ch == '\\'){
  ...
  */
  ...
  bool parseHex(std::istream& in) {
    ...
    int size = 0;
    if (num_digits <= 2) {
      size = writer_.writeInt8(static_cast<int8_t>(val));
    } else if (num_digits <= 4) {
      size = writer_.writeInt16(static_cast<int16_t>(val));
    } else if (num_digits <= 8) {
      size = writer_.writeInt32(static_cast<int32_t>(val));
  ...
  bool parseDecimal(std::istream& in, bool neg = false) {
    ...
    *pbuf = 0; // set null-terminator
    int64_t val = strtol(num_buf_, nullptr, 10);
    if (errno == ERANGE) {
      err_ = JsonbErrType::E_DECIMAL_OVERFLOW;
      return false;
    }
  ...
  bool internConvertBufferToDouble() {
    double val = strtod(num_buf_, nullptr);
...
using JsonbParser = JsonbParserT<JsonbOutStream>;
