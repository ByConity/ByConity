...
#include <Common/JSONParsers/jsonb/JSONBStream.h>
...
public:
    ...
    uint32_t writeKey(const char* key, uint8_t len, hDictInsert handler = nullptr) {
        if (len && !stack_.empty() && verifyKeyState()) {
            ...
            if (key_id < 0) {
                ...
                os_->put(0);
                os_->write(reinterpret_cast<char*>(&idx), sizeof(JsonbKeyValue::keyid_type));
    ...
    uint32_t writeValue(const JsonbValue* value) {
        if ((first_ && stack_.empty()) || (!stack_.empty() && verifyValueState())) {
            ...
            os_->write(reinterpret_cast<const char*>(value), value->numPackedBytes());
            kvState_ = WS_Value;
            return value->size();
        }
        return 0;
    ...
    uint32_t writeKey(JsonbKeyValue::keyid_type idx) {
        if (!stack_.empty() && verifyKeyState()) {
            os_->put(0);
            os_->write(reinterpret_cast<const char*>(&idx), sizeof(JsonbKeyValue::keyid_type));
            kvState_ = WS_Key;
    ...
    uint32_t writeNull() {
        if ((first_ && stack_.empty()) || (!stack_.empty() && verifyValueState())) {
            if (!writeFirstHeader()) return 0;
            os_->put(static_cast<JsonbTypeUnder>(JsonbType::T_Null));
            kvState_ = WS_Value;
    ...
    uint32_t writeBool(bool b) {
        if ((first_ && stack_.empty()) || (!stack_.empty() && verifyValueState())) {
            if (!writeFirstHeader()) return 0;
            if (b) {
                os_->put(static_cast<JsonbTypeUnder>(JsonbType::T_True));
            } else {
                os_->put(static_cast<JsonbTypeUnder>(JsonbType::T_False));
            }
    ...
    uint32_t writeInt(int64_t val) {
        if (val >= std::numeric_limits<int8_t>::min() &&
            ...
            return writeInt8(static_cast<int8_t>(val));
        } else if (val >= std::numeric_limits<int16_t>::min() &&
                   val <= std::numeric_limits<int16_t>::max()) {
            return writeInt16(static_cast<int16_t>(val));
    ...
    uint32_t writeInt8(int8_t v) {
        if ((first_ && stack_.empty()) || (!stack_.empty() && verifyValueState())) {
            if (!writeFirstHeader()) return 0;
            os_->put(static_cast<JsonbTypeUnder>(JsonbType::T_Int8));
            os_->put(v);
            kvState_ = WS_Value;
    ...
    uint32_t writeInt16(int16_t v) {
        if ((first_ && stack_.empty()) || (!stack_.empty() && verifyValueState())) {
            if (!writeFirstHeader()) return 0;
            os_->put(static_cast<JsonbTypeUnder>(JsonbType::T_Int16));
            os_->write(reinterpret_cast<const char*>(&v), sizeof(int16_t));
            kvState_ = WS_Value;
    ...
    uint32_t writeInt32(int32_t v) {
        if ((first_ && stack_.empty()) || (!stack_.empty() && verifyValueState())) {
            if (!writeFirstHeader()) return 0;
            os_->put(static_cast<JsonbTypeUnder>(JsonbType::T_Int32));
            os_->write(reinterpret_cast<const char*>(&v), sizeof(int32_t));
            kvState_ = WS_Value;
    ...
    uint32_t writeInt64(int64_t v) {
        if ((first_ && stack_.empty()) || (!stack_.empty() && verifyValueState())) {
            if (!writeFirstHeader()) return 0;
            os_->put(static_cast<JsonbTypeUnder>(JsonbType::T_Int64));
            os_->write(reinterpret_cast<const char*>(&v), sizeof(int64_t));
            kvState_ = WS_Value;
    ...
    uint32_t writeInt128(int128_t v) {
        if ((first_ && stack_.empty()) || (!stack_.empty() && verifyValueState())) {
            if (!writeFirstHeader()) return 0;
            os_->put(static_cast<JsonbTypeUnder>(JsonbType::T_Int128));
            os_->write(reinterpret_cast<const char*>(&v), sizeof(int128_t));
            kvState_ = WS_Value;
    ...
    uint32_t writeDouble(double v) {
        if ((first_ && stack_.empty()) || (!stack_.empty() && verifyValueState())) {
            if (!writeFirstHeader()) return 0;
            os_->put(static_cast<JsonbTypeUnder>(JsonbType::T_Double));
            os_->write(reinterpret_cast<const char*>(&v), sizeof(double));
            kvState_ = WS_Value;
    ...
    uint32_t writeFloat(float v) {
        if ((first_ && stack_.empty()) || (!stack_.empty() && verifyValueState())) {
            if (!writeFirstHeader()) return 0;
            os_->put(static_cast<JsonbTypeUnder>(JsonbType::T_Float));
            os_->write(reinterpret_cast<const char*>(&v), sizeof(float));
            kvState_ = WS_Value;
    ...
    bool writeStartString() {
        if ((first_ && stack_.empty()) || (!stack_.empty() && verifyValueState())) {
            if (!writeFirstHeader()) return false;
            os_->put(static_cast<JsonbTypeUnder>(JsonbType::T_String));
            str_pos_ = os_->tellp();

            // fill the size bytes with 0 for now
            uint32_t size = 0;
            os_->write(reinterpret_cast<const char*>(&size), sizeof(uint32_t));
    ...
    bool writeEndString() {
        if (kvState_ == WS_String) {
            ...
            int32_t size = static_cast<int32_t>(cur_pos - str_pos_ - sizeof(uint32_t));
            assert(size >= 0);
    ...
    uint32_t writeString(const std::string& str) {
        return writeString(str.c_str(), static_cast<uint32_t>(str.size()));
    }
    ...
    bool writeStartBinary() {
        if ((first_ && stack_.empty()) || (!stack_.empty() && verifyValueState())) {
            if (!writeFirstHeader()) return false;
            os_->put(static_cast<JsonbTypeUnder>(JsonbType::T_Binary));
            str_pos_ = os_->tellp();
    ...
    bool writeStartObject() {
        if (stack_.empty() || verifyValueState()) {
            ...
            os_->put(static_cast<JsonbTypeUnder>(JsonbType::T_Object));
    ...
    bool writeEndObject() {
        if (!stack_.empty() && stack_.top().state == WS_Object && kvState_ == WS_Value) {
            ...
            std::streampos cur_pos = os_->tellp();
            int32_t size = static_cast<int32_t>(cur_pos - ci.sz_pos - sizeof(uint32_t));
            assert(size >= 0);
    ...
    bool writeStartArray() {
        if (stack_.empty() || verifyValueState()) {
            ...
            os_->put(static_cast<JsonbTypeUnder>(JsonbType::T_Array));
            // save the size position
    ...
    JsonbValue* getValue() {
        return JsonbDocument::createValue(getOutput()->getBuffer(),
                                          static_cast<uint32_t>(getOutput()->getSize()));
    }
