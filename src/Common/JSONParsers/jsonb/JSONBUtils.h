...
public:
    ...
    const std::string to_json_string(const char* data, size_t size) {
        JsonbDocument* pdoc = JsonbDocument::createDocument(data, size);
        if (!pdoc) {
...
private:
    ...
    void intern_json(const JsonbValue* val) {
        ...
        case JsonbType::T_Int8: {
            os_.write((reinterpret_cast<const JsonbInt8Val*>(val))->val());
            break;
        }
        case JsonbType::T_Int16: {
            os_.write((reinterpret_cast<const JsonbInt16Val*>(val))->val());
            break;
        }
        case JsonbType::T_Int32: {
            os_.write((reinterpret_cast<const JsonbInt32Val*>(val))->val());
            break;
        }
        case JsonbType::T_Int64: {
            os_.write((reinterpret_cast<const JsonbInt64Val*>(val))->val());
            break;
        }
        case JsonbType::T_Double: {
            os_.write((reinterpret_cast<const JsonbDoubleVal*>(val))->val());
            break;
        }
        case JsonbType::T_Float: {
            os_.write((reinterpret_cast<const JsonbFloatVal*>(val))->val());
            break;
        }
        case JsonbType::T_Int128: {
            os_.write((reinterpret_cast<const JsonbInt128Val*>(val))->val());
            break;
        }
        case JsonbType::T_String: {
            string_to_json((reinterpret_cast<const JsonbStringVal*>(val))->getBlob(), (reinterpret_cast<const JsonbStringVal*>(val))->length());
            break;
        }
        case JsonbType::T_Binary: {
            os_.write("\"<BINARY>", 9);
            os_.write((reinterpret_cast<const JsonbBinaryVal*>(val))->getBlob(), (reinterpret_cast<const JsonbBinaryVal*>(val))->getBlobLen());
            os_.write("<BINARY>\"", 9);
            break;
        }
        case JsonbType::T_Object: {
            object_to_json(reinterpret_cast<const ObjectVal*>(val));
            break;
        }
        case JsonbType::T_Array: {
            array_to_json(reinterpret_cast<const ArrayVal*>(val));
            break;
        }
        default:
            break;
    ...
    void string_to_json(const char* str, size_t len) {
        ...
        for (const char* ptr = str; ptr != str + len && *ptr; ++ptr) {
            if (static_cast<unsigned char>(*ptr) > 31 && *ptr != '\"' && *ptr != '\\')
                os_.put(*ptr);
            else {
                os_.put('\\');
                unsigned char token;
    ...
    void array_to_json(const ArrayVal* val) {
        ...
        while (iter != iter_fence) {
            // convert value
            intern_json(static_cast<const JsonbValue*>(iter));
            ++iter;
            if (iter != iter_fence) {
                os_.put(',');
            }
...
public:
    JsonbValue* operator()(int32_t val) { return operator()(static_cast<int64_t>(val)); }
    ...
    JsonbValue* operator()(const char* str) { return operator()(str, static_cast<unsigned int>(strlen(str))); }
    ...
    JsonbValue* extractValue() {
        return static_cast<ArrayVal*>(
                       JsonbDocument::createValue(writer_.getOutput()->getBuffer(),
                                                  static_cast<int>(writer_.getOutput()->getSize())))
                ->get(0);
    }
