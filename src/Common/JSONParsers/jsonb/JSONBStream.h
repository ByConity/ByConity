...
public:
    explicit JsonbOutStream(uint32_t capacity = 1024)
        ...
        head_ = static_cast<char*>(malloc(capacity_));
    }
    ...
    void write(const char* c_str) { write(c_str, static_cast<uint32_t>(strlen(c_str))); }
    ...
    void seekp(pos_type pos) { size_ = static_cast<uint32_t>(pos); }
...
private:
    void realloc(uint32_t len) {
        ...
        if (alloc_) {
            char* new_buf = static_cast<char*>(::realloc(head_, capacity_));
            assert(new_buf);
            head_ = new_buf;
        } else {
            char* new_buf = static_cast<char*>(::malloc(capacity_));
            assert(new_buf);
