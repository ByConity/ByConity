...
public:
    ...
    JsonbValue* getValue() { return (reinterpret_cast<JsonbValue*>(payload_)); }

    void setValue(const JsonbValue* value);

    unsigned int numPackedBytes() const;

    ObjectVal* operator->() { return (reinterpret_cast<ObjectVal*>(payload_)); }

    const ObjectVal* operator->() const { return (reinterpret_cast<const ObjectVal*>(payload_)); }
...
public:
    ...
    void clearLegPtr() { leg_ptr = nullptr; }

    void setLegPtr(char* ptr) {
        clearLegPtr();
        leg_ptr = ptr;
    }

    char* getLegPtr() { return leg_ptr; }

    void clearLegLen() { leg_len = 0; }

    void addLegLen() { leg_len++; }

    unsigned int getLegLen() const { return leg_len; }
    ...
    void setHasEscapes(bool has) { has_escapes = has; }

    bool getHasEscapes() const { return has_escapes; }
...
struct LegInfo
{
    ...
    LegInfo(char * ptr_, unsigned int len_, int index_, unsigned int type_) : leg_ptr(ptr_), leg_len(len_), array_index(index_), type(type_) { }
};
...
public:
    ...
    static bool parseArray(Stream* stream, JsonbPath* path);
    static bool parseMember(Stream* stream, JsonbPath* path);

    //return true if json path valid else return false
    bool seek(const char* string, size_t length);

    void addLegToLegVector(std::unique_ptr<LegInfo> leg) {
        leg_vector.emplace_back(leg.release());
    }

    size_t getLegVectorSize() { return leg_vector.size(); }

    LegInfo* getLegFromLegVector(size_t i) { return leg_vector[i].get(); }

    void clean() { leg_vector.clear(); }

private:
    std::vector<std::unique_ptr<LegInfo>> leg_vector;
};
 ...
 */
template <class IterType, class ContType>
...
public:
    ...
    JsonbFwdIteratorT(const JsonbFwdIteratorT<IterTy, ContType>& rhs) : current_(rhs.base()) {}
    ...
    JsonbFwdIteratorT& operator++() {
        current_ = reinterpret_cast<iterator>(reinterpret_cast<const char*>(current_) + current_->numPackedBytes());
...
using JsonbTypeUnder = std::underlying_type<JsonbType>::type;
...
public:
    ...
    JsonbValue* value() const { return const_cast<JsonbValue*>(reinterpret_cast<const JsonbValue*>((reinterpret_cast<const char*>(this)) + keyPackedBytes())); }
...
public:
    int128_t val() const {
        switch (type_) {
        case JsonbType::T_Int8:
            return (reinterpret_cast<const JsonbInt8Val*>(this))->val();
        case JsonbType::T_Int16:
            return (reinterpret_cast<const JsonbInt16Val*>(this))->val();
        case JsonbType::T_Int32:
            return (reinterpret_cast<const JsonbInt32Val*>(this))->val();
        case JsonbType::T_Int64:
            return (reinterpret_cast<const JsonbInt64Val*>(this))->val();
        case JsonbType::T_Int128:
            return (reinterpret_cast<const JsonbInt128Val*>(this))->val();
        default:
            return 0;
        }
    }
    bool setVal(int128_t val) {
        ...
        case JsonbType::T_Int8:
            if (val < std::numeric_limits<int8_t>::min() ||
                ...
                return false;
            return (reinterpret_cast<JsonbInt8Val*>(this))->setVal(static_cast<int8_t>(val));
        case JsonbType::T_Int16:
            if (val < std::numeric_limits<int16_t>::min() ||
                val > std::numeric_limits<int16_t>::max())
                return false;
            return (reinterpret_cast<JsonbInt16Val*>(this))->setVal(static_cast<int16_t>(val));
        case JsonbType::T_Int32:
            if (val < std::numeric_limits<int32_t>::min() ||
                val > std::numeric_limits<int32_t>::max())
                return false;
            return (reinterpret_cast<JsonbInt32Val*>(this))->setVal(static_cast<int32_t>(val));
        case JsonbType::T_Int64:
            return (reinterpret_cast<JsonbInt64Val*>(this))->setVal(static_cast<int64_t>(val));
        case JsonbType::T_Int128:
            return (reinterpret_cast<JsonbInt128Val*>(this))->setVal(val);
        default:
            return false;
        }
    }
...
public:
    ...
    iterator search(const char* key, hDictFind handler = nullptr) {
        ...
        return search(key, static_cast<unsigned int>(strlen(key)), handler);
    }
    ...
    iterator search(int key_id) {
        ...
        while (pch < fence) {
            JsonbKeyValue* pkey = const_cast<JsonbKeyValue*>(reinterpret_cast<const JsonbKeyValue*>(pch));
    ...
    JsonbValue* find(const char* key, hDictFind handler = nullptr) {
        ...
        return find(key, static_cast<unsigned int>(strlen(key)), handler);
    }
    ...
    iterator begin() { return iterator(reinterpret_cast<pointer>(payload_)); }

    const_iterator begin() const { return const_iterator(reinterpret_cast<const_pointer>(payload_)); }

    iterator end() { return iterator(reinterpret_cast<pointer>(payload_ + size_)); }

    const_iterator end() const { return const_iterator(reinterpret_cast<const_pointer>(payload_ + size_)); }
...
public:
    ...
    JsonbValue* get(int idx) const {
        ...
        while (pch < fence && idx-- > 0) pch += (reinterpret_cast<const JsonbValue*>(pch)->numPackedBytes());
        if (idx > 0 || pch == fence) return nullptr;

        return const_cast<JsonbValue*>(reinterpret_cast<const JsonbValue*>(pch));
    }
    ...
    int numElem() const {
        ...
        while (pch < fence) {
            ++num;
            pch += (reinterpret_cast<const JsonbValue*>(pch))->numPackedBytes();
        }
...
inline JsonbDocument* JsonbDocument::makeDocument(char* pb, uint32_t size, JsonbType type) {
    ...
    JsonbDocument* doc = reinterpret_cast<JsonbDocument*>(pb);
    // Write header
    ...
    if (type == JsonbType::T_Object || type == JsonbType::T_Array)
        (reinterpret_cast<ContainerVal*>(value))->size_ = 0;
    if (type == JsonbType::T_String || type == JsonbType::T_Binary)
        (reinterpret_cast<JsonbBlobVal*>(value))->size_ = 0;
    return doc;
}
...
inline JsonbDocument* JsonbDocument::createDocument(const char* pb, uint32_t size) {
    ...
    const JsonbDocument* doc = reinterpret_cast<const JsonbDocument*>(pb);
    if (doc->header_.ver_ != JSONB_VER) {
        return nullptr;
    }

    const JsonbValue* val = reinterpret_cast<const JsonbValue*>(doc->payload_);
    if (val->type() < JsonbType::T_Null || val->type() >= JsonbType::NUM_TYPES ||
        size != sizeof(JsonbHeader) + val->numPackedBytes()) {
        return nullptr;
    }

    return const_cast<JsonbDocument*>(doc);
}
...
inline JsonbValue* JsonbDocument::createValue(const char* pb, uint32_t size) {
    ...
    return const_cast<JsonbValue*>(val);
}

inline unsigned int JsonbDocument::numPackedBytes() const {
    return (reinterpret_cast<const JsonbValue*>(payload_))->numPackedBytes() + sizeof(header_);
}

inline unsigned int JsonbKeyValue::numPackedBytes() const {
    unsigned int ks = keyPackedBytes();
    const JsonbValue* val = reinterpret_cast<const JsonbValue*>((reinterpret_cast<const char*>(this)) + ks);
...
inline unsigned int JsonbValue::numPackedBytes() const {
    ...
    case JsonbType::T_String:
    case JsonbType::T_Binary: {
        return (reinterpret_cast<const JsonbBlobVal*>(this))->numPackedBytes();
    }

    case JsonbType::T_Object:
    case JsonbType::T_Array: {
        return (reinterpret_cast<const ContainerVal*>(this))->numPackedBytes();
    }
    default:
        return 0;
    }
...
inline unsigned int JsonbValue::size() const {
    ...
    case JsonbType::T_String:
    case JsonbType::T_Binary: {
        return (reinterpret_cast<const JsonbBlobVal*>(this))->getBlobLen();
    }

    case JsonbType::T_Object:
    case JsonbType::T_Array: {
        return (reinterpret_cast<const ContainerVal*>(this))->getContainerSize();
    }
    case JsonbType::T_Null:
    case JsonbType::T_True:
    case JsonbType::T_False:
...
inline int JsonbValue::length() const {
    ...
    case JsonbType::T_Object: {
        return (reinterpret_cast<const ObjectVal*>(this))->numElem();
    }
    case JsonbType::T_Array: {
        return (reinterpret_cast<const ArrayVal*>(this))->numElem();
    }
    default:
        return 0;
    }
...
inline bool JsonbValue::contains(JsonbValue* rhs) const {
    ...
    case JsonbType::T_Int64:
    case JsonbType::T_Int128: {
        return (reinterpret_cast<const JsonbIntVal*>(this))->val() == (reinterpret_cast<const JsonbIntVal*>(rhs))->val();
    }
    case JsonbType::T_Double: {
        if (rhs->isDouble()) {
            return (reinterpret_cast<const JsonbDoubleVal*>(this))->val() == (reinterpret_cast<const JsonbDoubleVal*>(rhs))->val();
        }
        return false;
    }
    case JsonbType::T_Float: {
        if (rhs->isDouble()) {
            return (reinterpret_cast<const JsonbFloatVal*>(this))->val() == (reinterpret_cast<const JsonbFloatVal*>(rhs))->val();
        }
        return false;
    }
    case JsonbType::T_String:
    case JsonbType::T_Binary: {
        if (rhs->isString()) {
            const auto *str_value1 = reinterpret_cast<const JsonbStringVal*>(this);
            const auto *str_value2 = reinterpret_cast<const JsonbStringVal*>(rhs);
    ...
    case JsonbType::T_Array: {
        int lhs_num = (reinterpret_cast<const ArrayVal*>(this))->numElem();
        if (rhs->isArray()) {
            int rhs_num = (reinterpret_cast<const ArrayVal*>(rhs))->numElem();
            if (rhs_num > lhs_num) return false;
            int contains_num = 0;
            for (int i = 0; i < lhs_num; ++i) {
                for (int j = 0; j < rhs_num; ++j) {
                    if ((reinterpret_cast<const ArrayVal*>(this))->get(i)->contains((reinterpret_cast<const ArrayVal*>(rhs))->get(j))) {
                        contains_num++;
                        break;
                    }
                }
        ...
        for (int i = 0; i < lhs_num; ++i) {
            if ((reinterpret_cast<const ArrayVal*>(this))->get(i)->contains(rhs)) {
                return true;
            }
        }
        return false;
    ...
    case JsonbType::T_Object: {
        if (rhs->isObject()) {
            const auto *str_value1 = reinterpret_cast<const ObjectVal*>(this);
            const auto *str_value2 = reinterpret_cast<const ObjectVal*>(rhs);
...
inline const char* JsonbValue::getValuePtr() const {
    ...
    case JsonbType::T_Float:
    case JsonbType::T_Int128:
        return (reinterpret_cast<const char*>(this)) + sizeof(JsonbType);

    case JsonbType::T_String:
    case JsonbType::T_Binary:
        return (reinterpret_cast<const JsonbBlobVal*>(this))->getBlob();

    case JsonbType::T_Object:
    case JsonbType::T_Array:
        return (reinterpret_cast<const ContainerVal*>(this))->getPayload();
...
inline JsonbValue* JsonbValue::findValue(JsonbPath& path, hDictFind handler) {
    ...
    for (size_t i = 0; i < path.getLegVectorSize(); ++i) {
        switch (path.getLegFromLegVector(i)->type) {
        case MEMBER_CODE: {
            if (likely(pval->type_ == JsonbType::T_Object)) {
                if (path.getLegFromLegVector(i)->leg_len == 1 &&
                    *path.getLegFromLegVector(i)->leg_ptr == WILDCARD) {
                    continue;
                }

                pval = (reinterpret_cast<ObjectVal*>(pval))
                               ->find(path.getLegFromLegVector(i)->leg_ptr,
                                      path.getLegFromLegVector(i)->leg_len, handler);
        ...
        case ARRAY_CODE: {
            ...
            if (pval->type_ == JsonbType::T_Object &&
                path.getLegFromLegVector(i)->array_index == 0) {
                continue;
            }

            if (pval->type_ != JsonbType::T_Array ||
                path.getLegFromLegVector(i)->leg_ptr != nullptr ||
                path.getLegFromLegVector(i)->leg_len != 0)
                return nullptr;

            if (path.getLegFromLegVector(i)->array_index >= 0) {
                pval = (reinterpret_cast<ArrayVal*>(pval))->get(path.getLegFromLegVector(i)->array_index);
            } else {
                pval = (reinterpret_cast<ArrayVal*>(pval))
                               ->get((reinterpret_cast<ArrayVal*>(pval))->numElem() +
                                     path.getLegFromLegVector(i)->array_index);
            }
...
inline bool JsonbPath::parsePath(Stream* stream, JsonbPath* path) {
    if (stream->peek() == BEGIN_ARRAY) {
        return parseArray(stream, path);
    } else if (stream->peek() == BEGIN_MEMBER) {
        return parseMember(stream, path);
    } else {
        return false; //invalid json path
    }
}

inline bool JsonbPath::parseArray(Stream* stream, JsonbPath* path) {
    ...
    if (stream->peek() == WILDCARD) {
        stream->setLegPtr(const_cast<char*>(stream->position()));
        stream->addLegLen();
        stream->skip(1);
        if (stream->peek() == END_ARRAY) {
            std::unique_ptr<LegInfo> leg(
                    new LegInfo(stream->getLegPtr(), stream->getLegLen(), 0, ARRAY_CODE));
            path->addLegToLegVector(std::move(leg));
            stream->skip(1);
            return true;
        } else {
            return false;
    ...
    std::string_view idx_string(stream->getLegPtr(), stream->getLegLen());
    int index = 0;

    if (stream->getLegLen() >= 4 &&
        std::equal(LAST, LAST + 4, stream->getLegPtr(),
        ...
        } else if (stream->getLegLen() > 4) {
            return false;
        }

        std::unique_ptr<LegInfo> leg(new LegInfo(nullptr, 0, -index - 1, ARRAY_CODE));
        path->addLegToLegVector(std::move(leg));
    ...
    std::unique_ptr<LegInfo> leg(new LegInfo(nullptr, 0, index, ARRAY_CODE));
    path->addLegToLegVector(std::move(leg));

    return true;
}

inline bool JsonbPath::parseMember(Stream* stream, JsonbPath* path) {
    // advance past the .
    ...
    if (stream->peek() == WILDCARD) {
        ...
        stream->skip(1);
        std::unique_ptr<LegInfo> leg(
                new LegInfo(stream->getLegPtr(), stream->getLegLen(), 0, MEMBER_CODE));
        path->addLegToLegVector(std::move(leg));
        return true;
    }
    ...
    for (; !stream->exhausted(); stream->skip(1)) {
        if (stream->peek() == ESCAPE) {
            ...
            if (left_quotation_marks == nullptr) {
                ...
                stream->setLegPtr(const_cast<char*>(++left_quotation_marks));
                continue;
            } else {
    ...
    if ((left_quotation_marks != nullptr && right_quotation_marks == nullptr) ||
        stream->getLegPtr() == nullptr || stream->getLegLen() == 0) {
        return false; //invalid json path
    }

    if (stream->getHasEscapes()) {
        stream->removeEscapes();
    }
