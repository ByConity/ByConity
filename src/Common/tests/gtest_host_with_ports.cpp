...
TEST(HostWithPortsUtils, HostWithPortHash)
{
    ...
    HostWithPorts hp2 {"[fdbd:dc03:3:131::166]", rpc_port, tcp_port, http_port, exchange_port, exchange_status_port, ""};
    HostWithPorts hp3 {"fdbd:dc03:3:131::166", rpc_port, tcp_port, http_port, exchange_port, exchange_status_port, ""};
    EXPECT_EQ(hasher(hp2), hasher(hp3));
    EXPECT_EQ(hasher(hp2), hasher(hp2));
    EXPECT_EQ(hasher(hp3), hasher(hp3));

    HostWithPorts hp4 {"10.130.48.166", rpc_port, tcp_port, http_port, exchange_port, exchange_status_port, ""};
