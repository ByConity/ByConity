...
void HiveMetastoreClient::tryCallHiveClient(std::function<void(ThriftHiveMetastoreClientPool::Entry &)> func)
{
    ...
    if (i >= max_hive_metastore_client_retry)
        throw Exception(ErrorCodes::NETWORK_ERROR, "Hive Metastore expired because {}", err_msg);
}
...
{
    Strings tables;
    tryCallHiveClient([&](auto & client) { client->get_all_tables(tables, db_name); });
    return tables;
}
...
std::shared_ptr<ApacheHive::Table> HiveMetastoreClient::getTable(const String & db_name, const String & table_name)
{
    ...
    tryCallHiveClient([&](auto & client) { client->get_table(*table, db_name, table_name); });
...
HiveMetastoreClientFactory::createThriftHiveMetastoreClient(const String & name)
{
