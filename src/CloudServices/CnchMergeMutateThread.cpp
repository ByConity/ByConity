...
namespace
{
    constexpr auto DELAY_SCHEDULE_TIME_IN_SECOND = 60;
...
void CnchMergeMutateThread::shutdown()
{
    ...
    for (auto & worker : workers)
    {
        ...
        {
            /// TODO:
            /// worker->getManipulationTasks(storage_id.uuid, {}, false, true);
        }
        catch (...)
        {
...
void CnchMergeMutateThread::runHeartbeatTask()
{
    ...
    for (auto & [worker, request_tasks] : worker_with_tasks)
    {
        ...
        {
            // TODO:
            // auto response_tasks = worker->getManipulationTasks(storage_id.uuid, request_tasks);
...
void CnchMergeMutateThread::submitFutureManipulationTask(FutureManipulationTask & future_task)
{
    ...
    if (type == ManipulationType::Merge)
    {
        ...
        if (transaction->tryLock(partition_lock))
            LOG_DEBUG(log, "Acquired lock in succesful for partition " << partition_id);
        else
        {
...
void CnchMergeMutateThread::finishTask(
    const String & task_id, const MergeTreeDataPartPtr & merged_part, std::function<void()> && commit_parts)
{
    auto local_context = getContext();
