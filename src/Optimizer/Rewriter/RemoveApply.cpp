...
PlanNodePtr CorrelatedScalarSubqueryVisitor::visitApplyNode(ApplyNode & node, Void & v)
{
    ...
    auto group_agg_step = std::make_shared<AggregatingStep>(join_node->getStep()->getOutputStream(), keys, descs_with_mask, true);
...
PlanNodePtr CorrelatedInSubqueryVisitor::visitApplyNode(ApplyNode & node, Void & v)
{
    ...
    auto count_step = std::make_shared<AggregatingStep>(pre_expression_node->getStep()->getOutputStream(), keys, aggregate_descs, true);
...
PlanNodePtr CorrelatedExistsSubqueryVisitor::visitApplyNode(ApplyNode & node, Void & v)
{
    ...
    auto count_non_null_step
        = std::make_shared<AggregatingStep>(remove_null_node->getStep()->getOutputStream(), keys, aggregate_descs, true);
...
PlanNodePtr UnCorrelatedExistsSubqueryVisitor::visitApplyNode(ApplyNode & node, Void & v)
{
    ...
    aggregate_desc.function = agg_fun;
    AggregateDescriptions aggregate_descs{aggregate_desc};
    auto count_subquery_step = std::make_shared<AggregatingStep>(subquery_ptr->getStep()->getOutputStream(), keys, aggregate_descs, true);
