...
PatternPtr CreateTopNFilteringForAggregating::getPattern() const
{
    ...
    return Patterns::topN()
             ->withSingle(Patterns::aggregating()
                 ->withSingle(Patterns::any()->matching([](const PlanNodePtr & node, auto &)
                                                                  { return node->getStep()->getType() != IQueryPlanStep::Type::TopNFiltering;})));
}
...
TransformResult CreateTopNFilteringForAggregating::transformImpl(PlanNodePtr node, const Captures &, RuleContext & context)
{
    ...
    auto topn_filter_step = std::make_shared<TopNFilteringStep>(aggregate_child_node->getStep()->getOutputStream(),
                                                                topn_step.getSortDescription(),
                                                                topn_step.getLimit(),
                                                                TopNModel::DENSE_RANK);
    auto topn_filter_node = PlanNodeBase::createPlanNode(context.context->nextNodeId(),
                                                         topn_filter_step,
                                                         PlanNodes {aggregate_child_node});
...
{
    return Patterns::topN()->withSingle(Patterns::project());
}

TransformResult PushTopNThroughProjection::transformImpl(PlanNodePtr node, const Captures &, RuleContext &)
{
...
{
    return Patterns::topNFiltering()->withSingle(Patterns::project());
}
