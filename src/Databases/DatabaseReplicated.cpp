...
static UUID getTableUUIDIfReplicated(const String & metadata, ContextPtr context)
{
    ...
    ParserCreateQuery parser(context->getSettingsRef().dialect_type);
...
void DatabaseReplicated::recoverLostReplica(const ZooKeeperPtr & current_zookeeper, UInt32 our_log_ptr, UInt32 max_log_ptr)
{
    ...
    for (const auto & name_and_meta : table_name_to_metadata)
    {
        ...
        auto query_ast = parseQueryFromMetadataInZooKeeper(name_and_meta.first, name_and_meta.second, create_query_context->getSettingsRef().dialect_type);
...
ASTPtr DatabaseReplicated::parseQueryFromMetadataInZooKeeper(const String & node_name, const String & query, enum DialectType dt)
{
