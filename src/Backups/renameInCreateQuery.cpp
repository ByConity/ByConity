    ...
    private:
        ...
        static void visitCreateQuery(ASTCreateQuery & create, const Data & data)
        {
            ...
            if (!create.as_table.empty() && !create.as_database.empty())
                std::tie(create.as_database, create.as_table)
                    = data.renaming_config->getNewTableName({create.as_database, create.as_table});
        ...
        static void visitFunction(ASTFunction & function, const Data & data)
        {
            if ((function.name == "merge") || (function.name == "Merge"))
            {
                ...
                (function.name == "remote") || (function.name == "remoteSecure") || (function.name == "cluster")
        ...
        static void visitFunctionMerge(ASTFunction & function, const Data & data)
        {
            ...
            String db_name = evaluateConstantExpressionForDatabaseName(args[db_name_arg_index], data.context)
                                 ->as<ASTLiteral &>()
        ...
        static void visitFunctionRemote(ASTFunction & function, const Data & data)
        {
            ...
            String name
                = evaluateConstantExpressionForDatabaseName(args[db_name_index], data.context)->as<ASTLiteral &>().value.safeGet<String>();
            ...
            if (qualified_name.database.empty())
            {
                ...
                if (args.size() <= table_name_index)
                    return;
                qualified_name.table = evaluateConstantExpressionForDatabaseName(args[table_name_index], data.context)
                                           ->as<ASTLiteral &>()
                                           .value.safeGet<String>();
            }
