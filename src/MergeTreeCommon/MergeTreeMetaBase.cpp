...
/// TODO: Recheck by @yuanquan
void MergeTreeMetaBase::checkAlterInCnchServer(const AlterCommands & commands, ContextPtr local_context) const
{
    ...
    //         columns_alter_forbidden.insert(col);
    //     for (const String & col : metadata_snapshot->getSortingKeyColumns())
    ...
    if (metadata_snapshot->hasUniqueKey())
    {
        ...
        if (!merging_params.version_column.empty())
            columns_alter_forbidden.insert(merging_params.version_column);
    }

    // if (row_policy_ast)
    // {
    //     auto & func = row_policy_ast->as<ASTFunction &>();
    //     for (const auto & ast : func.arguments->children)
    //         columns_alter_forbidden.insert(getIdentifierName(ast));
    // }
    ...
    for (const AlterCommand & command : commands)
    {
        ...
        // if (command.type == AlterCommand::MODIFY_COLUMN && command.isMetadataOnlyConversion(old_types[command.column_name]))
        //     continue;
        ...
        // if (columns_already_in_alter.count(command.column_name) && !local_context->getSettingsRef().allow_alter_with_unfinished_task)
        //     throw Exception("Alter of column " + command.column_name + " already in other alter query, please wait.", ErrorCodes::LOGICAL_ERROR);

        // if (command.type == AlterCommand::ADD_ROOT_PATHS)
        //     command.root_paths = checkAndNormalizeRootPaths(command.root_paths);

        // /// Unique merge tree has problem work with alter cluster by. Temporary forbid.
        // /// TODO: need fix
        // if (hasUniqueKey() && command.type == AlterCommand::CLUSTER_BY)
        //     throw Exception("Unique table does not support alter cluster by", ErrorCodes::BAD_ARGUMENTS); 
    }

    /// Check that needed transformations can be applied to the list of columns without considering type conversions.
    auto new_metadata = getInMemoryMetadata();
    commands.apply(getStorageID(), new_metadata, local_context);

    auto old_metadata = getInMemoryMetadata();

    checkProperties(new_metadata, old_metadata, /* attach = */false);

    checkTTLExpressions(new_metadata, old_metadata);

    // checkConvertPossible(getColumns().getAllPhysical(), new_metadata.columns.getAllPhysical());
}
