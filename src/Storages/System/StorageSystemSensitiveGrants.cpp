...
NamesAndTypesList StorageSystemSensitiveGrants::getNamesAndTypes()
{
...
void StorageSystemSensitiveGrants::fillData(MutableColumns & res_columns, ContextPtr context, const SelectQueryInfo &) const
{
        ...
        for (const auto & element : elements)
        {
            ...
            if (element.any_column)
            {
                for (const auto & access_type : access_types)
                    add_row(grantee_name, grantee_type, access_type, database, table, nullptr, element.is_partial_revoke);
            }
            else
            {
                for (const auto & access_type : access_types)
                    for (const auto & column : element.columns)
                        add_row(grantee_name, grantee_type, access_type, database, table, &column, element.is_partial_revoke);
            }
        }
    };
    ...
    for (const auto & id : ids)
    {
        ...
        const SensitiveAccessRights * access = nullptr;
        if (auto role = typeid_cast<RolePtr>(entity))
            access = &role->sensitive_access;
        else if (auto user = typeid_cast<UserPtr>(entity))
            access = &user->sensitive_access;
        else
            continue;
