...
namespace ProfileEvents
{
    ...
    extern const Event NexusFSMemoryBufferEvict;
}
...
public:
    ...
    NexusFSIndex() = default;
    NexusFSIndex(const NexusFSIndex &) = delete;
    NexusFSIndex & operator=(const NexusFSIndex &) = delete;
    ...
    struct PACKED_LINLINE ItemRecord
    {
        ...
        // explicit ItemRecord(std::shared_ptr<SegmentHandle> handle_ = nullptr, UInt32 size_ = 0, UInt8 total_hits_ = 0, UInt8 current_hits_ = 0)
        //     : handle(handle_), size(size_), total_hits(total_hits_), current_hits(current_hits_)
        // {
        // }
        explicit ItemRecord(RelAddress address_ = RelAddress(), UInt32 size_ = 0, UInt8 total_hits_ = 0, UInt8 current_hits_ = 0)
            : address(address_), size(size_), total_hits(total_hits_), current_hits(current_hits_)
        {
        }
    };
    // static_assert(14 == sizeof(ItemRecord), "ItemRecord size is 14 bytes");

    struct LookupResult
    {
        friend class NexusFSIndex;
        ...
        // std::shared_ptr<SegmentHandle> getHandler() const
        // {
        //     chassert(found);
        //     return record.handle;
        // }

        RelAddress getAddress() const
        {
            chassert(found);
    ...
    LookupResult insert(UInt64 key, RelAddress address, UInt32 size);

    // // Replaces old address with new address if there exists the key with the identical old address.
    // bool replaceIfMatch(UInt64 key, RelAddress new_address, RelAddress old_address);
    ...
    bool removeIfMatch(UInt64 key, RelAddress address);
