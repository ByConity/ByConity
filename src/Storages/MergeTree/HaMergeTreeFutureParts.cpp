...
void HaMergeTreeFutureParts::add(UInt64 lsn, const Strings & new_parts)
{
    for (auto & part_name : new_parts)
    {
...
void HaMergeTreeFutureParts::remove(UInt64 lsn, const Strings & new_parts)
{
    for (auto & part_name : new_parts)
    {
        auto info = MergeTreePartInfo::fromPartName(part_name, format_version);
