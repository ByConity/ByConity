...
void CnchAttachProcessor::exec()
{
    ...
    try
    {
        ...
        if (!prepared_parts.first.empty())
        {
            ...
            if(disk_type ==  DiskType::Type::ByteHDFS)
            {
                commitPartsFromHDFS(prepared_parts, staged_parts_name);
            }
            else
            {   
                commitPartsFromOthers(prepared_parts);
            }
        }
    }
    catch(...)
...
void CnchAttachProcessor::commitPartsFromHDFS(const PartsWithHistory & prepared_parts, NameSet & staged_parts_name)
{
    // Commit transaction
    {
        ...
        if (is_unique_tbl)
        {
            ...
            cnch_writer.commitPreparedCnchParts(DumpedData{
                .staged_parts = std::move(prepared_parts.second),
            });
        }
        else
        {
...
void CnchAttachProcessor::commitPartsFromOthers(const PartsWithHistory & prepared_parts)
{
    if (is_unique_tbl)
    {
