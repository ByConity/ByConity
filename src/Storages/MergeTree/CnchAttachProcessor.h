...
public:
    ...
    static AttachFilter createPartFilter(const String & part_name, MergeTreeDataFormatVersion ver);
    ...
    void checkFilterResult(const std::vector<MutableMergeTreeDataPartsCNCHVector> & parts_from_sources, UInt64 attach_limit) const;
...
public:
    ...
    AttachContext(const Context & qctx, int pool_expand_thres, int max_thds, Poco::Logger * log)
        : query_ctx(qctx), expand_thread_pool_threshold(pool_expand_thres), max_worker_threads(max_thds), new_txn(nullptr), logger(log)
    {
    }
...
{
public:
    CnchAttachProcessor(StorageCnchMergeTree & tbl, const PartitionCommand & cmd, const ContextMutablePtr & ctx)
        : failure_injection_knob(ctx->getSettingsRef().attach_failure_injection_knob)
        , target_tbl(tbl)
        , from_storage(nullptr)
...
private:
    ...
    PartsFromSources
    collectPartsFromTableDetached(const StorageCnchMergeTree & tbl, const AttachFilter & filter, AttachContext & attach_ctx);
    // Search parts in path which match attach filter
    PartsFromSources collectPartsFromPath(const String & path, const AttachFilter & filter, AttachContext & attach_ctx);
    std::vector<CollectSource>
    discoverCollectSources(const StorageCnchMergeTree & tbl, const DiskPtr & disk, const String & rel_path, int & drill_down_level);
    PartsFromSources collectPartsFromSources(
    ...
    PartsFromSources collectPartsFromActivePartition(StorageCnchMergeTree & tbl, AttachContext & attach_ctx);
    ...
    MutableMergeTreeDataPartsCNCHVector prepareParts(const PartsFromSources & parts_from_sources, AttachContext & attach_ctx);
