...
public:
    ...
    UInt64 serialize(WriteBuffer & buffer, size_t rows_limit);
    ...
    static GinIndexPostingsListPtr deserialize(SeekableReadBuffer& buffer,
...
public:
    ...
    GinIndexStore(const String & name_, GinDataPartHelperPtr && storage_info_);
    GinIndexStore(const String & name_, GinDataPartHelperPtr && storage_info_, UInt64 max_digestion_size_);
    ...
    /// Get next available segment ID by updating file .gin_sid , only for write
    ...
    void setPostingsBuilder(const StringRef & term, GinIndexPostingsBuilderPtr builder);
    ...
    void setStoreDensity(const Float32 & density_);
...
public:
    ...
    GinIndexPostingsListPtr readSegmentPostingsList(const String& term, UInt32 segment_id);

    /// Read postings lists for terms (which are created by tokenzing query string)
    GinPostingsCachePtr createPostingsCacheFromTerms(const std::set<String> & terms);

private:
    /// Initialize gin index files
    void initFileStreams();
    void readSegmentPostingList(const String& term, UInt32 segment_id,
        const GinSegmentDictionaryPtr& dict, GinPostingsCache& cache);

    /// The store for the reader
    GinIndexStorePtr store;

    /// File streams for reading Gin Index
    std::unique_ptr<SeekableReadBuffer> metadata_file_stream;
    std::unique_ptr<SeekableReadBuffer> dict_file_stream;
    std::unique_ptr<SeekableReadBuffer> postings_file_stream;
