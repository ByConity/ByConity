...
namespace DB
{
MergeTreeMarkBitmapIndex::MergeTreeMarkBitmapIndex(MergeTreeData & data_):
    data(data_),
    bitmap_tracker(std::make_unique<BitmapTracker>(data.getContext())),
    log(&Poco::Logger::get("MergeTreeMarkBitmapIndex"))
{
}


MergeTreeBitmapIndexStatus MergeTreeMarkBitmapIndex::getMergeTreeBitmapIndexStatus()
{
    return MergeTreeBitmapIndexStatus();
}

void MergeTreeMarkBitmapIndex::getPartsForBuildingBitmap()
{
    ...
    auto existBitmap = [&](const NameAndTypePair & column, const String & part_path) -> bool
    {
        ...
        String adx_name = column_name + getIdxExtension();
        String irk_name = column_name + getIrkExtension();
    ...
    {
        NamesAndTypesList part_columns = part->getColumns();
        for (const NameAndTypePair & column : part_columns)
        {
            if (MergeTreeMarkBitmapIndex::isMarkBitmapIndexColumn(*column.type))
            {
...
bool MergeTreeMarkBitmapIndex::checkForBuildingBitMap()
{
    size_t part_cnt = 0;
    while(!emptyQueueForBuildingBitMap())
    {
        ...
        //TODO dongyifeng metainfo not merged, should be fixed later
//        try
//        {
//            const_cast<MergeTreeData::DataPart *>(part.get())->metainfo->has_mark_bitmap = true;
//            buildPartIndex(part);
//        }
//        catch(...)
//        {
//            const_cast<MergeTreeData::DataPart *>(part.get())->metainfo->has_mark_bitmap = false;
...
void MergeTreeMarkBitmapIndex::buildPartIndex(const MergeTreeData::DataPartPtr & part)
{
    //TODO dongyifeng metainfo not merged, should be fixed later
    ...
    {
        if(MergeTreeMarkBitmapIndex::isMarkBitmapIndexColumn(column) || isMarkBitmapIndexColumn(column.name))
        {
            if (!column.type->isMarkBitmapIndex())
                const_cast<IDataType *>(column.type.get())->setFlags(TYPE_MARK_BITMAP_INDEX_FALG);
...
//MergeTreeData::AlterDataPartTransactionPtr MergeTreeMarkBitmapIndex::buildPartIndex(const MergeTreeData::DataPartPtr & part, const NamesAndTypesList & columns)
//{
...
//    for (const auto & column : columns)
//    {
//        if (!MergeTreeMarkBitmapIndex::isMarkBitmapIndexColumn(column)) continue;
//
//        String original_column_name = column.name;
//
//        // Skip build this part if it already has adx file and irk file
//        Poco::File adx_file(part->getFullPath()+original_column_name+this->getIdxExtension());
//        Poco::File irk_file(part->getFullPath()+original_column_name+this->getIrkExtension());
//        if (adx_file.exists() && irk_file.exists())
//        {
...
bool MergeTreeMarkBitmapIndex::isMarkBitmapIndexColumn(const NameAndTypePair & column)
{
    return column.type->isMarkBitmapIndex();
}

bool MergeTreeMarkBitmapIndex::isMarkBitmapIndexColumn(const IDataType & type)
{
    return type.isMarkBitmapIndex();
}

bool MergeTreeMarkBitmapIndex::isMarkBitmapIndexColumn(const String & name)
{
    return MergeTreeMarkBitmapIndex::isMarkBitmapIndexColumn(*(data.getInMemoryMetadataPtr()->getColumns().get(name).type));
}

bool MergeTreeMarkBitmapIndex::onlyBuildBitmap(const NamesAndTypesList & old_columns, const NamesAndTypesList & new_columns)
{
    ...
    for (const NameAndTypePair & column : old_columns)
    {
        if (new_types.count(column.name))
        {
            ...
            if (!new_type->equals(*old_type))
                return false;
            else if ((MergeTreeMarkBitmapIndex::isMarkBitmapIndexColumn(*new_type) && !MergeTreeMarkBitmapIndex::isMarkBitmapIndexColumn(*old_type)))
...
void MergeTreeMarkBitmapIndex::dropPartIndex(const MergeTreeData::DataPartPtr & part)
{
    // TPDO dongyifeng add later
//    /// @TODO(pengxindong): support remote parts
//    if (part->info.storage_type != StorageType::Local)
//        return;
    ...
    for (const auto & column : columns)
    {
        if(MergeTreeMarkBitmapIndex::isMarkBitmapIndexColumn(column))
...
//MergeTreeData::AlterDataPartTransactionPtr MergeTreeMarkBitmapIndex::dropPartIndex(const MergeTreeData::DataPartPtr & part, const NamesAndTypesList & columns)
//{
//    MergeTreeData::AlterDataPartTransactionPtr transaction(new MergeTreeData::AlterDataPartTransaction(part)); /// Blocks changes to the part.
//    for (const auto & column : columns)
//    {
...
//        if (MergeTreeMarkBitmapIndex::isMarkBitmapIndexColumn(*column.type))
//        {
//            transaction->rename_map[IDataType::getFileNameForStream(column_name, {})+getIdxExtension()] = "";
//            transaction->rename_map[IDataType::getFileNameForStream(column_name, {})+getIrkExtension()] = "";
//        }
//    }
//
