...
{
    fillPerThreadInfo(threads_, sum_row_groups_);
}

///
void CnchHiveReadPool::fillPerThreadInfo(
    ...
    const size_t sum_row_groups)
{
    ...
    while(!parts.empty())
    {
        ...
        const auto & part = parts.back().data_part;
    ...
    for(size_t cnt = 0; cnt < threads; cnt++)
    {
        ...
        LOG_TRACE(&Poco::Logger::get("CnchHiveReadPool"), "current thread : {}  parts_and_groups size: {} sum_row_groups_in_parts size: {}", cnt , parts_and_groups.size(), sum_row_groups_in_parts);
        ...
        for(size_t j = 0; j < parts_and_groups.size(); j++)
        {
            ...
            LOG_TRACE(&Poco::Logger::get("CnchHiveReadPool"), " part_idx: {} path: {} need_read_row_group_index: {} ", part_idx, path, need_read_row_group_index);
        }
    }
}
...
CnchHiveReadTaskPtr CnchHiveReadPool::getTask(const size_t thread)
{
    ...
    LOG_TRACE(&Poco::Logger::get("CnchHiveReadPool"), "current thread {} get new Task part_idx {} current_row_group: {}", thread, part_idx, current_row_group);
