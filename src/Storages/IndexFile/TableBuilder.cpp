...
#include <assert.h>
#include <Storages/IndexFile/BlockBuilder.h>
...
void TableBuilder::WriteBlock(BlockBuilder * block, BlockHandle * handle)
{
    ...
    // TODO: Support more compression options: zstd?
    switch (type)
    {
        case kNoCompression:
            block_contents = raw;
        ...
        case kSnappyCompression: {
            ...
            compressed->resize(snappy::MaxCompressedLength(raw.size()));
            size_t outlen;
            snappy::RawCompress(raw.data(), raw.size(), &(*compressed)[0], &outlen);
...
void TableBuilder::WriteRawBlock(const Slice & block_contents, CompressionType type, BlockHandle * handle)
{
    ...
    if (r->status.ok())
    {
        ...
        trailer[0] = type;
        uint32_t crc = DB::CRC32C::Value(block_contents.data(), block_contents.size());
        crc = DB::CRC32C::Extend(crc, trailer, 1); // Extend crc to cover block type
        EncodeFixed32(trailer + 1, DB::CRC32C::Mask(crc));
