...
void StorageCnchMergeTree::allocateForBitEngine(
    ContextPtr local_context, const std::optional<std::set<Int64>> & bucket_numbers, WorkerEngineType engine_type, bool replicated)
{
    String local_table_name = getCloudTableName(local_context);

    if (bucket_numbers.has_value())
    {
        auto parts = getAllPartsWithDBM(local_context).first;
        collectResource(local_context, parts, local_table_name, *bucket_numbers, {}, engine_type, replicated);
    }
    else
    /// bucket_numbers = std::nullopt means no part need to be allocated
    {
        auto parts = ServerDataPartsVector{};
        collectResource(local_context, parts, local_table_name, std::set<Int64>{}, {}, engine_type, replicated);
    }
}

BitEngineDictionaryTableMapping StorageCnchMergeTree::generateDictSettingForCloud(ContextPtr & local_context)
{
    BitEngineDictionaryTableMapping underlying_dictionary_table_cloud;
    if (isBitEngineTable())
    {
        const auto & dicts_mapping = getUnderlyDictionaryTables();
        for (const auto & entry : dicts_mapping)
        {
            auto storage_dict = DatabaseCatalog::instance().tryGetTable(StorageID{entry.second.first, entry.second.second}, local_context);
            StorageCnchMergeTree * storage_dict_cnch = dynamic_cast<StorageCnchMergeTree *>(storage_dict.get());
            if (storage_dict_cnch)
            {
                auto dict_table_name_cloud = storage_dict_cnch->getCloudTableName(local_context);
                underlying_dictionary_table_cloud.emplace(entry.first, std::make_pair(entry.second.first, dict_table_name_cloud));
            }
        }
    }
    return underlying_dictionary_table_cloud;
}

void StorageCnchMergeTree::checkBitEngineTableAlterSetting(const String & new_mapping_str, const ContextPtr & local_context) const
{
    auto new_bitengine_dictionary_tables_mapping = parseUnderlyingDictionaryDependency(new_mapping_str);
    checkSchemaForBitEngineTableImpl(new_bitengine_dictionary_tables_mapping, local_context);
}
