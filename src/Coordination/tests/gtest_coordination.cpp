...
void addNode(DB::KeeperStorage & storage, const std::string & path, const std::string & data, int64_t ephemeral_owner=0)
{
    ...
    auto child_path = DB::PathUtils::getBaseName(child_it->key);
    storage.container.updateValue(DB::PathUtils::parentPath(StringRef{path}), [&](auto & parent)
    {
...
TEST_P(CoordinationTest, TestListRequestTypes)
{
    ...
    create_path(PathUtils::parentPath(StringRef{test_path}).toString(), false, false);
    ...
    {
        expected_persistent_children.insert(PathUtils::getBaseName(create_path(test_path, false)).toString());
    }
    ...
    {
        expected_ephemeral_children.insert(PathUtils::getBaseName(create_path(test_path, true)).toString());
    }
    ...
    const auto get_children = [&](const auto list_request_type)
    {
        ...
        int new_zxid = ++zxid;
        list_request->path = PathUtils::parentPath(StringRef{test_path}).toString();
