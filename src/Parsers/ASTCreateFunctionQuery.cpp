...
void ASTCreateFunctionQuery::formatImpl(const IAST::FormatSettings & settings, IAST::FormatState & state, IAST::FormatStateStacked frame) const
{
    ...
    if (!is_lambda)
    {
        const auto *ast = assert_cast<const ASTExternalFunction *>(function_core.get());

        auto type = getFunctionType(ast->args.flags);
        if (type != UDFFunctionType::Scalar)
            settings.ostr << getFunctionTypeStr(type) << " ";
    }
