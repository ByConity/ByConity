...
bool ParserShowColumnsQuery::parseImpl(Pos & pos, ASTPtr & node, Expected & expected)
{
    ...
    bool abbreviated_form = from1_str.find('.') != std::string::npos; // FROM database.table
    if (abbreviated_form)
    {
        std::vector<String> split;
        boost::split(split, from1_str, boost::is_any_of("."));
        query->database = formatTenantDatabaseName(split[0]);
        query->table = split[1];
    }
    else
    {
        if (ParserKeyword("FROM").ignore(pos, expected) || ParserKeyword("IN").ignore(pos, expected))
            if (!ParserIdentifier().parse(pos, from2, expected))
                return false;
        tryRewriteCnchDatabaseName(from2, pos.getContext());
    ...
    if (bool insensitive = ParserKeyword("ILIKE").ignore(pos, expected); insensitive || ParserKeyword("LIKE").ignore(pos, expected))
    {
        ...
        if (!ParserExpressionWithOptionalAlias(false, dt).parse(pos, query->where_expression, expected))
            return false;

    if (ParserKeyword("LIMIT").ignore(pos, expected))
        if (!ParserExpressionWithOptionalAlias(false, dt).parse(pos, query->limit_length, expected))
            return false;
