    ...
    ReadBuffer * istr)
{
    ...
    try
    {
        ...
        TxnTimestamp source_update_time_for_query_cache = TxnTimestamp::minTS();
        {
            ...
            LOG_INFO(&Poco::Logger::get("executeQuery"), "pipeline has all used StorageIDs: {}", res.pipeline.hasAllUsedStorageIDs());
            ...
            {
                LOG_INFO(&Poco::Logger::get("executeQuery"), "StorageIDs:");
                for (auto & storage_id : storage_ids)
                    LOG_INFO(&Poco::Logger::get("executeQuery"), "StorageID {}", storage_id.getNameForLogs());
                HostWithPorts target_hp = context->getCnchTopologyMaster()->getTargetServer(UUIDHelpers::UUIDToString(storage_ids.begin()->uuid), true);
                if (!target_hp.empty())
                {
                    if (!isLocalServer(target_hp.getRPCAddress(), std::to_string(context->getRPCPort())))
                    {
                        LOG_DEBUG(&Poco::Logger::get("executeQuery"), "redirect query to query cache host");
                        executeQueryByProxy(context, target_hp, ast, res);
                        LOG_DEBUG(&Poco::Logger::get("executeQuery"), "Query execution on remote server done");
                        return std::make_tuple(ast, std::move(res));
                    }
                    else
