    ...
    ReadBuffer * istr)
{
    ...
    try
    {
            ...
            try
            {
...

                {
                    ...
                    catch (...)
                    {
                            ...
                            && context->getSettingsRef().fallback_perfect_shard)
                        {
                            LOG_INFO(
                                &Poco::Logger::get("executeQuery"),
                                "Query failed in perfect-shard enabled, try to fallback to normal mode.");
                ...
                /// Everything related to query log.
                {
                    ...
                    auto status_info_to_query_log
                        = [](QueryLogElement & element, const QueryStatusInfo & info, const ASTPtr query_ast) mutable {
                           ...
                           init_time](
                              ...
                              if (log_processors_profiles)
                              {
                                  ...
                                  if (query_pipeline && processors_profile_log)
                                  {
                                      ...
                                      auto get_proc_id
                                          = [](const IProcessor & proc) -> UInt64 { return reinterpret_cast<std::uintptr_t>(&proc); };
        ...
        BlockIO executeQuery(
            const String & query, ContextMutablePtr context, bool internal, QueryProcessingStage::Enum stage, bool may_have_embedded_data)
        {
            ASTPtr ast;
            BlockIO streams;
            std::tie(ast, streams)
            ...
            {
                String format_name
                    = ast_query_with_output->format ? getIdentifierName(ast_query_with_output->format) : context->getDefaultFormat();
