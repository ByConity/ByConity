...
SQLBindingItemPtr BindingCatalogManager::getSQLBinding(const UUID & uuid, const bool & is_re_expression)
{
    return catalog->getSQLBinding(UUIDHelpers::UUIDToString(uuid), is_re_expression);
}

void BindingCatalogManager::removeSQLBinding(const UUID & uuid, const bool & is_re_expression)
{
    catalog->removeSQLBinding(UUIDHelpers::UUIDToString(uuid), is_re_expression);
}
