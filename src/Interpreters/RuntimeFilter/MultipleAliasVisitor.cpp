...
void MultipleAliasMatcher::visit(ASTSubquery & subquery, const ASTPtr & ast, Data & data)
{
    ...
    //   SELECT hash_uid  FROM misc_online_cohort_res_all  WHERE (tea_app_id = 6) AND ((cohort_id = 156) AND (version = 421)) ))  FROM misc_online_all
    //   WHERE (tea_app_id = 6) AND (event_date >= '2019-08-08') AND (event_date <= '2019-08-14') GROUP BY hash_uid SETTINGS distributed_perfect_shard = 1 )
    //
 ...
 * you can refer to https://bytedance.feishu.cn/docs/doccnBZrFotseozIeE1TFoh9ign for more details.
**/
