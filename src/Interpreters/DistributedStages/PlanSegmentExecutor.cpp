...
void PlanSegmentExecutor::buildPipeline(QueryPipelinePtr & pipeline, BroadcastSenderPtrs & senders)
{
    ...
    for (const auto & cur_plan_segment_output : plan_segment_outputs)
    {
        ...
        for (size_t i = 0; i < total_partition_num; i++)
        {
            ...
            auto data_key = std::make_shared<ExchangeDataKey>(plan_segment->getQueryId(), exchange_id, partition_id, coordinator_address);
    ...
    for (size_t i = 0; i < plan_segment_outputs.size(); ++i)
    {
        ...
        switch (exchange_mode)
        {
            ...
            case ExchangeMode::GATHER: {
                ...
                bool need_resize = keep_order && context->getSettingsRef().exchange_enable_keep_order_parallel_shuffle && partition_num > 1;
        ...
        [&](OutputPortRawPtrs ports) -> Processors {
            ...
            for (size_t i = 0; i < segs_output_ports.size(); ++i)
            {
                ...
                switch (exchange_mode)
                {
                    ...
                    case ExchangeMode::LOCAL_MAY_NEED_REPARTITION:
                    case ExchangeMode::GATHER:
                        current_new_processors
                            = buildRepartitionExchangeSink(current_exchange_senders, keep_order, i, header, segs_output_ports[i]);
                        break;
                    case ExchangeMode::LOCAL_NO_NEED_REPARTITION:
                        current_new_processors = buildLoadBalancedExchangeSink(current_exchange_senders, i, header, segs_output_ports[i]);
                        break;
                    case ExchangeMode::BROADCAST:
                        current_new_processors = buildBroadcastExchangeSink(current_exchange_senders, i, header, segs_output_ports[i]);
                        break;
                    default:
                        throw Exception(
                            "Cannot find expected ExchangeMode " + std::to_string(UInt8(exchange_mode)), ErrorCodes::LOGICAL_ERROR);
                }
...
Processors PlanSegmentExecutor::buildBroadcastExchangeSink(
    BroadcastSenderPtrs & senders, size_t output_index, const Block & header, OutputPortRawPtrs & ports)
{
    ...
    for (auto & port : ports)
    {
        ...
        auto exchange_sink = std::make_shared<BroadcastExchangeSink>(header, senders, options, name);
        connect(*port, exchange_sink->getInputs().front());
...
Processors PlanSegmentExecutor::buildLoadBalancedExchangeSink(
    BroadcastSenderPtrs & senders, size_t output_index, const Block & header, OutputPortRawPtrs & ports)
{
    Processors new_processors;
    ...
    for (auto & port : ports)
    {
        ...
        auto exchange_sink = std::make_shared<LoadBalancedExchangeSink>(header, senders, name);
        connect(*port, exchange_sink->getInputs().front());
