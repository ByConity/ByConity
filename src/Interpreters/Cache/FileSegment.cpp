...
String toString(FileSegmentKind kind)
{
    ...
    return "Invalid file segment kind";
}
...
{
    LOG_TRACE(log, "Updated state from {} to {}", stateToString(download_state), stateToString(state));
...
String FileSegment::getCallerId()
{
    if (!CurrentThread::isInitialized()
        ...
        || CurrentThread::getQueryId().toString().empty())
...
{
    LOG_TRACE(log, "Resetting downloader from {}", downloader_id);
    downloader_id.clear();
}
...
void FileSegment::assertIsDownloaderUnlocked(const std::string & operation, std::unique_lock<std::mutex> & segment_lock) const
{
    ...
    auto current_downloader = getDownloaderUnlocked(segment_lock);
    LOG_TRACE(log, "Downloader id: {}, caller id: {}", current_downloader, caller);
...
FileSegment::State FileSegment::wait()
{
    ...
    if (download_state == State::DOWNLOADING)
    {
        LOG_TRACE(log, "{} waiting on: {}, current downloader: {}", getCallerId(), range().toString(), downloader_id);
...
void FileSegment::completePartAndResetDownloaderUnlocked(std::unique_lock<std::mutex> & segment_lock)
{
    ...
    LOG_TRACE(log, "Complete batch. ({})", getInfoForLogUnlocked(segment_lock));
    cv.notify_all();
}
...
void FileSegment::completeBasedOnCurrentState(std::lock_guard<std::mutex> & cache_lock, std::unique_lock<std::mutex> & segment_lock)
{
    ...
    {
        LOG_TRACE(log, "Removing temporary file segment: {}", getInfoForLogUnlocked(segment_lock));
    ...
    switch (download_state)
    {
        ...
        case State::PARTIALLY_DOWNLOADED_NO_CONTINUATION:
        {
            if (is_last_holder)
            {
                if (current_downloaded_size == 0)
                {
                    LOG_TRACE(log, "Remove cell {} (nothing downloaded)", range().toString());
                ...
                {
                    LOG_TRACE(log, "Resize cell {} to downloaded: {}", range().toString(), current_downloaded_size);
    ...
    is_completed = true;
    LOG_TRACE(log, "Completed file segment: {}", getInfoForLogUnlocked(segment_lock));
}
...
String FileSegment::getInfoForLogUnlocked(std::unique_lock<std::mutex> & segment_lock) const
{
    ...
    info << "state: " << stateToString(download_state) << ", ";
...
void FileSegment::detachAssumeStateFinalized(std::unique_lock<std::mutex> & segment_lock)
{
    ...
    LOG_TRACE(log, "Detached file segment: {}", getInfoForLogUnlocked(segment_lock));
}
