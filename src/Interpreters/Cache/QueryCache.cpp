...
QueryCache::Writer::Writer(
    ...
    TxnTimestamp source_update_time_)
    : cache(cache_)
    , key(key_)
    ...
    , source_update_time(source_update_time_)
{
...
QueryCache::Writer::Writer(const Writer & other)
    ...
    , source_update_time(other.source_update_time)
{
}
...
QueryCache::Reader::Reader(Cache & cache_, const Key & key, TxnTimestamp source_update_time, const std::lock_guard<std::mutex> &)
{
...
QueryCache::Reader QueryCache::createReader(const Key & key, TxnTimestamp source_update_time)
{
    std::lock_guard lock(mutex);
    return Reader(cache, key, source_update_time, lock);
}

QueryCache::Writer QueryCache::createWriter(const Key & key, std::chrono::milliseconds min_query_runtime, bool squash_partial_results, size_t max_block_size, size_t max_query_cache_size_in_bytes_quota, size_t max_query_cache_entries_quota, TxnTimestamp source_update_time)
{
    ...
    std::lock_guard lock(mutex);
    return Writer(cache, key, max_entry_size_in_bytes, max_entry_size_in_rows, min_query_runtime, squash_partial_results, max_block_size, source_update_time);
}
...
TxnTimestamp getMaxUpdateTime(const std::set<StorageID> & storage_ids, ContextPtr context)
{
    ...
    {
        auto host_ports = context->getCnchTopologyMaster()->getTargetServer(UUIDHelpers::UUIDToString(storage_id.uuid), ts, true);
        if (host_ports.empty())
        {
        ...
        id->set_database(storage_id.database_name);
        id->set_name(storage_id.table_name);
        id->set_uuid(UUIDHelpers::UUIDToString(storage_id.uuid));
        distribution[host_ports].push_back(id);
    }

    UInt64 max_last_modification_time = 0;
    for (const auto & p : distribution)
    {
        auto rpc_client = context->getCnchServerClientPool().get(p.first);
        auto table_infos = rpc_client->getTableInfo(p.second);
