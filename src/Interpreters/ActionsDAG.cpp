...
Block ActionsDAG::updateHeader(Block header) const
{
...

    {
        ...
        {
            if (node_to_column.count(output_node) == 0)
            {
...
{
    std::sort(outputs.begin(), outputs.end(), [&key_names_pos_map](const Node * lhs, const Node * rhs)
    {
    ...
    const ColumnsWithTypeAndName & all_inputs)
{
    ...
    if (conjunction.rejected.empty())
    {
        ...
        {
            /// If filter column is not needed, remove it from outputs.
...
ActionsDAGPtr ActionsDAG::deserialize(ReadBuffer & buf, ContextPtr context)
{
     ...
     */
    size_t outputs_size;
    readBinary(outputs_size, buf);
    auto & actions_outputs = actions_dag->getOutputs();
    for (size_t i = 0; i < outputs_size; ++i)
    {
        size_t outputs_id;
        readBinary(outputs_id, buf);
        actions_outputs.emplace_back(node_mapping[outputs_id]);
    }
