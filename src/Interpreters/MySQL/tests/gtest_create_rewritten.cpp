...
TEST(MySQLCreateRewritten, ColumnsDataType)
{
    ...
    for (const auto & [test_type, mapped_type] : test_types)
    {
        EXPECT_EQ(queryToString(tryRewrittenCreateQuery(
            ...
            ") ENGINE = HaUniqueMergeTree('/clickhouse/test_database/test_table_1/{shard}/{replica}', '{replica}') " +
            "PARTITION BY intDiv(key, 4294967) ORDER BY tuple(key) UNIQUE KEY tuple(key) SETTINGS partition_level_unique_keys = 0");
...
TEST(MySQLCreateRewritten, PartitionPolicy)
{
    ...
    for (const auto & [test_type, mapped_type, partition_policy] : test_types)
    {
        EXPECT_EQ(queryToString(tryRewrittenCreateQuery(
            ...
            ") ENGINE = HaUniqueMergeTree('/clickhouse/test_database/test_table_1/{shard}/{replica}', '{replica}')" +
            partition_policy + " ORDER BY tuple(key) UNIQUE KEY tuple(key) SETTINGS partition_level_unique_keys = 0");
...
TEST(MySQLCreateRewritten, OrderbyPolicy)
{
    ...
    for (const auto & [test_type, mapped_type, partition_policy] : test_types)
    {
        EXPECT_EQ(queryToString(tryRewrittenCreateQuery(
            ...
            ") ENGINE = HaUniqueMergeTree('/clickhouse/test_database/test_table_1/{shard}/{replica}', '{replica}')" + partition_policy +
            " ORDER BY (key, assumeNotNull(key2)) UNIQUE KEY (key, assumeNotNull(key2)) SETTINGS partition_level_unique_keys = 0");
        ...
        EXPECT_EQ(queryToString(tryRewrittenCreateQuery(
            ...
            ") ENGINE = HaUniqueMergeTree('/clickhouse/test_database/test_table_1/{shard}/{replica}', '{replica}')" + partition_policy +
            " ORDER BY (key, key2) UNIQUE KEY (key, key2) SETTINGS partition_level_unique_keys = 0");

        EXPECT_EQ(queryToString(tryRewrittenCreateQuery(
            "CREATE TABLE `test_database`.`test_table_1` (`key` " + test_type + " KEY UNIQUE KEY)", context_holder.context)),
            "CREATE TABLE test_database.test_table_1 (`key` " + mapped_type +
            ") ENGINE = HaUniqueMergeTree('/clickhouse/test_database/test_table_1/{shard}/{replica}', '{replica}')" + partition_policy +
            " ORDER BY tuple(key) UNIQUE KEY tuple(key) SETTINGS partition_level_unique_keys = 0");
...
TEST(MySQLCreateRewritten, RewrittenQueryWithPrimaryKey)
{
    ...
    EXPECT_EQ(queryToString(tryRewrittenCreateQuery(
        ...
        "CREATE TABLE test_database.test_table_1 (`key` Int32" + std::string() +
        ") ENGINE = HaUniqueMergeTree('/clickhouse/test_database/test_table_1/{shard}/{replica}', '{replica}') " +
        "PARTITION BY intDiv(key, 4294967) ORDER BY tuple(key) UNIQUE KEY tuple(key) SETTINGS partition_level_unique_keys = 0");
    ...
    EXPECT_EQ(queryToString(tryRewrittenCreateQuery(
        ...
        "CREATE TABLE test_database.test_table_1 (`key_1` Int32, `key_2` Int32" + std::string() +
        ") ENGINE = HaUniqueMergeTree('/clickhouse/test_database/test_table_1/{shard}/{replica}', '{replica}') " +
        "PARTITION BY intDiv(key_1, 4294967) ORDER BY (key_1, key_2) UNIQUE KEY (key_1, key_2) SETTINGS partition_level_unique_keys = 0");
    ...
    EXPECT_EQ(queryToString(tryRewrittenCreateQuery(
        ...
        "CREATE TABLE test_database.test_table_1 (`key_1` Int64, `key_2` Int32" + std::string() +
        ") ENGINE = HaUniqueMergeTree('/clickhouse/test_database/test_table_1/{shard}/{replica}', '{replica}') " +
        "PARTITION BY intDiv(key_2, 4294967) ORDER BY (key_1, key_2) UNIQUE KEY (key_1, key_2) SETTINGS partition_level_unique_keys = 0");
}
...
TEST(MySQLCreateRewritten, RewrittenQueryWithPrefixKey)
{
    ...
    EXPECT_EQ(queryToString(tryRewrittenCreateQuery(
        ...
        "CREATE TABLE test_database.test_table_1 (`key` Int32, `prefix_key` String" + std::string() +
        ") ENGINE = HaUniqueMergeTree('/clickhouse/test_database/test_table_1/{shard}/{replica}', '{replica}') " +
        "PARTITION BY intDiv(key, 4294967) ORDER BY (key, prefix_key) UNIQUE KEY (key, prefix_key) SETTINGS partition_level_unique_keys = 0");
}
...
TEST(MySQLCreateRewritten, UniqueKeysConvert)
{
    ...
    EXPECT_EQ(queryToString(tryRewrittenCreateQuery(
        ...
        "CREATE TABLE test_database.test_table_1 (`code` String, `name` String, `id` Int64, `tenant_id` Int64" + std::string() +
        ") ENGINE = HaUniqueMergeTree('/clickhouse/test_database/test_table_1/{shard}/{replica}', '{replica}') " +
        "PARTITION BY intDiv(id, 18446744073709551) ORDER BY (code, name, tenant_id, id) UNIQUE KEY (code, name, tenant_id, id) SETTINGS partition_level_unique_keys = 0");
}
...
TEST(MySQLCreateRewritten, QueryWithColumnComments)
{
    ...
    EXPECT_EQ(queryToString(tryRewrittenCreateQuery(
        ...
        "CREATE TABLE test_database.test_table_1 (`key` Int32, `test` Nullable(Int32) COMMENT 'test_comment'" + std::string() +
        ") ENGINE = HaUniqueMergeTree('/clickhouse/test_database/test_table_1/{shard}/{replica}', '{replica}') " +
        "PARTITION BY intDiv(key, 4294967) ORDER BY tuple(key) UNIQUE KEY tuple(key) SETTINGS partition_level_unique_keys = 0");
}
...
TEST(MySQLCreateRewritten, QueryWithEnum)
{
    ...
    EXPECT_EQ(queryToString(tryRewrittenCreateQuery(
        ...
        "CREATE TABLE test_database.test_table_1 (`key` Int32, `test` Nullable(Enum8('a' = 1, 'b' = 2, 'c' = 3))" + std::string() +
        ") ENGINE = HaUniqueMergeTree('/clickhouse/test_database/test_table_1/{shard}/{replica}', '{replica}') " +
        "PARTITION BY intDiv(key, 4294967) ORDER BY tuple(key) UNIQUE KEY tuple(key) SETTINGS partition_level_unique_keys = 0");
    EXPECT_EQ(queryToString(tryRewrittenCreateQuery(
        "CREATE TABLE `test_database`.`test_table_1`(`key` INT NOT NULL PRIMARY KEY, `test` ENUM('a','b','c') NOT NULL)", context_holder.context)),
        "CREATE TABLE test_database.test_table_1 (`key` Int32, `test` Enum8('a' = 1, 'b' = 2, 'c' = 3)" + std::string() +
        ") ENGINE = HaUniqueMergeTree('/clickhouse/test_database/test_table_1/{shard}/{replica}', '{replica}') " +
        "PARTITION BY intDiv(key, 4294967) ORDER BY tuple(key) UNIQUE KEY tuple(key) SETTINGS partition_level_unique_keys = 0");
    EXPECT_EQ(queryToString(tryRewrittenCreateQuery(
        ...
        "CREATE TABLE test_database.test_table_1 (`key` Int32, `test` Nullable(Enum8('a' = 1, 'b' = 2, 'c' = 3)) COMMENT 'test_comment'" + std::string() +
        ") ENGINE = HaUniqueMergeTree('/clickhouse/test_database/test_table_1/{shard}/{replica}', '{replica}') " +
        "PARTITION BY intDiv(key, 4294967) ORDER BY tuple(key) UNIQUE KEY tuple(key) SETTINGS partition_level_unique_keys = 0");
}
