...
void ExternalDictionaryLibraryBridgeRequestHandler::handleRequest(HTTPServerRequest & request, HTTPServerResponse & response)
{
    ...
    try
    {
        ...
        if (lib_new)
        {
            ...
            auto format = FormatFactory::instance().getInputFormat(FORMAT, read_block_buf, *sample_block, getContext(), DEFAULT_BLOCK_SIZE);
        ...
        else if (method == "extDict_loadKeys")
        {
            ...
            auto & read_buf = request.getStream();
            auto format = FormatFactory::instance().getInputFormat(FORMAT, read_buf, *requested_sample_block, getContext(), DEFAULT_BLOCK_SIZE);
