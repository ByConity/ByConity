...
You can deploy ByConity through [package deployment](../deployment/package-deployment), using [docker wrapper](../deployment/docker-wrapper) or [deploy ByConity in Kubernetes](../deployment/deploy-k8s).   

After the cluster is up and running, let's give it a quick spin :)
