...
To enable high availability in the ByConity cluster, you need to enable the ZooKeeper configuration for all components. Please refer to the server.yaml file：
https://github.com/ByConity/byconity-deploy/blob/master/chart/byconity/files/server.yaml#L96，
enbale zookeeper：```yaml  partition_by: event_date  flush_interval_milliseconds: 15000
  mark_cache_size: 5368709120  zookeeper: {} # enable zookeeper
  cnch_config: /etc/byconity/cnch-config.yaml
```
The official provides a Helm configuration YAML file for high availability, eg：https://github.com/ByConity/byconity-deploy/blob/master/examples/k8s/value_HA_example.yml。
