...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
extern bool hualloc_use_numa_info;
extern bool hualloc_enable_mbind;
extern size_t hualloc_numa_node_count;
extern std::unordered_map<size_t, size_t> hualloc_cpu_index_to_numa_node;
...
static std::atomic<ui64> GAllocCnt = 0;
static std::atomic<ui64> LargeReclaimCnt = 0;
static std::atomic<ui64> SegmentReclaimCnt = 0;
static std::atomic<ui64> TotalSegmentAlloc = 0;
static std::atomic<ui64> TotalSegmentFree = 0;
static std::atomic<ui64> TotalLargeAlloc = 0;
static std::atomic<ui64> TotalLargeFree = 0;
static std::atomic<ui64> TotalGiantAlloc = 0;
static std::atomic<ui64> TotalGiantFree = 0;
...
NOINLINE static void* GAAlloc(yint szArg)
{
    ...
    GAllocCnt.fetch_add(1);
    yint k = GAHashPtr(ptr);
    TotalGiantAlloc.fetch_add(sz, std::memory_order_relaxed);
    for (yint kStart = k;;) {
...
static void GAFree(void *ptr)
{
    ...
    TotalGiantFree.fetch_add(sz, std::memory_order_relaxed);
    OsFree(ptr, sz);
}
...
static void LargeInit()
{
    ...
    if (hualloc_enable_mbind)
        mbind_memory(LargeMemoryPtr, LARGE_GROUP_COUNT * LARGE_GROUP_SIZE, LARGE_GROUP_SIZE);
...
static void* LargeAllocChunks(yint chunkCount)
{
    ...
    if (hualloc_use_numa_info)
    {
        int thread_numa_node = get_thread_numa_mem_index();
        group_start = LARGE_GROUP_COUNT*thread_numa_node/hualloc_numa_node_count;
    }
    else
    {
        group_start = 0;
    ...
    for (yint g_offset = 0; g_offset < LARGE_GROUP_COUNT; ++g_offset) {
        yint g;
        if constexpr ((LARGE_GROUP_COUNT & (LARGE_GROUP_COUNT-1)) > 0)
            g = (group_start+g_offset)%LARGE_GROUP_COUNT;
        else
            g = (group_start+g_offset)&(LARGE_GROUP_COUNT-1);
...
NOINLINE static void* LargeAlloc(yint sz)
{
    ...
    TotalLargeAlloc.fetch_add(LARGE_CHUNK_SIZE * chunkCount, std::memory_order_relaxed);
...
static void LargeFree(void *p)
{
    ...
    TotalLargeFree.fetch_add(chunkCount * LARGE_CHUNK_SIZE, std::memory_order_relaxed);
    if (chunkCount > LARGE_MAX_CHUNK_ALLOC_LEN) {
        yint len = (chunkCount + 63) / 64;
...
const yint SIZE_OF_BLOCK = 1 << BLOCK_SIZE_LN;
const yint NEED_2LAYER = BLOCK_SIZE_LN - 6;
const yint MIN_POW2 = SEGMENT_SIZE_LN - 18;
const yint MIN_ALLOC = 1ull << MIN_POW2;
const yint BITS_OFFSET_MULT = SIZE_OF_BLOCK / 64 / sizeof(ui64); // freebits per block size
...
static void SegmentInit()
{
    ...
    if (hualloc_enable_mbind)
        mbind_memory(SegmentMemoryPtr, SEGMENT_COUNT * SEGMENT_SIZE, SEGMENT_SIZE);

    Y_ASSERT(BITS_OFFSET_MULT * sizeof(ui64) * 8 >= SIZE_OF_BLOCK / MIN_ALLOC); // MemFreeBits has space for one bit per MIN_ALLOC
    Y_ASSERT(BITS_OFFSET_MULT * BLOCK_COUNT * sizeof(ui64) <= SIZE_OF_BLOCK); // MemFreeBits fits into first block
...
{
    return (chunkOffset & (SIZE_OF_BLOCK - 1)) >> (pow2 + 6);
}
...
static void SwitchSegment(ui64 allowMask, yint len)
{
    ...
    {
        int thread_numa_node = get_thread_numa_mem_index();
        segment_start = SEGMENT_COUNT*thread_numa_node/hualloc_numa_node_count;
    }
    else
    {
        segment_start = 0;
    ...
    yint lenLn = CalcLog2(len);
    for (yint hierStart3_offset = 0; hierStart3_offset < SEGMENT_COUNT; hierStart3_offset += SEGMENT_HIER_SIZE3) {
        yint hierStart3;
        if constexpr ((SEGMENT_COUNT & (SEGMENT_COUNT-1)) > 0)
            hierStart3 = (segment_start+hierStart3_offset)%SEGMENT_COUNT;
        else
            hierStart3 = (segment_start+hierStart3_offset)&(SEGMENT_COUNT-1);
...
{
    yint len = (sz + SIZE_OF_BLOCK - 1) / SIZE_OF_BLOCK;
    for (;;) {
        if (pThreadSegment) {
            ...
            if (BitMaskFind(segHdr->LocalFreeBlockMask & ALL_BLOCKS, len, &freePtr)) {
                ...
                TotalSegmentAlloc.fetch_add(SIZE_OF_BLOCK * len, std::memory_order_relaxed);
                return segment + freePtr * SIZE_OF_BLOCK;
            }
...
NOINLINE static void SegmentMediumFree(char *segment, yint block)
{
    ...
    yint len = (sz + SIZE_OF_BLOCK - 1) / SIZE_OF_BLOCK;
    TotalSegmentFree.fetch_add(SIZE_OF_BLOCK * len, std::memory_order_relaxed);
...
{
    TSegmentHeader *segHdr = (TSegmentHeader*)segment;
    yint idx = (memPtr & (SIZE_OF_BLOCK - 1)) >> pow2;
    TGlobalSizeContext &gCtx = GlobalSizeCtx[pow2];
    std::atomic<ui64> *blockFreeBits = ((std::atomic<ui64>*)segment) + BITS_OFFSET_MULT;
    std::atomic<ui64> &mask = blockFreeBits[block];
    ...
    for (;; SwitchSegment(ALL_2LAYER_BLOCKS, 1)) {
        ...
        if (segment) {
            ...
            if (!_BitScanForward64(&idx, sCtx.LocalFreeAllocMask)) {
                ...
                sCtx.ChunkOffset = block * SIZE_OF_BLOCK + (chunk << (6 + pow2));
...
{
    TSegmentHeader *segHdr = (TSegmentHeader*)segment;
    yint blockOffset = (memPtr & (SIZE_OF_BLOCK - 1)) >> pow2;
...
static void* hu_alloc(yint sz)
{
    ...
    if (pow2 < BLOCK_SIZE_LN) {
        ...
        TotalSegmentAlloc.fetch_add(1ull<<pow2, std::memory_order_relaxed);
        char *segment = pThreadSegment;
        if (segment) {
        ...
        if (pow2 < (ui8)NEED_2LAYER) {
            ...
            if (segment == pThreadSegment) {
                ...
                if ((memPtr & sCtx.ChunkOffsetMask) == sCtx.ChunkOffset) {
                    ...
                    TotalSegmentFree.fetch_add(1ull << pow2, std::memory_order_relaxed);
                    return;
                }
            }
            TotalSegmentFree.fetch_add(1ull << pow2, std::memory_order_relaxed);
            ...
            if (segment == pThreadSegment) {
                ...
                TSegmentSizeContext &sCtx = segHdr->SizeCtx[pow2];
                yint blockOffset = memPtr & ~(SIZE_OF_BLOCK - 1);
