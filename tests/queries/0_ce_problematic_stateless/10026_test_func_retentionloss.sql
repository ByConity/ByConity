...
CREATE TABLE test_func_retention_loss (`hash_uid` String, `server_time` Int64, `profile` String, first_day UInt8) ENGINE = MergeTree PARTITION BY toDate(server_time) ORDER BY server_time;
...
SELECT retentionLoss(10)(first_events, return_events)
FROM
(
    SELECT *
    FROM
