...
create table test.unique_with_partition_version_r1 (event_time DateTime, id UInt64, s String, m1 UInt32, m2 UInt64)
ENGINE=HaUniqueMergeTree('/clickhouse/tables/test/unique_with_partition_version_test', 'r1', toDate(event_time)) partition by toDate(event_time) order by s unique key id
SETTINGS partition_level_unique_keys=0, ha_unique_update_log_sleep_ms=10, ha_unique_replay_log_sleep_ms=10;

create table test.unique_with_partition_version_r2 (event_time DateTime, id UInt64, s String, m1 UInt32, m2 UInt64)
ENGINE=HaUniqueMergeTree('/clickhouse/tables/test/unique_with_partition_version_test', 'r2', toDate(event_time)) partition by toDate(event_time) order by s unique key id
...
system sync replica test.unique_with_partition_version_r2;
