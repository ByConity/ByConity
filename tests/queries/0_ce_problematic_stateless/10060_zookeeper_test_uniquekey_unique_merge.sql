...
CREATE table test.test_unique_merge_1 ( `d` Date, `k` UInt64, `v` UInt64)
ENGINE = HaUniqueMergeTree('/clickhouse/tables/test/test_unique_merge', 'r1') partition by d unique key k order by k
SETTINGS ha_unique_update_log_sleep_ms=10, ha_unique_replay_log_sleep_ms=10;

CREATE table test.test_unique_merge_2 (`d` Date, `k` UInt64, `v` UInt64)
ENGINE = HaUniqueMergeTree('/clickhouse/tables/test/test_unique_merge', 'r2')
partition by d unique key k order by k SETTINGS ha_unique_update_log_sleep_ms=10, ha_unique_replay_log_sleep_ms=10, replicated_can_become_leader=0;
...
system sync replica test.test_unique_merge_2;
