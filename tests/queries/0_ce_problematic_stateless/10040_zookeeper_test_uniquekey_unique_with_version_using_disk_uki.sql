...
create table test.unique_with_version_uki_r1 (event_time DateTime, id UInt64, s String, m1 UInt32, m2 UInt64)
ENGINE=HaUniqueMergeTree('/clickhouse/tables/test/unique_with_version_uki_1', 'r1', event_time) partition by toDate(event_time) order by s unique key id
SETTINGS ha_unique_update_log_sleep_ms=10, ha_unique_replay_log_sleep_ms=10, enable_disk_based_unique_key_index=0;

create table test.unique_with_version_uki_r2 (event_time DateTime, id UInt64, s String, m1 UInt32, m2 UInt64)
ENGINE=HaUniqueMergeTree('/clickhouse/tables/test/unique_with_version_uki_1', 'r2', event_time) partition by toDate(event_time) order by s unique key id
...
system sync replica test.unique_with_version_uki_r2;
