...
CREATE table test.drop_move_attach_partition_r1 (
    `event_time` DateTime,
    `product_id` UInt64)
ENGINE = HaUniqueMergeTree('/clickhouse/tables/test/move_partition_test_1', 'r1')
...
CREATE table test.drop_move_attach_partition_r2 (
    `event_time` DateTime,
    `product_id` UInt64)
ENGINE = HaUniqueMergeTree('/clickhouse/tables/test/move_partition_test_1', 'r2')
partition by toDate(event_time)
unique key product_id
order by product_id
...
CREATE table test.drop_move_attach_partition_r3 (
    `event_time` DateTime,
    `product_id` UInt64)
ENGINE = HaUniqueMergeTree('/clickhouse/tables/test/move_partition_test_2', 'r3')
partition by toDate(event_time)
unique key product_id
order by product_id
SETTINGS ha_unique_update_log_sleep_ms=10, ha_unique_replay_log_sleep_ms=10;
...
CREATE table test.drop_move_attach_partition_r4 (
    `event_time` DateTime,
    `product_id` UInt64)
ENGINE = HaUniqueMergeTree('/clickhouse/tables/test/move_partition_test_2', 'r4')
partition by toDate(event_time)
unique key product_id
order by product_id
SETTINGS ha_unique_update_log_sleep_ms=10, ha_unique_replay_log_sleep_ms=10, replicated_can_become_leader=0;
...
system sync replica test.drop_move_attach_partition_r2;
system sync replica test.drop_move_attach_partition_r4;
