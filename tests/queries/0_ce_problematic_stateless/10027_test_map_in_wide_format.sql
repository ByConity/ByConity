...
insert into test.10027_test_map_in_wide_format values(1000, {'s1':'s_v1', 's2':'s_v2'}, {'s_l1':['l_v1', 'l_v2', 'l_v3']}, {1: 1, 2: 2}, {1:[1, 2, 3], 2:[4,5,6]});
...
select '';
select 'select id, int_params, int_params{1}, int_list_params from test.10027_test_map_in_wide_format';
select id, int_params, int_params{1}, int_list_params from test.10027_test_map_in_wide_format order by id;
select '';
...
insert into test.10027_test_map_in_wide_format values(1001, {'s3':'s_v3', 's4':'s_v4'}, {'s_l2':['l_v4', 'l_v5']}, {2: 1, 3: 3}, {3:[4, 5, 6, 7]});
...
insert into test.10027_test_map_in_wide_format values(1002, {'s4':'s_v3', 's5':'s_v5'}, {'s_l2':['l_2'], 's_l3': ['l_3']}, {4: 4}, {4:[6, 7]});
optimize table test.10027_test_map_in_wide_format final;
select '';
select 'select id, string_params, string_params{\'s1\'}, string_list_params from test.10027_test_map_in_wide_format';
select id, string_params, string_params{'s1'}, string_list_params from test.10027_test_map_in_wide_format order by id;
...
create table test.10027_test_map_in_wide_format(
    ...
    `int_params` Map(Int32, Int32),
    `int_list_params` Map(Int32, Array(Int32))
) ENGINE = MergeTree ORDER BY id SETTINGS index_granularity = 8192, enable_compact_map_data = 1, min_bytes_for_wide_part = 0, index_granularity_bytes = 0;
...
select '-------------------------------------------------------------';
select '-------------------------------------------------------------';
select 'disable adaptive index granularity and test';
select 'set use_uncompressed_cache = 0, optimize_map_column_serialization = 0 and write data in wide format';
