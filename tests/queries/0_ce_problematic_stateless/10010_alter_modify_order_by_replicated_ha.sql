...
CREATE TABLE old_style(d Date, x UInt32) ENGINE HaMergeTree('/clickhouse/tables/test_10010/old_style', 'r1', d, x, 8192);
...
CREATE TABLE summing_r1(x UInt32, y UInt32, val UInt32) ENGINE HaSummingMergeTree('/clickhouse/tables/test_10010/summing', 'r1') ORDER BY (x, y);
CREATE TABLE summing_r2(x UInt32, y UInt32, val UInt32) ENGINE HaSummingMergeTree('/clickhouse/tables/test_10010/summing', 'r2') ORDER BY (x, y);
...
-- ALTER TABLE summing_r1 ADD COLUMN t UInt32 AFTER z, MODIFY ORDER BY (x, y, t * t) SETTINGS ha_alter_metadata_sync = 2; -- { serverError 341 }
