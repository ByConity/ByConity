...
if [ ! -z "$CLICKHOUSE_ADDITIONAL_CLIENT_OPT" ]; then
    CLICKHOUSE_CLIENT=`echo ${CLICKHOUSE_CLIENT} | sed "s@${CLICKHOUSE_ADDITIONAL_CLIENT_OPT}@@g"`
fi

if [ ! -z "$CLICKHOUSE_ADDITIONAL_CLIENT_URL_PARAMS" ]; then
    CLICKHOUSE_URL=`echo ${CLICKHOUSE_URL} | sed "s@${CLICKHOUSE_ADDITIONAL_CLIENT_URL_PARAMS}&@@g"`
fi
...
CLICKHOUSE_CLIENT=$(echo ${CLICKHOUSE_CLIENT} | sed 's/'"--send_logs_level=${CLICKHOUSE_CLIENT_SERVER_LOGS_LEVEL}"'//g')
