...
create table test.table_level_unique_composite_key_r1 (d Date, id Int32, s String, arr Array(Int32), sum materialized arraySum(arr))
ENGINE=HaUniqueMergeTree('/clickhouse/tables/test/table_level_unique_composite_key', 'r1') partition by d order by s unique key (id,s)
SETTINGS partition_level_unique_keys=0, ha_unique_update_log_sleep_ms=10, ha_unique_replay_log_sleep_ms=10;

create table test.table_level_unique_composite_key_r2 (d Date, id Int32, s String, arr Array(Int32), sum materialized arraySum(arr))
ENGINE=HaUniqueMergeTree('/clickhouse/tables/test/table_level_unique_composite_key', 'r2') partition by d order by s unique key (id,s)
...
system sync replica test.table_level_unique_composite_key_r2;
