...
create table test.table_level_unique_uki_r1 (d Date, id Int32, s String, arr Array(Int32), sum materialized arraySum(arr))
ENGINE=HaUniqueMergeTree('/clickhouse/tables/test/table_level_unique_uki', 'r1') partition by d order by (s, id) primary key s unique key id
...
create table test.table_level_unique_uki_r2 (d Date, id Int32, s String, arr Array(Int32), sum materialized arraySum(arr))
ENGINE=HaUniqueMergeTree('/clickhouse/tables/test/table_level_unique_uki', 'r2') partition by d order by (s, id) primary key s unique key id
...
system sync replica test.table_level_unique_uki_r2;
