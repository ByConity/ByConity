...
CREATE TABLE types(I16 Int16, I32 Int32, I64 Int64, F32 Float32, F64 Float64, D Date, DT DateTime) Engine = MergeTree ORDER BY tuple();
...
DROP TABLE types;
CREATE TABLE types(A32 Array(Int32), AN32 Array(Nullable(Int32)), NA32 Nullable(Array(Int32))) Engine = MergeTree ORDER BY tuple();
...
SELECT map(NULL, 'value1', 'key2', NULL, 'key3', 'value3');

-- function str_to_map
SELECT str_to_map('a=b&c=d', '&', '=');
SELECT str_to_map('a=b&c=d', '&&', '==');
SELECT str_to_map('a=b&c', '&', '=');
SELECT str_to_map('a=b&c=', '&', '=');
SELECT str_to_map('a=b', '&', ''); -- { serverError 44 }
SELECT str_to_map('a=b', '', '='); -- { serverError 44 }
SELECT str_to_map(_, '&', '=') from (SELECT arrayJoin(['a', 'a=b', 'a=b&c', 'a=b&c=d', 'a=b&&c=d']) as _);
