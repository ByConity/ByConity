...
$CLICKHOUSE_CLIENT -q "select x + 1 from (select y + 2 as x from (select dummy + 3 as y)) settings query_plan_max_optimizations_to_apply = 1, enable_optimizer = 0" 2>&1 |
     grep -o "Too many optimizations applied to query plan"

echo "> sipHash should be calculated after filtration"
$CLICKHOUSE_CLIENT -q "explain actions = 1 select sum(x), sum(y) from (select sipHash64(number) as x, bitAnd(number, 1024) as y from numbers_mt(1000000000) limit 1000000000) where y = 0 settings enable_optimizer = 0" | grep -o "FUNCTION sipHash64\|Filter column: equals"
echo "> sorting steps should know about limit"
$CLICKHOUSE_CLIENT -q "explain actions = 1 select number from (select number from numbers(500000000) order by -number) limit 10 settings enable_optimizer = 0" | grep -o "MergingSorted\|MergeSorting\|PartialSorting\|Limit 10"
...
$CLICKHOUSE_CLIENT -q "
    ...
    settings enable_optimize_predicate_expression=0, enable_optimizer=0" | grep -o "Aggregating\|Filter"
$CLICKHOUSE_CLIENT -q "
    select s, y from (select sum(x) as s, y from (
        select number as x, number + 1 as y from numbers(10)) group by y
    ) where y != 0 order by s, y
    settings enable_optimize_predicate_expression=0, enable_optimizer=0"
...
$CLICKHOUSE_CLIENT -q "
    ...
    settings enable_optimize_predicate_expression=0, enable_optimizer=0" | grep -o "Aggregating\|Filter\|COLUMN Const(UInt8) -> notEquals(y, 0)"
$CLICKHOUSE_CLIENT -q "
...
$CLICKHOUSE_CLIENT -q "
    ...
    ) array join x where y != 2 and x != 0 settings enable_optimizer=0" |
...
$CLICKHOUSE_CLIENT -q "
    ...
    ) where number != 2 settings enable_optimize_predicate_expression=0, enable_optimizer=0" |
...
$CLICKHOUSE_CLIENT -q "
    ...
    ) as r on a = r.b where a != 1 and b != 2 settings enable_optimize_predicate_expression = 0, enable_optimizer = 0" |
...
$CLICKHOUSE_CLIENT -q "
    ...
    ) where a != 1 settings enable_optimize_predicate_expression = 0, enable_optimizer = 0" |
    grep -o "Union\|Filter"
$CLICKHOUSE_CLIENT -q "
    select a, b from (
        select number + 1 as a, number + 2 as b from numbers(2) union all select number + 1 as b, number + 2 as a from numbers(2)
