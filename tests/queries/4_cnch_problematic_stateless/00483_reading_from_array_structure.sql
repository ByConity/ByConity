...
create table test.table (date Date, `Struct.Key1` Array(UInt64), `Struct.Key2` Array(UInt64), padding FixedString(16)) engine = CnchMergeTree() PARTITION BY toYYYYMM(date) ORDER BY date SETTINGS index_granularity = 16;
...
drop table if exists test.table;
create table test.table (date Date, `Struct.Key1` Array(UInt64), `Struct.Key2` Array(UInt64), padding FixedString(16), x UInt64) engine = CnchMergeTree() PARTITION BY toYYYYMM(date) ORDER BY date SETTINGS index_granularity = 8;
