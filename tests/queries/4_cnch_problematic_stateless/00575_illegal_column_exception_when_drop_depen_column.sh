...
${CLICKHOUSE_CLIENT} --query "CREATE TABLE test.test (dt Date DEFAULT now(), id UInt32, id2 UInt32 DEFAULT id + 1) ENGINE = CnchMergeTree() PARTITION BY toYYYYMM(dt) ORDER BY dt SETTINGS index_granularity=8192;"
