...
CREATE TABLE `test`.`test_log` (
    ...
    old_fields Nested(name String, v String, is_null Enum8('true'=1, 'false'=0)),
    new_fields Nested(name String, v String, is_null Enum8('true'=1, 'false'=0)),
    record_source_type Int8,
    record_source_timestamp DateTime,
    deleted Enum8('true'=1, 'false'=0)
) ENGINE = CnchMergeTree()
PARTITION BY toYYYYMM(date)
ORDER BY (date, path, gtid, query_serial, row_serial)
...
QUERY='INSERT INTO `test`.`test_log`(`date`, `datetime`, `path`, `gtid`, `query_serial`, `row_serial`,
    ...
    `old_fields`.`v`, `old_fields`.`is_null`, `new_fields`.`name`, `new_fields`.`v`,
    `new_fields`.`is_null`, `record_source_type`, `record_source_timestamp`, `deleted`) FORMAT TabSeparated'
QUERY="$(tr -d '\n' <<<"$QUERY")"
echo $QUERY
URL=$(python3 -c 'print("'${CLICKHOUSE_URL}'?query=" + __import__("urllib.parse").parse.quote("'"$QUERY"'"))')
