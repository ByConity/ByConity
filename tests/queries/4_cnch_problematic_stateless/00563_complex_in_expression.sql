...
CREATE TABLE test_00563 ( dt Date, site_id Int32, site_key String ) ENGINE = CnchMergeTree
PARTITION BY dt ORDER BY (site_id, site_key, dt);
...
CREATE TABLE join_with_index (key UInt32, data UInt64) ENGINE = CnchMergeTree ORDER BY key SETTINGS index_granularity=1;
