...
$CLICKHOUSE_CLIENT --query="CREATE TABLE test.csv (s String, n UInt64 DEFAULT 1, d Date DEFAULT '2019-06-19') ENGINE = CnchMergeTree() ORDER BY d";
...
default,,
default-eof,,' | $CLICKHOUSE_CLIENT --input_format_defaults_for_omitted_fields=1 --query="INSERT INTO test.csv FORMAT CSV";

$CLICKHOUSE_CLIENT --query="SELECT * FROM test.csv ORDER BY d";
$CLICKHOUSE_CLIENT --query="DROP TABLE test.csv";

$CLICKHOUSE_CLIENT --query="CREATE TABLE test.csv (t DateTime('Europe/Moscow'), s String) ENGINE = CnchMergeTree() ORDER BY s";
