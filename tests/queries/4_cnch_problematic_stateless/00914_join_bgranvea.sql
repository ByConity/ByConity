...
CREATE TABLE table1 (A String, B String, ts DateTime) ENGINE = CnchMergeTree PARTITION BY toStartOfDay(ts)  ORDER BY (ts, A, B);
CREATE TABLE table2 (B String, ts DateTime) ENGINE = CnchMergeTree PARTITION BY toStartOfDay(ts) ORDER BY (ts, B);
