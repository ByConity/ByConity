...
INSERT INTO col_masking VALUES ('Jane', 18, 12345678, '12345678');
INSERT INTO col_masking VALUES ('Bob', 24, 87655678, '87655678');

SELECT `credit_card_num`, `credit_card_num_plus_1`, `credit_card_str`, `credit_card_str_prepend_a` FROM col_masking ORDER BY `name`;

ALTER TABLE col_masking MODIFY COLUMN credit_card_num SET MASKING POLICY credit_card_mask_test_1;
ALTER TABLE col_masking MODIFY COLUMN credit_card_str SET MASKING POLICY credit_card_mask_test_2;

-- alias columns are calculated by the original values
SELECT `credit_card_num`, `credit_card_num_plus_1`, `credit_card_str`, `credit_card_str_prepend_a` FROM col_masking ORDER BY `name`;

-- unable to mask for alias columns
ALTER TABLE col_masking MODIFY COLUMN credit_card_str_prepend_a SET MASKING POLICY credit_card_mask_test_2; -- { serverError 16 }

-- clean up
ALTER TABLE col_masking MODIFY COLUMN credit_card_num UNSET MASKING POLICY;
ALTER TABLE col_masking MODIFY COLUMN credit_card_str UNSET MASKING POLICY;
