...
SELECT compression_codec FROM system.cnch_columns WHERE database = currentDatabase() AND table = 'alter_compression_codec' AND name = 'alter_column';
...
) ENGINE = CnchMergeTree PARTITION BY somedate ORDER BY id SETTINGS index_granularity = 2; -- min_bytes_for_wide_part = 0
...
CREATE TABLE store_of_hash_00804 (hash UInt64) ENGINE = CnchMergeTree ORDER BY hash;
