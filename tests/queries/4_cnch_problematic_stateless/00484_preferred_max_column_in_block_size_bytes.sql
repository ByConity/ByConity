...
create table test.tab (date Date, x UInt64, s FixedString(128)) engine = CnchMergeTree() PARTITION BY toYYYYMM(date) ORDER BY (date, x) SETTINGS index_granularity = 8192;
...
drop table if exists test.tab;
create table test.tab (date Date, x UInt64, s FixedString(128)) engine = CnchMergeTree() PARTITION BY toYYYYMM(date) ORDER BY (date, x) SETTINGS index_granularity = 32;
...
drop table if exists test.tab;
create table test.tab (date Date, x UInt64, s String) engine = CnchMergeTree() PARTITION BY toYYYYMM(date) ORDER BY (date, x) SETTINGS index_granularity = 8192;
