...
## Masking policy
$CLICKHOUSE_CLIENT --query="DROP TABLE IF EXISTS test.col_masking;"
...
$CLICKHOUSE_CLIENT --query="CREATE TABLE test.col_masking (name String, age Int64, credit_card_num String) ENGINE = CnchMergeTree() PARTITION BY name PRIMARY KEY name ORDER BY name;"
...
$CLICKHOUSE_CLIENT --query="ALTER TABLE test.col_masking MODIFY COLUMN credit_card_num SET MASKING POLICY credit_card_mask_test;"
...
$CLICKHOUSE_CLIENT --query="SELECT name FROM test.col_masking PREWHERE age = 12" 2>&1 | grep -o -m 1 "Code: 182"

# Mixing of commands in ALTER TABLE
# $CLICKHOUSE_CLIENT --query="ALTER TABLE test.col_masking MODIFY COLUMN credit_card_num UNSET MASKING POLICY, COMMENT COLUMN credit_card_num 'comment';" 2>&1 | grep -o -m 1 "Code: 495"

$CLICKHOUSE_CLIENT --query="ALTER TABLE test.col_masking MODIFY COLUMN credit_card_num UNSET MASKING POLICY;"
$CLICKHOUSE_CLIENT --query="DROP MASKING POLICY credit_card_mask_test;"
$CLICKHOUSE_CLIENT --query="DROP TABLE test.col_masking;"

## Row policy
$CLICKHOUSE_CLIENT --query="DROP TABLE IF EXISTS test.row_policy;"
$CLICKHOUSE_CLIENT --query="DROP TABLE IF EXISTS test.row_policy_mismatch;"
...
$CLICKHOUSE_CLIENT --query="DROP TABLE IF EXISTS test.rp_mapping_table;"
$CLICKHOUSE_CLIENT --query="CREATE TABLE test.rp_mapping_table (x UInt64) Engine = CnchMergeTree ORDER BY x;"
$CLICKHOUSE_CLIENT --query="CREATE TABLE test.row_policy (v UInt64, w UInt64) Engine = CnchMergeTree ORDER BY v;"
...
$CLICKHOUSE_CLIENT --query="CREATE TABLE test.row_policy_mismatch (v String, w UInt64) Engine = CnchMergeTree ORDER BY v ROW ACCESS POLICY rp on (v,w);" 2>&1 | grep -o -m 1 "Code: 53"

# Cannot create row policy with ambiguous input columns
$CLICKHOUSE_CLIENT --query="CREATE ROW ACCESS POLICY ambiguous_rp AS (x UInt64, y UInt64) -> CASE WHEN exists((SELECT x from test.rp_mapping_table WHERE x = x)) THEN 1 ELSE 0 END;" 2>&1 | grep -o -m 1 "Code: 49"

# Masking policy and Row policy cannot be applied to the same column at the same time
$CLICKHOUSE_CLIENT --query="ALTER TABLE test.row_policy SET ROW ACCESS POLICY rp ON (v, w);"
$CLICKHOUSE_CLIENT --query="ALTER TABLE test.row_policy MODIFY COLUMN v SET MASKING POLICY int_mask;" 2>&1 | grep -o -m 1 "Code: 6001"

# Row policy does not support PREWHERE
$CLICKHOUSE_CLIENT --query="SELECT v FROM test.row_policy PREWHERE v = 1" 2>&1 | grep -o -m 1 "Code: 182"

$CLICKHOUSE_CLIENT --query="ALTER TABLE test.row_policy UNSET ROW ACCESS POLICY;"
$CLICKHOUSE_CLIENT --query="DROP TABLE test.row_policy;"
$CLICKHOUSE_CLIENT --query="DROP TABLE test.rp_mapping_table;"
