...
INSERT INTO lambda_udf (a, b, c, d, e, f)
...
DROP FUNCTION IF EXISTS test_lambda_tuple;
DROP FUNCTION IF EXISTS test_2.test_lambda_array;

CREATE FUNCTION test_lambda_tuple AS (x) -> IF(empty(x.2), toString(x.1*10), concat(toString(x.1*10), '_', x.2));
...
SELECT
  ...
  f,
  test_lambda_tuple(e),
  test_2.test_lambda_array(f)
FROM
  lambda_udf
ORDER BY a;

DROP TABLE lambda_udf;
DROP FUNCTION test_lambda_tuple;
