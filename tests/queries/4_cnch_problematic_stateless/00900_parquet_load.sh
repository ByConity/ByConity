...
DATA_DIR=$CUR_DIR/../0_stateless/data_parquet

if [ ! -d $DATA_DIR ];then
  # echo "DATA_DIR ($DATA_DIR) is not exist, try cnch-verfication env path"
...
for NAME in `ls -1 $DATA_DIR/*.parquet | xargs -n 1 basename | sort`; do
    ...
    ${CLICKHOUSE_CLIENT} --query="CREATE TABLE test.parquet_load ($COLUMNS) ENGINE = CnchMergeTree order by tuple()"
