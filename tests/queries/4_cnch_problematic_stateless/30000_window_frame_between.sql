...
SELECT
  ...
  SUM(b) OVER (PARTITION BY a ORDER BY b ROWS BETWEEN CURRENT ROW AND 0 PRECEDING) as res16,
  SUM(b) OVER (PARTITION BY a ORDER BY b ROWS BETWEEN CURRENT ROW AND (2-2) PRECEDING) as res17,
  SUM(b) OVER (PARTITION BY a ORDER BY b ROWS BETWEEN (1+1) FOLLOWING AND CURRENT ROW) as res18,
