...
select MergeStreamStack(a) from( select a from (select cast([(1, 2), (2, 2)] as Array(Tuple(Int, Int))) as a )full outer join (select cast([(1, 2), (3, 4)] as Array(Tuple(Int, Int)) ) as a ) using a ); -- { serverError 49 }
