...
${CLICKHOUSE_CLIENT} --query="CREATE TABLE test.single_col_partition_key(x UInt32) ENGINE CnchMergeTree ORDER BY x PARTITION BY intDiv(x, 10)"
...
${CLICKHOUSE_CLIENT} --query="CREATE TABLE test.composite_partition_key(a UInt32, b UInt32, c UInt32) ENGINE CnchMergeTree ORDER BY c PARTITION BY (intDiv(a, 100), intDiv(b, 10), c)"
