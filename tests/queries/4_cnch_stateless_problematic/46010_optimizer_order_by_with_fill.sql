...
CREATE TABLE fill (date Date, str String, val Int) ENGINE = CnchMergeTree() partition by date order by str;
...
DROP TABLE IF EXISTS fill;
CREATE TABLE fill (a UInt32, b Int32) ENGINE = CnchMergeTree() partition by a order by b;
...
DROP TABLE IF EXISTS fill;
CREATE TABLE fill (number UInt32) ENGINE = CnchMergeTree() partition by number order by number;
