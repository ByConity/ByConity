...
select locate(0xD091, 0xD0B0D0B1D0B2 collate utf8mb4_bin);
select locate(0xD0B1, 0xD0B0D091D0B2 collate utf8mb4_bin);
...
--
select soundex(0xE99885E8A788E99A8FE697B6E69BB4E696B0E79A84E696B0E997BB);
select hex(soundex(0xE99885E8A788E99A8FE697B6E69BB4E696B0E79A84E696B0E997BB));
select soundex(0xD091D092D093);
select hex(soundex(0xD091D092D093));
...
create table t1 (a varchar(10) character set latin1, b int);
insert into t1 values ('a',1);
select concat(a, if(b>10, 0x78, 0x79)) from t1;
--error 1267
select concat(a, if(b>10, 0xC3A6, 0xC3AF)) from t1;
drop table t1;
...
select hex(0x616263FF);
--error ER_INVALID_CHARACTER_STRING
select hex(X'616263FF');
--error ER_INVALID_CHARACTER_STRING
select hex(B'001111111111');
--error ER_INVALID_CHARACTER_STRING
select (X'616263FF');
-- set sql_mode=default;
select hex(char(0xFF using utf8mb4));
...
UPDATE t2 SET utf8mb3_encoding= x'ea9da8' where u_decimal= 42856;
