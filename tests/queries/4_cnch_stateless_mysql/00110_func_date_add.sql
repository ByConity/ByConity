...
having mts < DATE_SUB(NOW(),INTERVAL 3 MONTH) order by visitor_id;
select visitor_id,max(ts) as mts from t1 group by visitor_id
having DATE_ADD(mts,INTERVAL 3 MONTH) < NOW() order by visitor_id;
drop table t1;
...
select '2007-01-01' + interval a day from t1 order by a;
select b + interval a day from t1 order by a;
drop table t1;
