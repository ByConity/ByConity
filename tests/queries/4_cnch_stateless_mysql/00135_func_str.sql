...
select 'hello','hello','''hello''','''h''e''l''l''o''','''hel''lo''';
select 'hello monty';
select length('\n\t\r\b\0\_\%\\');
select bit_length('\n\t\r\b\0\_\%\\');
select char_length('\n\t\r\b\0\_\%\\');
...
--error 1270
select FIELD('b','A','B');
select FIELD('1','3','2',1);

select POSITION('B' IN 'abcd');
select POSITION('B' IN 'abcd' COLLATE latin1_bin);
select POSITION('B' COLLATE latin1_bin IN 'abcd');
--error 1267
select POSITION('B' COLLATE latin1_general_ci IN 'abcd' COLLATE latin1_bin);
--error 1267
select POSITION('B' IN 'abcd');
...
-- # fix this:
--disable_parsing
select FIND_IN_SET('B','a,b,c,d' COLLATE latin1_bin);
select FIND_IN_SET('B' COLLATE latin1_bin,'a,b,c,d');
--enable_parsing

--error 1267
select FIND_IN_SET('B' COLLATE latin1_general_ci,'a,b,c,d' COLLATE latin1_bin);
--error 1267
select FIND_IN_SET('B', 'a,b,c,d');

select SUBSTRING_INDEX('abcdabcdabcd','d',2);
...
explain extended select FIND_IN_SET('B', 'a,b,c,d');
...
select concat('a',concat_ws('a', 0x2859, 'a' ,
