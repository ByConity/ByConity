...
-- There are test for bug which contain MyISAM specific tests
...
INSERT INTO t1 VALUES ('0xF2', '0xF2'), (X'2004',X'2004');
...
-- Check LPAD/RPAD
--
SELECT hex(LPAD(X'0420',10,X'0421'));
SELECT hex(LPAD(X'0420',10,X'0000042100000422'));
SELECT hex(LPAD(X'0420',10,X'000004210000042200000423'));
SELECT hex(LPAD(X'000004200000042100000422000004230000042400000425000004260000042700000428000004290000042A0000042B',10,X'000004210000042200000423'));

SELECT hex(RPAD(X'0420',10,X'0421'));
SELECT hex(RPAD(X'0420',10,X'0000042100000422'));
SELECT hex(RPAD(X'0420',10,X'000004210000042200000423'));
SELECT hex(RPAD(X'000004200000042100000422000004230000042400000425000004260000042700000428000004290000042A0000042B',10,X'000004210000042200000423'));

CREATE TABLE t1 SELECT 
LPAD(X'0420',10,X'0421') l,
RPAD(X'0420',10,X'0421') r;
...
SELECT * FROM t1 WHERE word LIKE x'0000006300000025'; # 'c%'
SELECT * FROM t1 WHERE word LIKE x'00000063000000610000005F'; # 'ca_'
DROP TABLE t1;
...
select insert(0x000000610000006200000063,10,2,0x000000640000006500000066);
select insert(0x000000610000006200000063,1,2,0x000000640000006500000066);
...
--
select hex(substr(0x000000e4000000e500000068,1));
select hex(substr(0x000000e4000000e500000068,2));
select hex(substr(0x000000e4000000e500000068,3));
select hex(substr(0x000000e4000000e500000068,-1));
select hex(substr(0x000000e4000000e500000068,-2));
select hex(substr(0x000000e4000000e500000068,-3));
...
-- Cyrillic A, BE, VE
select hex(soundex(0x000004100000041100000412));
-- Make sure that 'U+00BF INVERTED QUESTION MARK' is not considered as letter
select hex(soundex(0x000000BF000000C0));
-- set names latin1;
...
--
create table t1 (a varchar(10) character set utf32);
insert into t1 values (0x0010FFFF);
--error ER_INVALID_CHARACTER_STRING
insert into t1 values (0x00110000);
--error ER_INVALID_CHARACTER_STRING
insert into t1 values (0x00110101);
--error ER_INVALID_CHARACTER_STRING
insert into t1 values (0x01000101);
--error ER_INVALID_CHARACTER_STRING
insert into t1 values (0x11000101);
select hex(a) from t1;
drop table t1;
...
select hex(concat(0x0410 collate utf32_general_ci, 0x61));
...
select hex(weight_string(0x10000));
select hex(weight_string(0x10001));
