...
select A, B, bitOr(A, B), bitXor(A, B), bitAnd(A, B)
  from t1_30237_bool where C is null order by A, B;

--echo Testing that OR is associative 
select A, B, C, bitOr(bitOr(A, B), C), bitOr(A, bitOr(B, C)), bitOr(bitOr(A, B), C)
...
select A, B, C, bitXor(bitXor(A, B), C), bitXor(A, bitXor(B, C)), bitXor(bitXor(A, B), C)
  from t1_30237_bool order by A, B, C;

select count(*) from t1_30237_bool
  where bitXor(bitXor(A, B), C) != bitXor(A, bitXor(B, C));
...
select count(*) from t1_30237_bool
  where bitAnd(bitAnd(A, B), C) != bitAnd(A, bitAnd(B, C));
