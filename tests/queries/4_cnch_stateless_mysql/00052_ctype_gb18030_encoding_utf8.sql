...
CREATE TABLE `表格` (`字段一` CHAR(1));
CREATE TABLE `模式` (`列列列` CHAR(1));
CREATE TABLE `隞嗷㐁` (`列㐄㐅` CHAR(1));
-- Table should not be created because table names exist
CREATE TABLE IF NOT EXISTS `表格`(`字段一` CHAR(1));
CREATE TABLE IF NOT EXISTS `模式` (`列列列` CHAR(1));
CREATE TABLE IF NOT EXISTS `隞嗷㐁` (`列㐄㐅` CHAR(1));
-- Tables should be created
CREATE TABLE IF NOT EXISTS `㐅㐅㐅` (`㐄㐄㐄` CHAR(1));
CREATE TEMPORARY TABLE `㐇㐈㐉` (`㐐㐐㐐` CHAR(1));
...
CREATE TABLE `表格` (`字段一` CHAR(5));
CREATE TABLE `隞嗷㐁` (`㐂㐃㐄` CHAR(5));
...
CREATE TABLE `表二` (c CHAR(5), v VARCHAR(10), t TEXT);
...
--echo 'Test for ENUM'
CREATE TABLE `表二` (e ENUM('口', '日', '目', '田', '晶'), INDEX(e));
...
CREATE TABLE `表一` (c1 CHAR(20), INDEX(c1));
...
CREATE TABLE t1a (c CHAR(1) PRIMARY KEY);
CREATE TABLE t1b (c CHAR(1) PRIMARY KEY);

CREATE TABLE t2a (c CHAR(1) PRIMARY KEY);
CREATE TABLE t2b (c CHAR(1) PRIMARY KEY);
...
CREATE TABLE t3 (`䑃` CHAR(1), `㐐` CHAR(1), `段` CHAR(1));
CREATE TABLE t4 (c1 CHAR(3));
...
CREATE TABLE `表格一` (id INT PRIMARY KEY, c CHAR(5), b BINARY(50), v VARBINARY(100));
