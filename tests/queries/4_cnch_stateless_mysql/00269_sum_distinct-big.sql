...
SELECT AVG(DISTINCT id) FROM t1 GROUP BY id % 13 ORDER BY id;
SELECT SUM(DISTINCT id)/COUNT(DISTINCT id) FROM t1 GROUP BY id % 13 ORDER BY id;
...
SELECT SUM(DISTINCT id) sm FROM t1 group by id % 13 ORDER BY id;
