...
select from_days(to_days('19960101')),to_days(19960201)-to_days('19960101'),to_days(date_add(curdate(), interval 1 day))-to_days(curdate()),weekday('1997-11-29');
select period_add('199602',-12),period_diff(199505,'199404') ;

select now()-now(),weekday(curdate())-weekday(now()),unix_timestamp()-unix_timestamp(now());
select from_unixtime(unix_timestamp('1994-03-02 10:11:12')),from_unixtime(unix_timestamp('1994-03-02 10:11:12'),'%Y-%m-%d %h:%i:%s'),from_unixtime(unix_timestamp('1994-03-02 10:11:12'))+0;
select sec_to_time(9001),sec_to_time(9001)+0,time_to_sec('15:12:22'),
  sec_to_time(time_to_sec('0:30:47')/6.21);
select sec_to_time(time_to_sec('12:59:59'));
select now()-curdate()*1000000-curtime();
select strcmp(now(),concat(current_date(),' ',current_time()));
select strcmp(localtime(),concat(current_date(),' ',current_time()));
select strcmp(localtimestamp(),concat(current_date(),' ',current_time()));
select date_format('1997-01-02 03:04:05', '%M %W %D %Y %y %m %d %h %i %s %w');
select date_format('1997-01-02', concat('%M %W %D ','%Y %y %m %d %h %i %s %w'));
select dayofmonth('1997-01-02'),dayofmonth(19970323);
select month('1997-01-02'),year('1998-02-03'),dayofyear('1997-12-31');
...
select date_format(19980121,'%H|%I|%k|%l|%i|%p|%r|%S|%T| %M|%W|%D|%Y|%y|%a|%b|%j|%m|%d|%h|%s|%w');
...
SELECT '1900-01-01 00:00:00' + INTERVAL bitShiftLeft(1, 38) SECOND;
SELECT '1900-01-01 00:00:00' + INTERVAL bitShiftLeft(1, 33) MINUTE;
SELECT '1900-01-01 00:00:00' + INTERVAL bitShiftLeft(1, 30) HOUR;
...
select unix_timestamp(FROM_UNIXTIME(1));
...
select last_day('2000-02-05') as f1, last_day('2002-12-31') as f2,
       last_day('2003-03-31') as f3, last_day('2003-04-01') as f4,
       last_day('2001-01-01 01:01:01') as f5, last_day(NULL),
       last_day('2001-02-12');

create table t1 select last_day('2000-02-05') as a,
                from_days(to_days('19960101')) as b;
describe t1;
select * from t1;
drop table t1;
...
explain select period_add('199602',-12),period_diff(199505,'199404'),from_days(to_days('19960101')),dayofmonth('1997-01-02'), month('1997-01-02'), monthname('1972-03-04'),dayofyear('0000-00-00'),HOUR('1997-03-03 23:03:22'),MINUTE('23:03:22'),SECOND(20230322),QUARTER(19980303),WEEK('1998-03-03'),yearweek('2000-01-01',1),week(19950101,1),year('1998-02-03'),weekday(curdate())-weekday(now()),dayname('1962-03-03'),unix_timestamp(),sec_to_time(time_to_sec('01:30:47')/6.21),curtime(),utc_time(),curdate(),utc_date(),utc_timestamp(),date_format('1997-01-02 03:04:05', '%M %W %D %Y %y %m %d %h %i %s %w'),from_unixtime(unix_timestamp('1994-03-02 10:11:12')),'1997-12-31 23:59:59' + INTERVAL 1 SECOND,'1998-01-01 00:00:00' - INTERVAL 1 SECOND,INTERVAL 1 DAY + '1997-12-31', extract(YEAR FROM '1999-01-02 10:11:12'),date_add('1997-12-31 23:59:59',INTERVAL 1 SECOND);
...
SELECT count(*) FROM t1 WHERE d>FROM_DAYS(TO_DAYS('2007-08-01 12:22:49')) AND d<=FROM_DAYS(TO_DAYS('2007-08-01 12:22:49')+1);
DROP TABLE t1;
...
SELECT 1 FROM t1 ORDER BY makedate(a,a);
DROP TABLE t1;
...
SELECT 1 FROM t1 GROUP BY UNIX_TIMESTAMP(a);
