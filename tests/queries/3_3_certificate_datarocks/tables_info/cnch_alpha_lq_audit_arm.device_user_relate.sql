...
CREATE TABLE IF NOT EXISTS cnch_alpha_lq_audit_arm.device_user_relate (`device_id` String, `p_date` Date, `user_id` Int64, `mc` String, `udid` String) ENGINE = MergeTree() PARTITION BY p_date ORDER BY (device_id, intHash64(user_id)) SAMPLE BY intHash64(user_id) SETTINGS index_granularity = 8192;
