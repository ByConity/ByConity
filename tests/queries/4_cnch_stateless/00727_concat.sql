...
CREATE TABLE concat_saf_test(x SimpleAggregateFunction(max, Int32)) ENGINE=CnchMergeTree ORDER BY tuple();
...
CREATE TABLE concat_nested_test(attrs Nested(k String, v String)) ENGINE = CnchMergeTree ORDER BY tuple();
