...
create table if not exists test.test_bitmap_index_single (date Date, id Int32, int_vid Array(Int32) BitmapIndex, float_vid Array(Float32) BitmapIndex, ext Array(Int32)) engine = CnchMergeTree partition by date order by id settings min_bytes_for_wide_part = 0, enable_late_materialize = 1;
