...
select IPv4StringToNum('127.0.0.1') == (0x7f000001), IPv4NumToString(0x7f000001) == '127.0.0.1';
select IPv4StringToNum(materialize('127.0.0.1')) == (materialize(0x7f000001)), IPv4NumToString(materialize(0x7f000001)) == materialize('127.0.0.1');
...
select IPv6NumToString(IPv6StringToNum('::ffff:127.0.0.1')) == '::ffff:127.0.0.1';
select IPv6NumToString(IPv6StringToNum(materialize('::ffff:127.0.0.1'))) == materialize('::ffff:127.0.0.1');
...
CREATE TABLE addresses(addr UInt32) ENGINE = CnchMergeTree ORDER BY addr;
...
DROP TABLE IF EXISTS addresses;
CREATE TABLE addresses(addr String) ENGINE = CnchMergeTree ORDER BY addr;
