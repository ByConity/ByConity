...
select 50 = length(format((select arrayStringConcat(arrayMap(x ->'{', range(100)))), '')) SETTINGS enable_optimizer=0; -- early constant folding
select 100 = length(format(concat((select arrayStringConcat(arrayMap(x ->'}', range(100)))), (select arrayStringConcat(arrayMap(x ->'{', range(100))))), '')) SETTINGS enable_optimizer=0; -- early constant folding
...
select 50 = length(format((select arrayStringConcat(arrayMap(x ->'{', range(101)))), '')) SETTINGS enable_optimizer=0; -- { serverError 36 }  early constant folding
