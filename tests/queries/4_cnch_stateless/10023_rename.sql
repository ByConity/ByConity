...
SELECT type, table, status, expected_status FROM system.dm_bg_jobs WHERE database = currentDatabase() AND table = 'rename1' AND type IN ('PartGCThread', 'MergeMutateThread') ORDER BY type;
SELECT type, table, status FROM system.bg_threads WHERE database = currentDatabase() AND table = 'rename1' AND type IN ('PartGCThread', 'MergeMutateThread') ORDER BY type;
...
SELECT type, table, status, expected_status FROM system.dm_bg_jobs WHERE database = currentDatabase() AND table = 'rename11' AND type IN ('PartGCThread', 'MergeMutateThread') ORDER BY type;
SELECT type, table, status FROM system.bg_threads WHERE database = currentDatabase() AND table = 'rename11' AND type IN ('PartGCThread', 'MergeMutateThread') ORDER BY type;
