...
CLICKHOUSE_CLIENT_MYSQL="$CLICKHOUSE_CLIENT --dialect_type=MYSQL --join_use_nulls=0"
...
$CLICKHOUSE_CLIENT_MYSQL -nmq \
  "set dialect_type='CLICKHOUSE', join_use_nulls=1; \
   ...
   where name in ('dialect_type', 'join_use_nulls') \
   order by name \
   settings dialect_type='MYSQL', join_use_nulls=0;"
...
$CLICKHOUSE_CLIENT_CK -nmq \
  "set dialect_type='MYSQL', join_use_nulls=0; \
   select name, value from system.settings \
   where name in ('dialect_type', 'join_use_nulls') \
   order by name;"
