...
SELECT caseWithExpr([NULL], -588755.149, []); -- { serverError 42 }
SELECT sequenceCount((CAST((( SELECT NULL ) AS rg, ( SELECT ( SELECT [], '<e', caseWithExpr([], [NULL], -588755.149, -1), retention(addWeeks((CAST((-7644612.39732) AS DateTime)), -23578040.02833), (CAST(('1970-01-01 00:00:00') AS DateTime)), (CAST(([010977.08]) AS String))), emptyArrayToSingle('') ) , '\0', toUInt64([], 't3hw@'), '\0', toStartOfQuarter(-4230.1872, []) ) ) AS Date))); -- { serverError 43 }
