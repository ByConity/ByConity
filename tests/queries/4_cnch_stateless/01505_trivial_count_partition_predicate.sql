...
create table test1(p DateTime, k int) engine CnchMergeTree partition by toDate(p) order by k settings index_granularity = 1;
...
create table test_tuple(p DateTime, i int, j int) engine CnchMergeTree partition by (toDate(p), i) order by j settings index_granularity = 1;
...
create table test_two_args(i int, j int, k int) engine CnchMergeTree partition by i + j order by k settings index_granularity = 1;
