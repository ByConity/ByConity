...
CREATE TABLE bucket (name String, age Int64) ENGINE = CnchAggregatingMergeTree() PARTITION BY name CLUSTER BY age INTO 1 BUCKETS ORDER BY name;
CREATE TABLE bucket2 (name String, age Int64) ENGINE = CnchAggregatingMergeTree() PARTITION BY name CLUSTER BY age INTO 1 BUCKETS ORDER BY name;
CREATE TABLE bucket3 (name String, age Int64) ENGINE = CnchAggregatingMergeTree() PARTITION BY name CLUSTER BY age INTO 1 BUCKETS ORDER BY name;
CREATE TABLE normal (name String, age Int64) ENGINE = CnchAggregatingMergeTree() PARTITION BY name ORDER BY name;
CREATE TABLE bucket_with_split_number (name String, age Int64) ENGINE = CnchAggregatingMergeTree() PARTITION BY name CLUSTER BY (name, age) INTO 1 BUCKETS SPLIT_NUMBER 60 ORDER BY name;
CREATE TABLE bucket_with_split_number_n_range (name String, age Int64) ENGINE = CnchAggregatingMergeTree() PARTITION BY name CLUSTER BY (name, age) INTO 1 BUCKETS SPLIT_NUMBER 60 WITH_RANGE ORDER BY name;
CREATE TABLE dts_bucket_with_split_number_n_range (name String, age Int64) ENGINE = CnchAggregatingMergeTree() PARTITION BY name CLUSTER BY (name) INTO 1 BUCKETS SPLIT_NUMBER 60 WITH_RANGE ORDER BY name;
CREATE TABLE test_optimize (`id` UInt32, `code` UInt32, `record` String) ENGINE = CnchAggregatingMergeTree PARTITION BY id CLUSTER BY code INTO 4 BUCKETS ORDER BY code;
CREATE TABLE test_optimize_with_date_column (`id` UInt32, `code` UInt32, `record` Date) ENGINE = CnchAggregatingMergeTree PARTITION BY id CLUSTER BY record INTO 4 BUCKETS ORDER BY record;
