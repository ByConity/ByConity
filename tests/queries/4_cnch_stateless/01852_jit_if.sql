...
CREATE TABLE test_jit_nonnull (value UInt8) ENGINE = CnchMergeTree() order by tuple();
...
CREATE TABLE test_jit_nullable (value Nullable(UInt8)) ENGINE = CnchMergeTree() order by tuple();
