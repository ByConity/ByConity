...
echo `${CLICKHOUSE_CLIENT} --query "SELECT (kv_commit_time > 0) FROM system.cnch_parts WHERE (database = currentDatabase()) AND (table = 'attach_bug2') AND active;"`
...
echo `${CLICKHOUSE_CLIENT} --query "SELECT (kv_commit_time > 0) FROM system.cnch_parts WHERE (database = currentDatabase()) AND (table = 'attach_bug2')"`
