...
CREATE TABLE nse_lhs (key1 int, value Nullable(UInt8)) ENGINE=CnchMergeTree() order by key1;
CREATE TABLE nse_rhs (key2 int, value Nullable(UInt8)) ENGINE=CnchMergeTree() order by key2;
...
SELECT nse_lhs.key1, nse_lhs.value FROM nse_lhs JOIN nse_rhs USING(key1, key2);
