...
CREATE TABLE nse_lhs (key1 int, value Nullable(UInt8)) ENGINE=CnchMergeTree() order by key;
CREATE TABLE nse_rhs (key2 int, value Nullable(UInt8)) ENGINE=CnchMergeTree() order by key;
