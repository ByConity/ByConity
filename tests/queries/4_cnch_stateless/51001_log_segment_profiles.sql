...
SYSTEM FLUSH LOGS;
WITH ( SELECT query_id FROM system.query_log WHERE current_database = currentDatabase() AND Settings['enable_optimizer']='1' AND Settings['log_segment_profiles']='1' AND Settings['log_queries']='1' AND query like 'SELECT 1%' ORDER BY event_time desc LIMIT 1) AS query_id_ SELECT length(segment_profiles) > 0 FROM system.query_log WHERE query_id = query_id_ UNION SELECT length(segment_profiles) > 0 FROM cnch_system.cnch_query_log WHERE query_id = query_id_;
...
SYSTEM FLUSH LOGS;
WITH ( SELECT query_id FROM system.query_log WHERE current_database = currentDatabase() AND Settings['enable_optimizer']='1' AND Settings['log_segment_profiles']='1' AND Settings['log_queries']='1' AND query like '%user_id < 102%' ORDER BY event_time desc LIMIT 1) AS query_id_ SELECT length(segment_profiles) > 0 FROM system.query_log WHERE query_id = query_id_ UNION SELECT length(segment_profiles) > 0 FROM cnch('vw_default', system.query_log) WHERE startsWith(query_id, query_id_);
