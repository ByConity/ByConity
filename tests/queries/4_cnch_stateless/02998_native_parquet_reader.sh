...
QUERY="SELECT * from file('$PAR_PATH', '$CH_SCHEMA', Parquet)"
$CLICKHOUSE_LOCAL --max_threads 1 --max_download_threads 1 --input_format_parquet_use_native_reader 1 --query "$QUERY"

PAR_PATH="$CURDIR"/data_parquet/list_columns.parquet
CH_SCHEMA="\
    int64_list Array(Nullable(Int64)), utf8_list Array(Nullable(String))"
QUERY="SELECT * from file('$PAR_PATH', '$CH_SCHEMA', 'Parquet')"
$CLICKHOUSE_LOCAL --max_threads 1 --max_download_threads 1 --input_format_parquet_use_native_reader 1 --query "$QUERY"

PAR_PATH="$CURDIR"/data_parquet/map_data.snappy.parquet
CH_SCHEMA="\
    id Int32, info Map(String, Int32)"
QUERY="SELECT * from file('$PAR_PATH', '$CH_SCHEMA', 'Parquet')"
$CLICKHOUSE_LOCAL --max_threads 1 --max_download_threads 1 --input_format_parquet_use_native_reader 1 --query "$QUERY"

PAR_PATH="$CURDIR"/data_parquet/alltypes_dictionary.parquet
CH_SCHEMA="\
    id Nullable(Int32), \
    bool_col Nullable(UInt8), \
    tinyint_col Nullable(Int32), \
    smallint_col Nullable(Int32), \
    ...
    timestamp_col Nullable(Int64)"
QUERY="SELECT * from file('$PAR_PATH', '$CH_SCHEMA', 'Parquet')"
$CLICKHOUSE_LOCAL --max_threads 1 --max_download_threads 1 --input_format_parquet_use_native_reader 1 --query "$QUERY"

PAR_PATH="$CURDIR"/data_parquet/alltypes_plain.parquet
CH_SCHEMA="\
    id Nullable(Int32), \
    bool_col Nullable(UInt8), \
    tinyint_col Nullable(Int32), \
