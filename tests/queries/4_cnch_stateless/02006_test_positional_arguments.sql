...
create table test(x1 Int, x2 Int, x3 Int) engine = CnchMergeTree() order by tuple();
...
explain syntax select x3, x2, x1 from test order by 1 settings enable_optimizer=0;
explain syntax select x3 + 1, x2, x1 from test order by 1 settings enable_optimizer=0;
explain syntax select x3, x3 - x2, x2, x1 from test order by 2 settings enable_optimizer=0;
explain syntax select x3, if(x3 > 10, x3, plus(x1, x2)), x1 + x2 from test order by 2 settings enable_optimizer=0;
explain syntax select max(x1), x2 from test group by 2 order by 1, 2 settings enable_optimizer=0;
explain syntax select 1 + greatest(x1, 1), x2 from test group by 1, 2 settings enable_optimizer=0;

select max(x1), x2 from test group by 1, 2 settings enable_optimizer=0; -- { serverError 43 }
select 1 + max(x1), x2 from test group by 1, 2 settings enable_optimizer=0; -- { serverError 43 }
...
select total from (select first_col, count(second_col) as total from tp2 group by 1) order by total;
