...
SELECT engine FROM system.tables WHERE database = currentDatabase() and name = 'engine_conv';

CREATE TABLE as_engine_conv Engine=MergeTree PARTITION BY d ORDER BY t AS SELECT * FROM engine_conv; 
SELECT engine FROM system.tables WHERE database = currentDatabase() and name = 'as_engine_conv';
...
SELECT engine FROM system.tables WHERE database = currentDatabase() and name = 'engine_aggregating_conv';
