...
cat "$DATA_FILE"  | ${CLICKHOUSE_CLIENT} -q "INSERT INTO test.test_parquet_date_out_of_range32 format Parquet"  2>&1 | sed 's/Exception/Ex---tion/'
...
cat "$DATA_FILE"  | ${CLICKHOUSE_CLIENT} -q "INSERT INTO test.test_parquet_date_out_of_range format Parquet"  2>&1 | sed 's/Exception/Ex---tion/'
