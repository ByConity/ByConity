...
select '-';
select * from (select dummy as val from system.one) s1 any left join (select dummy as val from system.one) s2 on val + 0 = val * 1 settings enable_optimizer = 0; -- { serverError 352 }
