...
CREATE TABLE pk (d Date DEFAULT '2000-01-01', x DateTime, y UInt64, z UInt64) ENGINE = CnchMergeTree() PARTITION BY toYYYYMM(d) ORDER BY (toStartOfMinute(x), y, z) SETTINGS index_granularity=1;
