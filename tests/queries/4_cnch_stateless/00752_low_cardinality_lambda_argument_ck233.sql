...
create table lc_lambda (arr Array(LowCardinality(UInt64))) ENGINE = CnchMergeTree() ORDER BY tuple();
...
CREATE TABLE test_array(resources_host Array(LowCardinality(String))) ENGINE = CnchMergeTree() ORDER BY (resources_host);
