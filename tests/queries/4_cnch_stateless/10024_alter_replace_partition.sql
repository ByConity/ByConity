...
select count() from system.cnch_parts where database = currentDatabase(1) and table = 'replace_dst' and active;

alter table replace_dst replace partition id '20190101' from replace_src;
select count() from system.cnch_parts where database = currentDatabase(1) and table = 'replace_dst' and active;
select count() from system.cnch_parts where database = currentDatabase(1) and table = 'replace_src' and active;
