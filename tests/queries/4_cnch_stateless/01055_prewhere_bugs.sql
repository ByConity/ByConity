...
CREATE TABLE test_prewhere_default_column (APIKey UInt8, SessionType UInt8) ENGINE = CnchMergeTree() PARTITION BY APIKey ORDER BY tuple();
...
CREATE TABLE test_prewhere_column_type (`a` LowCardinality(String), `x` Nullable(Int32)) ENGINE = CnchMergeTree ORDER BY tuple();
