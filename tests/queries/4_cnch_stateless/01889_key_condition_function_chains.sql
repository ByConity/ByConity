...
create table tab (t DateTime) engine = CnchMergeTree order by toStartOfDay(t);
...
drop table if exists tab;
create table tab (t DateTime) engine = CnchMergeTree order by toStartOfDay(t + 1);
insert into tab values ('2020-02-02 01:01:01');
...
drop table if exists tab;
create table tab (x Int32, y Int32) engine = CnchMergeTree partition by x + y order by tuple();
...
drop table if exists tab;
create table tab (x Int32, y Int32) engine = CnchMergeTree partition by ((x + y) + 1) * 2 order by tuple();
insert into tab values (1, 1), (2, 2);
