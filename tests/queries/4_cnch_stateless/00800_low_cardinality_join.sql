...
select '-';
select * from (select dummy as val from system.one) any left join (select dummy as val from system.one) on val + 0 = val * 1 settings enable_optimizer = 0; -- { serverError 352 }
