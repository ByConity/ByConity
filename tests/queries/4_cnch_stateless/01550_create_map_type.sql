...
create table table_map (a Map(String, String) KV) engine = CnchMergeTree order by tuple();
...
drop table if exists table_map;
create table table_map (a Map(String, UInt64) KV) engine = CnchMergeTree order by a;
...
-- MergeTree Engine
drop table if exists table_map;
create table table_map (a Map(String, String) KV, b String) engine = CnchMergeTree order by a;
...
create table table_map (d DATE, m Map(Int8, UInt256) KV) ENGINE = CnchMergeTree order by d;
...
create table table_map (d DATE, m Map(Int8, Int8) KV) ENGINE = CnchMergeTree order by d;
insert into table_map values ('2020-01-01', map(1, 0, 2, -1));
select * from table_map;
drop table table_map;
...
-- Unsigned Int type
drop table if exists table_map;
create table table_map(a Map(UInt8, UInt64) KV, b UInt8) Engine = CnchMergeTree order by b;
...
-- Array Type
drop table if exists table_map;
create table table_map(a Map(String, Array(UInt8)) KV) Engine = CnchMergeTree order by a;
...
CREATE TABLE table_map (n UInt32, m Map(String, Int) KV)
ENGINE = CnchMergeTree ORDER BY n SETTINGS min_bytes_for_wide_part = 0;
