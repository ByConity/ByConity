...
# byte map unsupported value types
${CLICKHOUSE_CLIENT} --multiquery --testmode <<EOF
...
CREATE TABLE 00745_merge_tree_map_data_type (a Int32, b Map(String, Array(Tuple(String, String))) KV) ENGINE = CnchMergeTree ORDER BY a;
...
${CLICKHOUSE_CLIENT} --query "CREATE TABLE 00745_merge_tree_map_merge3 (n UInt8, \`m[a]\` Map(String, String), \`m{a}\` Map(String, String)) ENGINE = CnchMergeTree ORDER BY tuple();"
...
${CLICKHOUSE_CLIENT} --query "CREATE TABLE 00745_merge_tree_map_merge5 (empty_map Map(String, String)) ENGINE = CnchMergeTree ORDER BY tuple();"
${CLICKHOUSE_CLIENT} --query "CREATE TABLE 00745_merge_tree_map_merge6 (string_map Map(String, String)) ENGINE = CnchMergeTree ORDER BY tuple();"
# byte map with horizontal merges
${CLICKHOUSE_CLIENT} --query "CREATE TABLE 00745_merge_tree_map_merge7 (n UInt8, m Map(String, String)) ENGINE = CnchMergeTree ORDER BY n SETTINGS enable_vertical_merge_algorithm = 0;"
...
${CLICKHOUSE_CLIENT} --query "SYSTEM START MERGES 00745_merge_tree_map_merge7;"
...
do
    ...
    ${CLICKHOUSE_CLIENT} --query "INSERT INTO 00745_merge_tree_map_merge7 VALUES ($i, {'key1': 'v1', 'key2': 'v2'});"
done
...
${CLICKHOUSE_CLIENT} --query "INSERT INTO 00745_merge_tree_map_merge7 VALUES (4, {});"
...
${CLICKHOUSE_CLIENT} --multiquery <<EOF
    ...
    OPTIMIZE TABLE 00745_merge_tree_map_merge7 FINAL SETTINGS mutations_sync=1;
EOF
...
${CLICKHOUSE_CLIENT} --query "SELECT count(1) FROM system.cnch_parts WHERE database = currentDatabase() AND table = '00745_merge_tree_map_merge7' AND active;"
...
${CLICKHOUSE_CLIENT} --query "SELECT * FROM 00745_merge_tree_map_merge7 ORDER BY n;"
...
${CLICKHOUSE_CLIENT} --query "DROP TABLE 00745_merge_tree_map_merge7;"
