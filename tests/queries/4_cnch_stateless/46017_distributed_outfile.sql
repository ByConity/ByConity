...
CREATE TABLE test.people (`name` String, `country` String) ENGINE = CnchMergeTree() PARTITION BY `country` PRIMARY KEY `name` ORDER BY `name` SETTINGS index_granularity = 8192;

INSERT INTO test.people values ('Jack', 'sg');
...
-- agg query
SELECT count(distinct name) from test.people
INTO OUTFILE 'hdfs:///user/clickhouse/' FORMAT Parquet COMPRESSION 'none'
