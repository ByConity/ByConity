...
cat "$DATA_FILE"  | ${CLICKHOUSE_CLIENT} -q "INSERT INTO test.test_orc_date_out_of_range_32 format ORC SETTINGS input_format_orc_use_fast_decoder = 0"  2>&1 | sed 's/Exception/Ex---tion/'
...
cat "$DATA_FILE"  | ${CLICKHOUSE_CLIENT} -q "INSERT INTO test.test_orc_date_out_of_range format ORC SETTINGS input_format_orc_use_fast_decoder = 0"  2>&1 | sed 's/Exception/Ex---tion/'
