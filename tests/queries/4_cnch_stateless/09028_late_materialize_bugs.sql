...
CREATE TABLE test_prewhere_default_column (APIKey UInt8, SessionType UInt8) ENGINE = CnchMergeTree() PARTITION BY APIKey ORDER BY tuple() SETTINGS enable_late_materialize = 1;
...
SELECT OperatingSystem FROM test_prewhere_default_column WHERE SessionType = 42;


CREATE TABLE test_prewhere_column_type (`a` LowCardinality(String), `x` Nullable(Int32)) ENGINE = CnchMergeTree ORDER BY tuple() SETTINGS enable_late_materialize = 1;
INSERT INTO test_prewhere_column_type VALUES ('', 2);

SELECT a, y FROM test_prewhere_column_type where (x = 2) AS y;
