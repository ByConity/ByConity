...
CREATE TABLE vec1 (id UInt64, v Array(UInt8)) ENGINE = CnchMergeTree() ORDER BY tuple();
CREATE TABLE vec2 (id UInt64, v Array(Int64)) ENGINE = CnchMergeTree() ORDER BY tuple();
CREATE TABLE vec2f (id UInt64, v Array(Float32)) ENGINE = CnchMergeTree() ORDER BY tuple();
CREATE TABLE vec2d (id UInt64, v Array(Float64)) ENGINE = CnchMergeTree() ORDER BY tuple();
...
WHERE length(v1.v) == length(v2.v) ORDER BY v1.id;
