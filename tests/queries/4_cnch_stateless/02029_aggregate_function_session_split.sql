...
SELECT * FROM (SELECT arrayJoin(ifnull(sessionSplitR2(1800, 86400, 0, 0)(server_time, event, time, start_time, end_time, url, referer_type), [])) AS pt FROM test_session GROUP BY hash_uid ORDER BY hash_uid) ORDER BY
pt.3;
...
SELECT * FROM (SELECT arrayJoin(ifnull(sessionSplitR2(1800, 86400, 0, 1)(server_time, event, time, start_time, end_time, url, referer_type), [])) AS pt FROM test_session GROUP BY hash_uid ORDER BY hash_uid) ORDER BY
pt.3;
