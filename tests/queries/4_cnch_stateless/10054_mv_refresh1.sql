...
insert into table mv1.source1(app_id, server_time, event_name, uid, cost, duration, event_date) values (2, 1642149961 , 'send', 100002, 2, 2, '2022-06-15');
insert into table mv1.source1(app_id, server_time, event_name, uid, cost, duration, event_date) values (1, 1642149961, 'show', 100001, 1, 1, '2022-06-17');
insert into table mv1.source1(app_id, server_time, event_name, uid, cost, duration, event_date) values (2, 1642149961 , 'send', 100002, 2, 2, '2022-06-18');
select count(*)==4 as res from mv1.source1;
...
refresh materialized view mv1.mv1 where toDate(event_date) < '2022-06-14';
...
refresh materialized view mv1.mv1 where toDate(event_date) < '2022-06-16';
select app_id, event_name, event_date, sumMerge(sum_cost), maxMerge(max_duration) from mv1.mv1 group by app_id, event_name, event_date order by app_id;

refresh materialized view mv1.mv1 where toDate(event_date) < '2022-06-19' settings max_threads_to_refresh_by_partition=2;
select app_id, event_name, event_date, sumMerge(sum_cost), maxMerge(max_duration) from mv1.mv1 group by app_id, event_name, event_date order by app_id, event_name, event_date;
