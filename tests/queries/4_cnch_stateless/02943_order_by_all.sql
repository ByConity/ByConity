...
-- SET allow_experimental_analyzer = 0;
SELECT format('{}', a) AS all FROM order_by_all ORDER BY all;  -- { serverError UNEXPECTED_EXPRESSION }
SELECT format('{}', a) AS all FROM order_by_all ORDER BY ALL;  -- { serverError UNEXPECTED_EXPRESSION }
SELECT format('{}', a) AS all FROM order_by_all ORDER BY all SETTINGS enable_order_by_all = false;
