...
SELECT a FROM at_dc ORDER BY a SETTINGS disk_cache_mode = 'FORCE_CHECKSUMS_DISK_CACHE';

ALTER DISK CACHE DROP TABLE test.at_dc PARTITION 1 SYNC;
SELECT a FROM at_dc WHERE p = 1 ORDER BY a SETTINGS disk_cache_mode = 'FORCE_CHECKSUMS_DISK_CACHE'; -- { serverError 5046 } --
...
SELECT a FROM test_bucket_preload ORDER BY a SETTINGS disk_cache_mode = 'FORCE_CHECKSUMS_DISK_CACHE';

ALTER DISK CACHE DROP TABLE test.test_bucket_preload SYNC SETTINGS drop_vw_disk_cache = 1;
SELECT a FROM test_bucket_preload WHERE p = 1 ORDER BY a SETTINGS disk_cache_mode = 'FORCE_CHECKSUMS_DISK_CACHE'; -- { serverError 5046 } --
