...
$CLICKHOUSE_CLIENT --query "CREATE USER user_test_02184 IDENTIFIED WITH no_password;"
$CLICKHOUSE_CLIENT --query "REVOKE ALL ON *.* FROM user_test_02184;"

$CLICKHOUSE_CLIENT --query "GRANT CREATE ON *.* TO user_test_02184;"
NEW_USER="user_test_02184"
$CLICKHOUSE_CLIENT --query "CREATE TABLE IF NOT EXISTS int_table (a int) engine = CnchMergeTree ORDER BY a;"
$CLICKHOUSE_CLIENT  --user=$NEW_USER --query "SELECT * FROM int_table;" 2>&1| grep -Fo "Not enough privileges" | uniq

$CLICKHOUSE_CLIENT  --user=$NEW_USER --query "CREATE TABLE t AS int_table;"
$CLICKHOUSE_CLIENT --query "SHOW CREATE TABLE t;"
$CLICKHOUSE_CLIENT  --user=$NEW_USER --query "DROP TABLE t;" 2>&1| grep -Fo "Not enough privileges" | uniq
...
$CLICKHOUSE_CLIENT --user=$NEW_USER --query "DROP TABLE t;"

# Create new user user_test_02185 and user_test_02184 will grant SELECT permissions to user_test_02185
$CLICKHOUSE_CLIENT --query "CREATE USER user_test_02185 IDENTIFIED WITH no_password;"
...
$CLICKHOUSE_CLIENT --user=$NEW_USER --query "GRANT SELECT ON int_table TO user_test_02185"
NEW_USER1="user_test_02185"
$CLICKHOUSE_CLIENT --user=$NEW_USER1 --query "SELECT a FROM int_table"
