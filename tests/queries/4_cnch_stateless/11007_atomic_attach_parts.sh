...
DIR_PATH=$(${CLICKHOUSE_CLIENT} --query "SELECT directory FROM system.cnch_fs_lock WHERE database = currentDatabase() AND table = 'atomic_attach_parts';")

if [ "${DIR_PATH}" == "${HDFS_PATH_ROOT}/some_never_used_directory" ]; then
    echo "directory is correct"
else
    echo "PATH is not correct, please check SELECT directory FROM system.cnch_fs_lock WHERE database = currentDatabase() AND table = 'atomic_attach_parts'; the expected result is ${HDFS_PATH_ROOT}/some_never_used_directory"
    exit -1
fi
...
TXN_ID=$(${CLICKHOUSE_CLIENT} --query "SELECT txn_id FROM system.cnch_fs_lock WHERE database = currentDatabase() AND table = 'atomic_attach_parts';")
...
${CLICKHOUSE_CLIENT} --query "SELECT directory FROM system.cnch_fs_lock WHERE database = currentDatabase() AND table = 'atomic_attach_parts';"
