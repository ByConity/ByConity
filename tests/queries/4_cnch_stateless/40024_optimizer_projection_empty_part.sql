...
CREATE DATABASE IF NOT EXISTS test_proj_empty_db;
USE test_proj_empty_db;

DROP TABLE IF EXISTS test_proj_empty;

CREATE TABLE test_proj_empty
(
    `part` Int32,
    `key` Int32,
    `unused1` Int32,
...
INSERT INTO test_proj_empty(part, key, val)
SELECT
    number / 100000,
    0 + (number % 10),
    1
...
ALTER TABLE test_proj_empty ADD PROJECTION unused_proj1
(
SELECT
    unused1,
    sum(val)
...
ALTER TABLE test_proj_empty ADD PROJECTION unused_proj2
(
SELECT
    unused2,
    sum(val)
...
ALTER TABLE test_proj_empty ADD PROJECTION proj1
(
SELECT
    key,
    sum(val)
...
ALTER TABLE test_proj_empty ADD PROJECTION unused_proj3
(
SELECT
    unused3,
    sum(val)
