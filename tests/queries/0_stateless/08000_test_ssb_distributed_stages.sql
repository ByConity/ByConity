...
drop table if exists ci_ssb.lineorder_local;
drop table if exists ci_ssb.customer_local;
drop table if exists ci_ssb.part_local;
drop table if exists ci_ssb.dates_local;
drop table if exists ci_ssb.lineorder_flat_local;
drop table if exists ci_ssb.supplier_local;

-- CREATE TABLE ci_ssb.lineorder_local (`LO_ORDERKEY` UInt32, `LO_LINENUMBER` UInt8, `LO_CUSTKEY` UInt32, `LO_PARTKEY` UInt32, `LO_SUPPKEY` UInt32, `LO_ORDERDATE` Date, `LO_ORDERPRIORITY` LowCardinality(String), `LO_SHIPPRIORITY` UInt8, `LO_QUANTITY` UInt8, `LO_EXTENDEDPRICE` UInt32, `LO_ORDTOTALPRICE` UInt32, `LO_DISCOUNT` UInt8, `LO_REVENUE` UInt32, `LO_SUPPLYCOST` UInt32, `LO_TAX` UInt8, `LO_COMMITDATE` UInt32, `LO_SHIPMODE` LowCardinality(String)) ENGINE = MergeTree ORDER BY (LO_ORDERDATE, LO_ORDERKEY) SETTINGS index_granularity = 8192;

CREATE TABLE ci_ssb.lineorder_local
(
    LO_ORDERKEY             UInt32,
    LO_LINENUMBER           UInt8,
    LO_CUSTKEY              UInt32,
...
CREATE TABLE ci_ssb.lineorder as ci_ssb.lineorder_local ENGINE = Distributed(test_shard_localhost, ci_ssb, lineorder_local, rand());

CREATE TABLE ci_ssb.customer_local (`C_CUSTKEY` UInt32, `C_NAME` String, `C_ADDRESS` String, `C_CITY` LowCardinality(String), `C_NATION` LowCardinality(String), `C_REGION` LowCardinality(String), `C_PHONE` FixedString(15), `C_MKTSEGMENT` LowCardinality(String)) ENGINE = MergeTree ORDER BY C_CUSTKEY SETTINGS index_granularity = 8192;
CREATE TABLE ci_ssb.customer as ci_ssb.customer_local ENGINE = Distributed(test_shard_localhost, ci_ssb, customer_local, rand());

CREATE TABLE ci_ssb.part_local (`P_PARTKEY` UInt32, `P_NAME` String, `P_MFGR` LowCardinality(String), `P_CATEGORY` LowCardinality(String), `P_BRAND` LowCardinality(String), `P_COLOR` LowCardinality(String), `P_TYPE` LowCardinality(String), `P_SIZE` UInt8, `P_CONTAINER` LowCardinality(String)) ENGINE = MergeTree ORDER BY P_PARTKEY SETTINGS index_granularity = 8192;
CREATE TABLE ci_ssb.part as ci_ssb.part_local ENGINE = Distributed(test_shard_localhost, ci_ssb, part_local, rand());

CREATE TABLE ci_ssb.dates_local (`D_DATEKEY` Date, `D_DATE` String, `D_DAYOFWEEK` String, `D_MONTH` String, `D_YEAR` UInt32, `D_YEARMONTHNUM` UInt32, `D_YEARMONTH` String, `D_DAYNUMINWEEK` UInt32, `D_DAYNUMINMONTH` UInt32, `D_DAYNUMINYEAR` UInt32, `D_MONTHNUMINYEAR` UInt32, `D_WEEKNUMINYEAR` UInt32, `D_SELLINGSEASON` String, `D_LASTDAYINWEEKFL` UInt32, `D_LASTDAYINMONTHFL` UInt32, `D_HOLIDAYFL` UInt32, `D_WEEKDAYFL` UInt32) ENGINE = MergeTree ORDER BY D_DATEKEY SETTINGS index_granularity = 8192;
CREATE TABLE ci_ssb.dates as ci_ssb.dates_local ENGINE = Distributed(test_shard_localhost, ci_ssb, dates_local, rand());

CREATE TABLE ci_ssb.lineorder_flat_local (`LO_ORDERKEY` UInt32, `LO_LINENUMBER` UInt8, `LO_CUSTKEY` UInt32, `LO_PARTKEY` UInt32, `LO_SUPPKEY` UInt32, `LO_ORDERDATE` UInt32, `LO_ORDERPRIORITY` LowCardinality(String), `LO_SHIPPRIORITY` UInt8, `LO_QUANTITY` UInt8, `LO_EXTENDEDPRICE` UInt32, `LO_ORDTOTALPRICE` UInt32, `LO_DISCOUNT` UInt8, `LO_REVENUE` UInt32, `LO_SUPPLYCOST` UInt32, `LO_TAX` UInt8, `LO_COMMITDATE` UInt32, `LO_SHIPMODE` LowCardinality(String), `C_NAME` String, `C_ADDRESS` String, `C_CITY` LowCardinality(String), `C_NATION` LowCardinality(String), `C_REGION` LowCardinality(String), `C_PHONE` FixedString(15), `C_MKTSEGMENT` LowCardinality(String), `S_NAME` String, `S_ADDRESS` String, `S_CITY` LowCardinality(String), `S_NATION` LowCardinality(String), `S_REGION` LowCardinality(String), `S_PHONE` String, `P_NAME` String, `P_MFGR` LowCardinality(String), `P_CATEGORY` LowCardinality(String), `P_BRAND` LowCardinality(String), `P_COLOR` LowCardinality(String), `P_TYPE` LowCardinality(String), `P_SIZE` UInt8, `P_CONTAINER` LowCardinality(String)) ENGINE = MergeTree PARTITION BY toInt64(LO_ORDERDATE / 10000) ORDER BY (LO_ORDERDATE, LO_ORDERKEY) SETTINGS index_granularity = 8192;
CREATE TABLE ci_ssb.lineorder_flat as ci_ssb.lineorder_flat_local ENGINE = Distributed(test_shard_localhost, ci_ssb, lineorder_flat_local, rand());

CREATE TABLE ci_ssb.supplier_local (`S_SUPPKEY` UInt32, `S_NAME` String, `S_ADDRESS` String, `S_CITY` LowCardinality(String), `S_NATION` LowCardinality(String), `S_REGION` LowCardinality(String), `S_PHONE` String) ENGINE = MergeTree ORDER BY S_SUPPKEY SETTINGS index_granularity = 8192;
CREATE TABLE ci_ssb.supplier as ci_ssb.supplier_local ENGINE = Distributed(test_shard_localhost, ci_ssb, supplier_local, rand());
