...
${CLICKHOUSE_CLIENT} --query "CREATE TABLE test.00745_merge_tree_check_column_vadility (n UInt8, \`a_a_\` Map(String, LowCardinality(String))) Engine=MergeTree ORDER BY n settings min_bytes_for_wide_part = 0, enable_compact_map_data = 1"  2>&1 | grep -q "Code: 44," && echo "OK" || echo "Fail"
