...
select 'a';
select 1 + 1;
select * from test.test_scalar_local where id = 1;
select * from test.test_scalar_local where id = 1 and event = 'a';
select * from test.test_scalar_local as a join test.test_scalar_local as b on a.id = b.id order by id;
