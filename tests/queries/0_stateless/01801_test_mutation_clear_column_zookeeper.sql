...
select name from system.parts where table='test_mutation_1' and active order by name;
...
CREATE TABLE test_mutation_1 (`p_date` Date, `id` Int32, `age` Int32, `event` String, ab Array(Int32)) ENGINE = HaMergeTree('/clickhouse/test/tables/01801_test_mutation_clear_column_zookeeper', '1') PARTITION BY (p_date, id) ORDER BY id SETTINGS index_granularity = 8192;
CREATE TABLE test_mutation_2 (`p_date` Date, `id` Int32, `age` Int32, `event` String, ab Array(Int32)) ENGINE = HaMergeTree('/clickhouse/test/tables/01801_test_mutation_clear_column_zookeeper', '2') PARTITION BY (p_date, id) ORDER BY id SETTINGS index_granularity = 8192, ha_queue_update_sleep_ms=1000;
