...
create table test.test_bitmapjoinandcard (tag_id Int32, tag_value String, id_map BitMap64, p_date Date, split_id Int32) engine = MergeTree partition by (p_date, split_id) order by (p_date, split_id, tag_id, tag_value) settings index_granularity = 128;
create table test.test_bitmapjoinandcard_all (tag_id Int32, tag_value String, id_map BitMap64, p_date Date, split_id Int32) engine = Distributed(test_cluster_two_shards_localhost, 'test', 'test_bitmapjoinandcard');
...
from(
    ...
    (
        ...
        from test.test_bitmapjoinandcard_all
