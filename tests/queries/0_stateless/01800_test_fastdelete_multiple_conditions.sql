...
alter table test_mutation fastdelete age where id = 1 and id not in (2,3,4);

select * from test_mutation order by id;

alter table test_mutation fastdelete id where event = 'c' and id = 2;

select * from test_mutation order by id;

alter table test_mutation fastdelete id where event in (select event from test_mutation) and id = 1;
...
alter table test_mutation fastdelete id where event in ('a', 'b') and (event, id) in (select event, id from test_mutation);
...
insert into id_mutation values ('2021-01-01', 1);
insert into id_mutation values ('2021-01-01', 2);

alter table test_mutation fastdelete id where id in (select id from id_mutation) and id != 2;
