...
-- CREATE TABLE test.t40032 (`arr` Array(UInt16), `cnt` UInt64 ) ENGINE = CnchMergeTree() ORDER BY tuple();
...
-- SELECT  arrayReduce('sum',arrayJoin(pathCount(10, 10)(path, 1, cnt).2)) as sum2  FROM (select arrayMap(x -> tuple(x, ''), arr) as path, cnt from test.t40032) order by sum2;
