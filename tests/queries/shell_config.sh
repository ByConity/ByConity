...
export HDFS_PATH_ROOT=/user/clickhouse

export CNCH_WRITE_WORKER_HOST=${CNCH_WRITE_WORKER_HOST:="localhost"}
export CNCH_WRITE_WORKER_PORT_TCP=${CNCH_WRITE_WORKER_PORT_TCP:="52149"}
...
[ -v CLICKHOUSE_PASSWORD ] && CNCH_WRITE_WORKER_CLIENT_OPT+=" --password=${CLICKHOUSE_PASSWORD} "

[ -v CLICKHOUSE_PORT_TCP ] && CLICKHOUSE_CLIENT_OPT0+=" --port=${CLICKHOUSE_PORT_TCP} "
[ -v CLICKHOUSE_PASSWORD ] && CLICKHOUSE_CLIENT_OPT0+=" --password=${CLICKHOUSE_PASSWORD} "
[ -v CLICKHOUSE_PORT_TCP ] && CLICKHOUSE_BENCHMARK_OPT0+=" --port=${CLICKHOUSE_PORT_TCP} "
[ -v CLICKHOUSE_PASSWORD ] && CLICKHOUSE_BENCHMARK_OPT0+=" --password=${CLICKHOUSE_PASSWORD} "
[ -v CLICKHOUSE_CLIENT_SERVER_LOGS_LEVEL ] && CLICKHOUSE_CLIENT_OPT0+=" --send_logs_level=${CLICKHOUSE_CLIENT_SERVER_LOGS_LEVEL} "
[ -v CLICKHOUSE_DATABASE ] && CLICKHOUSE_CLIENT_OPT0+=" --database=${TENANT_DEFAULT_DB_PREFIX}${CLICKHOUSE_DATABASE} "
[ -v CLICKHOUSE_LOG_COMMENT ] && CLICKHOUSE_CLIENT_OPT0+=" --log_comment='${CLICKHOUSE_LOG_COMMENT}' "
[ -v CLICKHOUSE_DATABASE ] && CLICKHOUSE_BENCHMARK_OPT0+=" --database=${TENANT_DEFAULT_DB_PREFIX}${CLICKHOUSE_DATABASE} "
[ -v CLICKHOUSE_LOG_COMMENT ] && CLICKHOUSE_BENCHMARK_OPT0+=" --log_comment='${CLICKHOUSE_LOG_COMMENT}' "
export CLICKHOUSE_TENANT_DATABASE=${TENANT_DB_PREFIX}${CLICKHOUSE_DATABASE}
...
export COMMUNITY_CLICKHOUSE_BINARY=${COMMUNITY_CLICKHOUSE_BINARY:="/home/clickhouse"}
export COMMUNITY_CLICKHOUSE_CLIENT="$COMMUNITY_CLICKHOUSE_BINARY client --host=${CLICKHOUSE_HOST} --port=${CLICKHOUSE_PORT_TCP}"
...
then
  export CLICKHOUSE_URL_PARAMS="${CLICKHOUSE_URL_PARAMS}&database=${TENANT_HTTP_DEFAULT_DB_PREFIX}${CLICKHOUSE_DATABASE}"
else
  export CLICKHOUSE_URL_PARAMS="database=${TENANT_HTTP_DEFAULT_DB_PREFIX}${CLICKHOUSE_DATABASE}"
fi
...
export CLICKHOUSE_PURE_URL=${CLICKHOUSE_PURE_URL:="${CLICKHOUSE_PORT_HTTP_PROTO}://${CLICKHOUSE_HOST}:${CLICKHOUSE_PORT_HTTP}/"}
