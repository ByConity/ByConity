...
CREATE DICTIONARY test.dict_flat_no_ip_port1(id UInt64, a UInt64 DEFAULT 0, b Int32 DEFAULT -1, c String DEFAULT 'none') PRIMARY KEY id SOURCE(CLICKHOUSE(USER default TABLE 'table_for_no_ip_port_dict' PASSWORD '' DB 'test')) LIFETIME(MIN 3 MAX 3) LAYOUT(FLAT());

DROP DICTIONARY IF EXISTS test.dict_flat_no_ip_port1;
DROP DICTIONARY IF EXISTS test.dict_flat_no_ip_port;
DROP TABLE IF EXISTS test.interger_col;
DROP TABLE IF EXISTS test.table_for_no_ip_port_dict;
